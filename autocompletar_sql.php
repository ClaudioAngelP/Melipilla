<?php
  ...
  if($_GET['tipo']=='buscar_meds' or $_GET['tipo']=='buscar_meds_stock')
  {
    ...
    ";
              ...
              if(isset($_GET['arsenal']))
              {    
                    if($_GET['arsenal']=='1'){
                        $arsenal='AND art_arsenal=true';
                    }else{
                        $arsenal='';
                    }
              }
  ...
  if($_GET['tipo']=='pacientes') {
    ...
    if($_GET['tipo']=='pacientes_edad')
    {
        ...
        if(isset($_GET['pac_tipo'])){
            $tipo_busqueda=$_GET['pac_tipo']*1;
            if($tipo_busqueda==0){
                $w_where=" to_tsvector('spanish', pac_rut || ' ' || pac_appat || ' ' || pac_apmat || ' ' || pac_nombres || ' ' || pac_ficha) @@ to_tsquery('".$cadena."')";
            } elseif($tipo_busqueda==3) {
                $w_where=" pac_ficha ilike '%".$cadena."%'";
            } elseif($tipo_busqueda==1){
                $w_where=" pac_pasaporte='".$cadena."'";
            } elseif($tipo_busqueda==2) {
                $cadena=$cadena*1;
                $w_where="pac_id=$cadena";
            }
        }
        else {
            $w_where=" to_tsvector('spanish', pac_rut || ' ' || pac_appat || ' ' || pac_apmat || ' ' || pac_nombres || ' ' || pac_ficha) @@ to_tsquery('".$cadena."')";
        ...
        $consulta="
        SELECT 
        ltrim(upper(pac_rut),'0')as pac_rut, 
        ltrim(upper(pac_rut),'0')as pac_rut, 
        pac_appat || ' ' || pac_apmat || ' ' || pac_nombres,
        pac_ficha, 
 ...
 //*******************************************************************************
  ...
  if($_GET['tipo']=='carpeta') {
	...
	if($_GET['tipo']=='proc_prestacion') {
		...
		$procmed = pg_query($conn, "
		SELECT codigo, codigo, pc_desc, glosa, pc_id
		FROM codigos_prestacion
		JOIN procedimiento_codigo ON esp_id=$esp_id AND pc_codigo=codigo
		WHERE codigo ILIKE '%".$cadena."%' OR glosa ILIKE '%".$cadena."%' OR
		pc_desc ILIKE '%".$cadena."%' LIMIT 10
		");
	...
	if($_GET['tipo']=='proc_prestacion_examen') {
		$cadena = pg_escape_string(utf8_decode($_GET['cod_presta']));
        $esp_id = pg_escape_string(utf8_decode($_GET['esp_id']));
		if(isset($_GET['exam'])) {
            if(($esp_id*1)==0) {
                $w_esp="esp_id in (6117,6120)";
                $w_espkit="kit_esp_id in (6117,6120)";
            }
            else {
                $w_esp="esp_id=$esp_id";
                $w_espkit="kit_esp_id=$esp_id";
            }
        }
        else {
            $w_esp="esp_id=$esp_id";
        ...
        if(!isset($_GET['agenda'])) {
			$consulta="
            SELECT codigo,codigo2,pc_desc,glosa,pc_id,tipo
            FROM (
			SELECT distinct on (pc_id)pc_id,codigo,codigo2,pc_desc,glosa,tipo 
			FROM (
            SELECT codigo,(case when strpos(pc_codigo, '.')>0 then pc_codigo else codigo end)as codigo2,pc_desc,glosa,pc_id, 0 as tipo
            FROM codigos_prestacion
            JOIN procedimiento_codigo ON ".$w_esp." AND split_part(pc_codigo, '.', 1)=codigo and pc_activo ".$w_grupo_examen."
            WHERE 
            codigo ILIKE '%".$cadena."%'
            OR sinacentos(pc_desc) ILIKE sinacentos('%".$cadena."%')
            )as foo
            )as fooo
            UNION
            SELECT kit_codigo,kit_codigo,upper(kit_nombre || '##') as pc_desc,upper(kit_nombre || '##') as glosa,kit_id, 1 as tipo
            from examen_kits 
            where ".$w_espkit." 
            and (
				kit_codigo ILIKE '%".$cadena."%' 
				OR sinacentos(kit_nombre) ILIKE sinacentos('%".$cadena."%')
            )
            order by pc_desc 
            LIMIT 20";
            //print($consulta);
            ...
            if(isset($_GET['grupo_exam'])) {
                if($_GET['grupo_exam']!="0") {
                    if($_GET['grupo_exam']!="")
                        $w_grupo_examen=" AND pc_grupo_examen='".pg_escape_string(utf8_decode($_GET['grupo_exam']))."'";
                    else 
                        $w_grupo_examen=" AND (pc_grupo_examen is null or pc_grupo_examen='')";
                }
            }
            
            $consulta="
            SELECT codigo,codigo2,pc_desc,pc_grupo_examen,glosa,pc_id,0 as tipo
            FROM (
                SELECT distinct on (pc_id)pc_id,codigo,codigo2,pc_desc,pc_grupo_examen,glosa,0 as tipo
                FROM (
                    SELECT codigo,(case when strpos(pc_codigo, '.')>0 then pc_codigo else codigo end)as codigo2,pc_desc,pc_grupo_examen,glosa,pc_id,0 as tipo
                    FROM codigos_prestacion
                    JOIN procedimiento_codigo ON ".$w_esp." AND split_part(pc_codigo, '.', 1)=codigo and pc_activo ".$w_grupo_examen."
                    WHERE codigo ILIKE '%".$cadena."%' OR sinacentos(pc_desc) ILIKE sinacentos('%".$cadena."%')
                )as foo
            )as fooo
            UNION
            SELECT kit_codigo,kit_codigo,upper(kit_nombre || '##') as pc_desc,kit_grupo,upper(kit_nombre || '##') as glosa,kit_id,1 as tipo
            from examen_kits 
            where ".$w_espkit." and (kit_codigo ILIKE '%".$cadena."%' OR sinacentos(kit_nombre) ILIKE sinacentos('%".$cadena."%'))
            order by pc_desc 
            LIMIT 20";
        }
        
        $procmed = pg_query($conn, $consulta);
        if(pg_num_rows($procmed)>0)  {
            for($i=0; $i<pg_num_rows($procmed); $i++) {
                $array[$i]=pg_fetch_row($procmed);
                if(strstr(trim($array[$i][2]),'##')){
                    $tmp=explode('##',trim($array[$i][2]));
			$array[$i][2]=htmlentities($tmp[0]).'<span style="font-weight:bold;color:red;"> [KIT]</span>';
                } else {
                    $array[$i][2]=htmlentities(trim($array[$i][2]));
                }
                $array[$i][3]=trim($array[$i][3]);
                if(sizeof($array[$i][3])>40)
                    $array[$i][3]=substr($array[$i][3],0,37).'...';
  ...
  if($_GET['tipo']=='prestacion_patologia') {
    ...
    if($_GET['tipo']=='especialidad') {
        ...
        if(isset($_GET['esp']))
            ...
            $query.=" and (esp_codigo_ifl_usuario = '' or esp_codigo_ifl_usuario is null) ";
    ...
    if($_GET['tipo']=='especialidad_sigges')
    {
        $cadena = pg_escape_string(utf8_decode($_GET['cadena']));
        $esp = pg_query($conn, "SELECT esp_id, esp_id, esp_desc FROM especialidades
        WHERE esp_desc ILIKE '%$cadena%' AND (NOT esp_codigo_ifl_usuario = '' and NOT esp_codigo_ifl_usuario is null) ORDER BY esp_desc");
        if(pg_num_rows($esp)>0) {
            for($i=0; $i<pg_num_rows($esp); $i++) {
                $array[$i]=pg_fetch_row($esp);
                $array[$i][2]=htmlentities($array[$i][2]);
  ...
  if($_GET['tipo']=='servicios_hospitalizacion') {
    ...
    WHERE 
    tcama_tipo ILIKE '%$cadena%'  and  tcama_id>58
    LIMIT 10
    ");
  ...
  if($_GET['tipo']=='centros_pabellon') {
    ...
    centro_ruta ILIKE '.subdireccionmedica.%'
    LIMIT 10
    ");
...
if($_GET['tipo']=='centro_costo') {
  ...
  if($_GET['tipo']=='personal_pabellon') {
	...
	if($_GET['tipo']=='profesional_externo') {
		$cadena = pg_escape_string($_GET['profesional']);
		...
		prof_id,
		prof_rut,
		upper(prof_paterno || ' ' || prof_materno || ' ' || prof_nombres)
		FROM profesionales_externos
		WHERE
		prof_rut ILIKE '%".$cadena."%'
		OR
		prof_paterno ILIKE '%".$cadena."%'
		OR
		prof_materno ILIKE '%".$cadena."%'
		OR
		prof_nombres ILIKE '%".$cadena."%'
		OR
		prof_paterno || ' ' || prof_materno || ' ' || prof_nombres ILIKE '%".$cadena."%'
		ORDER BY prof_paterno, prof_materno, prof_nombres
		LIMIT 10
		");

		if(pg_num_rows($profe)>0)
			for($i=0; $i<pg_num_rows($profe); $i++) {
				$array[$i]=pg_fetch_row($profe);
				$array[$i][2]=htmlentities($array[$i][2]);
			}
			else
				$array='';

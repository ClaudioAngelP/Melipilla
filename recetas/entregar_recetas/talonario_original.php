<?php
    ...
    function Header()
    {
		GLOBAL $receta, $bodega, $mes, $dia, $anio, $fill, $margin, $fecha, $ratio, $log_fecha;
		...
		$this->SetFont('', '', 10*$ratio);
		$this->Cell(100*$ratio, 4, $receta['esp_doctor'],1,0,'L');
		$this->Ln();
    ...
    if($todas)
    {
        if(isset($_GET['gestion_cama']))    
        {
            ...
            $reg_recetas=cargar_registros_obj("SELECT receta_id 
            FROM receta where receta_bod_id=35 and receta_fecha_emision between '$fecha1 $h1l_w' and '$fecha2 $h2l_w'
            and $w_centro_ruta and $w_doc");
            if(!$reg_recetas)
            {
	...
	ORDER BY log_fecha ,log_fecha::time ASC");
        ...
        foreach($detalle as $art)
        {
            ...
            {
                $txt_dosis=number_format($recetad_cant, 1,',','.')." ".$art['art_unidad_administracion']." cada ".($art['recetad_horas']/$div_h)." ".$txt_horas." por ".($art['recetad_dias']/$div_d)." ".$txt_dias.".";
            }

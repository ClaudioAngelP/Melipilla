<?php
    ...
    if(isset($_GET['med_c'])) {
        ...
        $medico_q = pg_query($conn, "SELECT doc_id FROM doctores WHERE doc_rut='".pg_escape_string($medico_rut)."'");
    ...
    if(isset($_GET['receta_c'])) {
        ...
        $receta_q = pg_query($conn, "SELECT receta_id FROM receta WHERE receta_numero=".($receta_num)."");
...
<div class='sub-content3' style='height:450px;overflow:auto;'>
    <table width=100%>
        <tr class='tabla_header' style='font-weight: bold;'>
            <td colspan=4>Datos de la Receta</td>
            <td colspan=5 style='width: 100px;'>Acciones</td>
        </tr>
        <tr class='tabla_header' style='font-weight: bold;'>
        ...
        centro_nombre, receta_fecha_emision,receta_numero,receta_tipotalonario_id
        FROM receta
        ...
        for($i=0;$i<pg_num_rows($recetas);$i++) {
            ...
            <td style="text-align: center;">
                <i>'.htmlentities($fila[6]).'</i>
            </td>');
            if(($fila[7])*1==0)
                print('<td style="text-align: center;"><b><i>Normal</i></b></td>');
            else if(($fila[7])*1==1)
                print('<td style="text-align: center;"><b><i>Estupefecientes</i></b></td>');
            else
                print('<td style="text-align: center;"><b><i>Psicotr&oacute;picos</i></b></td>');
            ...
            <td>
            <center>
            <img src="iconos/zoom_in.png" style="cursor: pointer;" onClick="visualizar_receta('.$fila[0].');" alt="Ver Receta..." title="Ver Receta...">
            </center>
            </td>
            ');
            ...
            if(_cax(21)) {
                ...
                <td>
                <center>
                <img src="iconos/delete.png" style="cursor: pointer;" onClick="eliminar_receta('.$fila[0].');" alt="Eliminar Receta..." title="Eliminar Receta...">
                </center>
                </td>
                ');
            }

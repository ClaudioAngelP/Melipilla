...
if(!isset($_POST['confirma'])) {
	...
	// DEPRECATED!!! VERSION ANTIGUA DEL SIGGES
	//curl_setopt($ch, CURLOPT_URL, 'http://www.sigges.cl/actions/paciente/cartolaAction.do?dispatch=load&DataSource=dsAT_Dedicada');
	curl_setopt($ch, CURLOPT_URL, 'http://www.sigges.cl/actions/paciente/cupAction.do?dispatch=load&DataSource=dsAT_Dedicada');
	curl_setopt($ch, CURLOPT_FOLLOWLOCATION, false );
	curl_setopt($ch, CURLOPT_POST, 0);
	...
	file_put_contents('tmp/logpac_'.date('YmdHisu').'.txt', $datos);
	
	$docs = $domc->find('input[class=htmlbuttonP2]');
	...
	for($d=0;$d<sizeof($docs);$d++) {
		...
		if($doc_id!='' AND $caso_id!='')	{	
			...
			//$url_doc='http://www.sigges.cl/actions/paciente/cupAction.do?';
			//$url_doc.="dispatch=enviar&numDocu=".$doc_tipo."&idDocu=".$doc_id."&idCaso=".$caso_id."&nuevoFormulario=0";
			...
			file_put_contents('tmp/logdoc_'.$doc_tipo.'_'.$doc_id.'_'.date('YmdHisu').'.txt', $html_doc);

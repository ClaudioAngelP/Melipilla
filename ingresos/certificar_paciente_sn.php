...
User-Agent: Apache-HttpClient/4.1.1 (java 1.5)");
	 ...
	 if(!strstr($data,$rut)) {
		...
		$chq = cargar_registro("SELECT * FROM pacientes_fonasa WHERE pac_rut = '$rutdv' ORDER BY cert_fecha DESC LIMIT 1");
        ...
        if($chq) {
			...
			$pac=cargar_registro("SELECT * FROM pacientes WHERE pac_rut='$rutdv' ORDER BY pac_id DESC LIMIT 1;");
			...
			$prev=cargar_registro("SELECT * FROM prevision WHERE prev_id=$prevision");
       		        ...
       		        $prev['frec_id']=$func['frec_id']*1;
                	$prev['desc']=$prev['prev_desc'];
	                $prev['prais']='000';
//	                	return json_encode(false);a
			return json_encode($prev);
		}
        }

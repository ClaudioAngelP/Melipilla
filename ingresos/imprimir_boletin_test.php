<?php 
	...
	$b=cargar_registro("SELECT *, bolfec AS bolfec FROM boletines WHERE bolnum=".$bolnum);
	...
	//if(!$paga_cuota) {
		...
		for($i=0;$i<sizeof($p);$i++) {
			...
			if((($i==20 OR (($i-20)%27)==0)) AND $i<sizeof($p)-1) {
				$pdf->AddPage();
			}
...
1.	INSTITUCIONAL: A través del Consultorio de Atención Primaria, y solo debe cancelar al CRS el copago según Tramo de la previsión FONASA de acuerdo a las normas estipuladas para dichos cobros.
...
$pdf->Ln();
	...
	$pdf->SetFontSize(10);
	$pdf->Ln();
	$pdf->Cell(55,5,'ADMISOR CRS:',0,0,'L');

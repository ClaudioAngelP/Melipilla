...
CASE WHEN seg_id >0 THEN 
 ...
 ELSE (CASE WHEN (CASE WHEN (boletines.prvcod='12' OR b2.prvcod='12' OR boletines.prvcod='5' OR b2.prvcod='5' OR boletines.prvcod='6' OR b2.prvcod='6' or boletines.bolmod='mle' or  b2.bolmod='mle') 
		  ...
		  END) 
	THEN 
	   (CASE WHEN position('-' in boletin_detalle.bdet_codigo)>0 
	    THEN 
		codigo='FARMACIA_CODIGO' 
	    ELSE 
		boletin_detalle.bdet_codigo ilike codigos_prestacion_item.codigo || '%' 
...
boletin_detalle.bdet_codigo,codigos_prestacion_item.codigo,codigos_prestacion_item.item_nombre,tipo_prestacion,
...
");
	...
	//if(!$paga_cuota) {
		...
		for($i=0;$i<sizeof($cuentas_pre);$i++) {
			...
			$pdf->SetFontSize(8);
			$pdf->SetFont('','');				
			$pdf->Cell(25,5,substr(trim($fecha),0,16),1,0,'C');
			$pdf->SetFontSize(8);
			$pdf->SetFont('','B');				
			$pdf->Cell(20,5,trim($cuentas_pre[$i]['cuenta']),1,0,'C');
			$pdf->SetFont('','');				
			$pdf->SetFontSize(8);
			$pdf->Cell(90,5,trunc(trim($cuentas_pre[$i]['item_nombre']),45),1,0,'L');
			$pdf->Cell(5,5,trim(1),1,0,'C');
			$pdf->SetFontSize(10);
			$totalt+=$cuentas_pre[$i]['copago']*1;
			$totalb+=$cuentas_pre[$i]['copago']*1;
			if(!$paga_cuota) {
				$pdf->Cell(30,5,'$ '.number_format($cuentas_pre[$i]['copago'],0,',','.').'.-',1,0,'R');
				if($p[$i]['bdet_cobro']=='S') {
					if($b['garantia']=='t'){
						$pdf->Cell(30,5,'$ '.number_format($cuentas_pre[$i]['copago'],0,',','.').'.-',1,0,'R');
					}

<?php 
	...
	if(isset($_POST['xls'])) {
    	...
    	header("Content-Disposition: filename=\"resumen_items.xls\";");
		$xls=1; 
	} else 
		$xls=0;
	...
	for($i=0;$i<sizeof($l);$i++) {
		$grupo=substr($l[$i]['item_codigo'],0,2);
		$subgrupo=substr($l[$i]['item_codigo'],0,4);
		$presta=$l[$i]['item_codigo'];
...
<?php 
	...
	foreach($pr AS $codigo => $detalle) {
		$clase=$c++%2==0?'tabla_fila':'tabla_fila2';
		if($codigo=='') $codigo='<i>(Sin C&oacute;digo Asignado...)</i>';

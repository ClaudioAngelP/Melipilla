<?php 
	...
	function vdevolucion($devol,$xls=false,$ruta='') {
	
			if(!$xls) {
				return "<span style='cursor:pointer;white-space:nowrap;text-decoration:underline;font-weight:bold;color:blue;' onClick='abrir_devolucion($devol, \"$ruta\");'>
							".number_format($devol,0,',','.')."<img src='".$ruta."iconos/magnifier.png' width=10 height=10>
							</span>";
			} else {
				return ($devol*1);
			}
...
<table style='width:100%;'>
<tr class='tabla_header'><td><b><u>Devoluci&oacute;n de Prestaciones.</u></b></td></tr>
</table>

<table style='width:100%;'>
<tr class='tabla_header'>
<td>N. Devoluci&oacute;n</td>
<td>Bolet&iacute;n</td>
<td>RUT Cliente</td>
<td>Nombre Cliente</td>
<td>Monto Devoluci&oacute;n</td>
</tr>

<?php 

	$db=cargar_registros_obj("
		
SELECT * FROM devolucion_boletines
		JOIN boletines USING (bolnum)
		JOIN pacientes ON boletines.pac_id=pacientes.pac_id 
		WHERE dev_ejecuta::date >= '$fecha1' AND 
		dev_ejecuta::date <= '$fecha2' AND func_id_ejecuta=7 
		ORDER BY bolnum		
	");
	...
	if($db)
		for($i=0;$i<sizeof($db);$i++) {
			...
			$clase=($i%2==0)?'tabla_fila':'tabla_fila2';	
	//<td style='text-align:center;' align='center'>".vdevolucion($db[$i]['dev_id'])."</td>
	
			print("
				<tr class='$clase'>
				<td style='text-align:center;' align='center'>".($db[$i]['devol_id'])."</td>
				<td style='text-align:center;' align='center'>".vboletin($db[$i]['bolnum'])."</td>
				<td style='text-align:right;' align='right'>".$db[$i]['pac_rut']."</td>
				<td style='text-align:left;' align='left'>".htmlentities(($db[$i]['pac_appat'].' '.$db[$i]['pac_apmat'].' '.$db[$i]['pac_nombres']))."</td>
				<td style='text-align:right;' align='right'>".dinero($db[$i]['monto_total']*-1)."</td>
				</tr>
			");
			
			$total+=$db[$i]['monto_total']*-1;			

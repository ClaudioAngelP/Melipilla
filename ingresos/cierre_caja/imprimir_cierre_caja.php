<?php 
	...
	$pdf->Cell(140,6,'Nombre Recaudador',1,0,'C');	
	$pdf->Ln();
	...
	$efe_dev=cargar_registros_obj(" SELECT *,upper(pac_nombres || ' ' || pac_appat || ' ' || pac_apmat) AS pac_nombre FROM devolucion_boletines 	left join boletines on devolucion_boletines.bolnum=boletines.bolnum 	LEFT JOIN pacientes USING (pac_id) 	WHERE devolucion_boletines.bolnum IN (SELECT bolnum FROM apertura_cajas JOIN devolucion_boletines ON dev_ejecuta BETWEEN ac_fecha_apertura AND ac_fecha_cierre AND devolucion_boletines.func_id_ejecuta=apertura_cajas.func_id WHERE apertura_cajas.ac_id=$ac_id) ORDER BY boletines.bolnum");
	$tot_dev=cargar_registros_obj(" SELECT sum(monto_total)as monto_total FROM devolucion_boletines 	left join boletines on devolucion_boletines.bolnum=boletines.bolnum 	LEFT JOIN pacientes USING (pac_id) 	WHERE devolucion_boletines.bolnum IN (SELECT bolnum FROM apertura_cajas JOIN devolucion_boletines ON dev_ejecuta BETWEEN ac_fecha_apertura AND ac_fecha_cierre AND devolucion_boletines.func_id_ejecuta=apertura_cajas.func_id WHERE apertura_cajas.ac_id=$ac_id) ");
	...
	$pdf->SetFontSize(10);	
	$pdf->SetFont('Arial','');	
	$pdf->Multicell(100,5,utf8_decode(num2texto($total-50000)).' PESOS.',1,'C');

	$h=$pdf->GetY()-$tmpy;
	
	$pdf->SetXY($tmpx,$tmpy);
	
	$pdf->Cell(50,$h,'Total Recaudado:',1,0,'R');
	
	$pdf->SetFontSize(14);	
	$pdf->SetFont('Arial','B');	
	$pdf->Cell(50,$h,'$ '.number_format($total-50000,0,',','.').'.-',1,0,'C');
	...
	$pdf->SetFontSize(10);	
	$pdf->SetFont('Arial','');	
	$pdf->Multicell(100,5,utf8_decode(num2texto(50000)).' PESOS.',1,'C');

	$h=$pdf->GetY()-$tmpy;
	
	$pdf->SetXY($tmpx,$tmpy);
	
	$pdf->Cell(50,$h,'Fondo Fijo:',1,0,'R');
	
	$pdf->SetFontSize(14);	
	$pdf->SetFont('Arial','B');	
	$pdf->Cell(50,$h,'$ '.number_format(50000,0,',','.').'.-',1,0,'C');
	...
	*/
	...
	if($efe3) {
		...
		$pdf->Cell(155,5,'Total Ingresos:',1,0,'R');	
		$pdf->Cell(45,5,'$'.number_format($total_efectivo-50000-$tot_dev[0]['monto_total']*1,0,',','.').'.-',1,0,'R');
		$pdf->Ln();	
		...
		if($efe_dev) {
				...
				for($i=0;$i<sizeof($efe_dev);$i++) {
					...
					$pdf->Cell(25,5,$identifica_efe3,1,0,'R',$fill);	
					$pdf->Cell(90,5,trunc($efe_dev[$i]['pac_nombre'],40),1,0,'L',$fill);
				...
				$pdf->Cell(155,5,'Total Devoluciones:',1,0,'R');	
				$pdf->Cell(45,5,'$'.number_format($tot_dev[0]['monto_total']*1,0,',','.').'.-',1,0,'R');
				$pdf->Ln();	
		...
		$pdf->Cell(155,5,'Total Efectivo:',1,0,'R');	
		$pdf->Cell(45,5,'$'.number_format($total_efectivo-50000,0,',','.').'.-',1,0,'R');
		$pdf->Ln();	
		$pdf->Ln();	
	...
	if($chq) {
		...
		$pdf->Cell(40,5,'$'.number_format($total_chq,0,',','.').'.-',1,0,'R');
		$pdf->Ln();	
	...
	if($pag5) {
		...
		$pdf->Cell(25,5,'Bonos MEDS',1,0,'C');	
		$pdf->Cell(20,5,'Nro. Corr.',1,0,'C');	
		$pdf->Cell(25,5,utf8_decode('R.U.T.'),1,0,'C');	
		$pdf->Cell(55,5,'Nombre',1,0,'C');	
		$pdf->Cell(25,5,'Tipo Pago',1,0,'C');	
		...
		for($i=0;$i<sizeof($pag5);$i++) {
		
			$pdf->Cell(25,5,'#'.($i+1),1,0,'C');	
			$pdf->Cell(20,5,$pag5[$i]['bolnum'],1,0,'C');
			if(($pag5[$i]['pac_rut']+'')==''){
				$identifica_pag=$pag5[$i]['id_sidra']	;
			
			}else{
				$identifica_pag=formato_rut($pag5[$i]['pac_rut'])	;
			
			}	
			$pdf->Cell(25,5,$identifica_pag,1,0,'R');	
			$pdf->Cell(55,5,trunc($pag5[$i]['pac_nombre'],20),1,0,'L');	
			$pdf->Cell(25,5,$pag5[$i]['fpago_nombre'],1,0,'L');	
			$pdf->Cell(25,5,$pag5[$i]['numero'],1,0,'C');	
			//$pdf->Cell(30,5,$pag[$i]['fecha'],1,0,'C');	
			$pdf->Cell(25,5,'$'.number_format($pag5[$i]['monto'],0,',','.').'.-',1,0,'R');
			$pdf->Ln();	
	...
	if($detalle_pagares) {
		...
		$pdf->Cell(35,5,'$'.number_format($pagares[0]['total'],0,',','.').'.-',1,0,'R');
		$pdf->Cell(35,5,'',1,0,'R');
		$pdf->Ln();	

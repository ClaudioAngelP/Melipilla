<?php 
	...
	$total=$_GET['total']*1;
	$prestaArray=json_decode($_GET['prestaArray']);
	$derpabArray=json_decode($_GET['derpabArray']);
	
	//var_dump($presta);
	
	for($i=0;$i<sizeof($prestaArray);$i++) {
		
		$codigo_prestacion[$i*3]=$prestaArray[$i]->codigo;
		$codigo_prestacion[$i*3+1]=substr($prestaArray[$i]->glosa,0,30);
		$codigo_prestacion[$i*3+2]=$prestaArray[$i]->copago;	
		
	}
	$sizeof=sizeof($codigo_prestacion);
		$codigo_prestacion[$sizeof]=$derpabArray[0];
		$codigo_prestacion[$sizeof+1]=$derpabArray[1];
		$codigo_prestacion[$sizeof+2]=$derpabArray[2];
		
		$codigo_prestacion[$sizeof+3]=$derpabArray[3];
		$codigo_prestacion[$sizeof+4]=$derpabArray[4];
		$codigo_prestacion[$sizeof+5]=$derpabArray[5];
...
<script>
	...
	*/
	...
	presta_codes=<?php echo json_encode($codigo_prestacion); ?>;
	function combo_tipo_presta(vsel) {
		
		var html='';	
		
		for(var i=0;i<presta_codes.length/3;i++) {
			var sel=(presta_codes[i*3]==vsel)?'SELECTED':'fail';
			if(presta_codes[i*3+0]!=null)
			{
			html+='<option value="'+i+'">N'+i+' Codigo: '+presta_codes[i*3+0]+' Glosa: '+presta_codes[i*3+1]+' Monto: '+presta_codes[i*3+2]+'</option>';
	...
	function calcular_totales(refresh) {
		...
		for(var i=0;i<nro_cheques;i++) {
			...
			htmlcheques+='name="rut_otr_'+i+'" class="cheq" value="'+c.rut+'" onKeyUp="validacion_rut(this);">';
			htmlcheques+='</td>';
			htmlcheques+='<td>';
		...
		htmlcheques+='<td>Rut</td>';
		htmlcheques+='<td>N&uacute;mero</td>';
		htmlcheques+='<td>Fecha</td>';
		htmlcheques+='<td>Monto</td>';
		htmlcheques+='<td>Prestaci&oacute;n</td>';
		htmlcheques+='</tr>';	
		...
		for(var i=0;i<nro_otras;i++) {
			...
			htmlcheques+='</select></td>';
			htmlcheques+='<td>';
			htmlcheques+='<input type="text" id="rut_otr_'+i+'" style="text-align:right;" ';
			htmlcheques+='name="rut_otr_'+i+'" class="cheq" value="'+c.rut+'">';
			htmlcheques+='</td>';
			htmlcheques+='<td>';
			...
			htmlcheques+='</td>';
			htmlcheques+='<td>';
			htmlcheques+='<select  id="presta_otr_'+i+'" style="text-align:right;" ';
			htmlcheques+='name="presta_otr_'+i+'" class="cheq">';
			htmlcheques+='<option value="-1">SIN OPCION</option>';
			htmlcheques+=combo_tipo_presta(c.presta);
			htmlcheques+='</select>';			
			htmlcheques+='</td>';
			htmlcheques+='</tr>';		
	...
	function confirmar_pago() {
		...
		for(var i=0;i<pago.otras.length;i++) {

					
			if(!validacion_rut($('rut_otr_'+i))) {

				alert("El RUT de alg&uacute;n(os) bono(s) no es v&aacute;lido.".unescapeHTML());
				return;
			}
			if(!validacion_fecha($('fecha_otr_'+i)) ) {
				alert("La fecha de alg&uacute;n(os)  bono(s) no es v&aacute;lido.".unescapeHTML());
				return;			
			}	
			
			if($('presta_otr_'+i).value*1==-1){
				alert("No ha selecciondo la prestaci&oacute;n.".unescapeHTML());
				return;
			}
			
			if($('numero_otr_'+i).value=='' ||
				$('monto_otr_'+i).value*1==0 				
				//||$('presta_otr_'+i).value=='-1'
				) {
				alert("Los dato(s) de algun(os) bono(s) est&aacute; vac&iacute;os.".unescapeHTML());
				return;
			}
		...
		for(i=0;i<pago.otras.length;i++) {
			c=pago.otras[i];
			io+=c.tipo+'|'+c.rut+'|'+c.numero+'|'+c.fecha+'|'+c.monto+'|'+c.presta+'[|]';
		}

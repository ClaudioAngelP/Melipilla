<?php 
	...
	if($filtro==0) {
		...
		$fecha_w="nom_fecha::date>='01/02/2014'";
		$fecha2_w="fecha_despacho::date>='01/02/2014'";
	} else if($filtro==3) {
	...
	*/
	...
	$p=cargar_registros_obj("
		...
		SELECT * FROM (
			...
			SELECT 
			nomd_id AS presta_id, nomd_tipo_atencion as tipo_atencion,
	...
	function info_codigo2($codigo, $modalidad, $rut) {
	...
	function calcular_precio($codigo, $modalidad, $rut) {
	
		GLOBAL $pac, $prev_id, $frec_id, $pagare;
	
		$cod=info_codigo2($codigo, $modalidad, $rut);
		...
		if($pagare==3) {
			...
			$cod2=info_codigo2($codigo, 'mle', $rut); if($cod2) { $valor=$cod2['copago_c']; $total=$cod2['copago_b']; } else { $valor=$cod['precio']; $total=$cod['precio']; }
			...
			if($frec_id>0) {
				...
				elseif($prev_id==5) { $cod2=info_codigo2($codigo, 'mle', $rut); if($cod2) $valor=$cod2['copago_b']; else $valor=$cod['precio']; }
				elseif($prev_id==6) { $cod2=info_codigo2($codigo, 'mle', $rut); if($cod2) $valor=$cod2['copago_b']; else $valor=$cod['precio']; }
				else $valor=$cod['copago_d'];
			
			} else {
			
				if($prev_id==6) $valor=$cod['precio'];
				elseif($prev_id==5) { $cod2=info_codigo2($codigo, 'mle', $rut); if($cod2) { $total=$cod2['copago_b']; $valor=$cod2['copago_b']; } else $valor=$cod['precio']; }
			...
			if($frec_id>0) {
				...
				$cod2=info_codigo2($codigo, 'mai', $rut); if($cod2) { $total=$cod2['precio']; $valor=$cod2['precio']; } else { $total=$cod['precio']; $valor=$cod['precio']; }
	...
	for($i=0;$i<sizeof($p);$i++) {
		...
		$_codigo=info_codigo2($p[$i]['codigo'], $p[$i]['modalidad'], $p[$i]['doc_rut']);

		list($p[$i]['precio'], $p[$i]['copago'])=calcular_precio($p[$i]['codigo'], $p[$i]['modalidad'], $p[$i]['doc_rut']);
		...
		if($_codigo['canasta']!='') {
			...
			list($p2[$num]['precio'],$p2[$num]['copago'])=calcular_precio($c['codigo'], $p[$i]['modalidad'], $p[$i]['doc_rut']);

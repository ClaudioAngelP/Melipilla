<?php 
	...
	if($filtro==0) {
		$fecha_w="nom_fecha::date BETWEEN '2014-01-14' and CURRENT_DATE::date";
		$fecha2_w="log_fecha::date BETWEEN '2014-01-14' and CURRENT_DATE::date";
	} else if($filtro==1) {
		...
		$fecha_w="nom_fecha::date>= '2014-01-14'";
		$fecha2_w="log_fecha::date>='2014-01-14'";
	}
...
//	$prev_id=$prev_id;
	...
	if($modalidad=='mai') {
		if ($prev_id*1>=10 AND $prev_id*1<=15 AND $pac['edad_anios']*1>=60) $campo='0';
		else if($prev_id*1==12) $campo='codigos_prestacion.copago_a';
		else if($prev_id*1==10) $campo='codigos_prestacion.copago_b';
		else if($prev_id*1==11) $campo='codigos_prestacion.copago_c';
		else if($prev_id*1==15) $campo='codigos_prestacion.copago_d';
		
		$campo2='codigos_prestacion.precio';
	} else {
		if ($prev_id*1>=10 AND $prev_id*1<=15) { $campo='codigos_prestacion.transferen'; $campo2='codigos_prestacion.transferen'; }
	...
	if($prev_id*1==5) {
		...
		SELECT * FROM (
			...
			SELECT 
			recetad_id AS presta_id, 'AT' as tipo_atencion,
			art_codigo,
			glosa,
			...
			WHERE receta_paciente_id=$pac_id AND $fecha2_w AND recetad_pago=0
			group by recetad_id,art_codigo,glosa,precio,transferen,log_fecha,tipo,copago_d

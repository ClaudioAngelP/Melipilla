<?php 
    ...
    $ctacte=cargar_registro("SELECT * FROM fap WHERE fap_id=$fap_id AND fap_fecha_egreso IS NULL ORDER BY fap_fecha_egreso LIMIT 1;");
    if($ctacte)
        $fap_id=$ctacte['fap_id']*1;
    else
        $fap_id=0;
...
?>
<html>
    <title>Hoja de Cargo URGENCIA</title>
    <?php cabecera_popup('../..'); ?>
    <script>
		...
		//parameters:'hosp_id=<?php echo $hosp_id; ?>&fap_id=<?php echo $fap_id; ?>'
                parameters:'fap_id=<?php echo $fap_id; ?>'
            });
        }
		...
		return;
            ...
            {
                method:'post',
		//parameters:'hosp_id=<?php echo $hosp_id; ?>&fap_id=<?php echo $fap_id; ?>&'+$('art_id').serialize()+'&'+$('art_cantidad').serialize(),
                parameters:'fap_id=<?php echo $fap_id; ?>&'+$('art_id').serialize()+'&'+$('art_cantidad').serialize(),
                onComplete:function() {
                    listado_hoja_cargo();
                    $('art_id').value='';
                    $('art_codigo').value='';
	...
	eliminar_hc=function(urghc_id) {
            var myAjax=new Ajax.Request('sql_agregar_hoja_cargo.php',
            {
                method:'post',
		parameters:'urghc_id='+urghc_id,
		onComplete:function() {
                    listado_hoja_cargo();
		}
            });
	...
	agregar_prestacion=function() {
		...
		return;
            ...
            {
                method:'post',
		//parameters:'hosp_id=<?php echo $hosp_id; ?>&fap_id=<?php echo $fap_id; ?>&'+$('codigo_presta').serialize()+'&'+$('nombre_presta').serialize()+'&'+$('cant_presta').serialize(),
                parameters:'fap_id=<?php echo $fap_id; ?>&'+$('codigo_presta').serialize()+'&'+$('nombre_presta').serialize()+'&'+$('cant_presta').serialize(),
                onComplete:function() {
	...
	eliminar_hospp=function(hospp_id) {
        ...
        listado_recien_nacidos=function() {
            var myAjax=new Ajax.Updater('listado_recien_nacidos','../asignar_camas/listado_recien_nacidos.php',
            {
                method:'post',
		...
		return;
            ...
            var myAjax=new Ajax.Request('../asignar_camas/sql_agregar_recien_nacido.php',
            {
                method:'post',
		parameters:'hosp_id=<?php echo $hosp_id; ?>&fap_id=<?php echo $fap_id; ?>&'+$('rn_condicion').serialize()+'&'+$('rn_sexo').serialize()+'&'+
                $('rn_peso').serialize()+'&'+$('rn_apgar').serialize(),
                onComplete:function() {
        ...
        <div class='sub-content'>
            <table style='width:100%;'>
                <tr>
                    ...
                    <td style='font-weight:bold;font-size:18px;color:blue;'><?php echo $fap_id; ?></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td style='font-weight:bold;font-size:16px;color:yellowgreen;'>
                        <?php echo $r[0]['pac_nombres'].' '.$r[0]['pac_appat'].' '.$r[0]['pac_apmat']; ?>
                    </td>
                </tr>
            </table>
        </div>

<?php 
    ...
    if($hp){
        for($i=0;$i<sizeof($hp);$i++) {
            ...
            <td>
            <center>
            <input type='checkbox' id='hospp_".$hp[$i]['hospp_id']."' name='hospp_".$hp[$i]['hospp_id']."' value='' $checked onClick='realizar_hospp(".$hp[$i]['hospp_id'].");' />
            </center>
            </td>
            <td style='text-align:center;'>".$hp[$i]['hospp_fecha_realizado']."</td>
            ");
            
            print("<td><center><img src='../../iconos/delete.png' style='cursor:pointer;' onClick='eliminar_hospp(".$hp[$i]['hospp_id'].");'></center></td>");
            print("</tr>");
        }
	print("</table>");
    }

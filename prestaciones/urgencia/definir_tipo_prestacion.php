<?php 
    ...
    $pp=cargar_registros_obj("SELECT * FROM fap_pabellon_prestaciones WHERE ppresta_codigo='$codigo' ORDER BY ppresta_id;");
    if(!$pp)
    {
        exit("PPI");
    }
    $pac=cargar_registro("SELECT *,date_part('year',age(now()::date, pac_fc_nac)) as edad_anios FROM pacientes WHERE pac_id=$pac_id;");

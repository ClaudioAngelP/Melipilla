<?php 
    ...
    $fap_llegada=$_POST['fap_llegada']*1;
    ...
    */
    if(!$urgencia) {
        ...
        $chk=cargar_registros_obj("SELECT * FROM fap LEFT JOIN pacientes on pac_id=fap_pac_id WHERE fap_pac_id=$pac_id AND fap_fecha::date='$fecha'");
        if($chk AND $confirma==0) {
            // En caso de exitir FAP creado para la misma fecha,
        ...
        pg_query("INSERT INTO fap VALUES (
        $fid,
        0,
        nextval('fap_fap_id_seq'),
        '$fecha $hora',
        $fap_tipo_consulta,
        $fap_pronostico,
        -1,

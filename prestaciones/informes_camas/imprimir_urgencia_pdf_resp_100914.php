<?php
	...
	$q=cargar_registros_obj("
			...
			--LEFT JOIN hospitalizacion_condicion USING (hcon_id)
			WHERE hosp_anulado!=1 AND t1.tcama_id=49 AND hosp_numero_cama>0 AND hosp_fecha_egr IS NULL
		) AS foo ORDER BY hosp_numero_cama" );
	
	function dibujar_encabezado_tabla($pdf,$query,$q,$id_medico,$medico){
		...
		$pdf->Ln();
		imprimir_urgencia($pdf,$query,$q,$medico,$id_medico);
	...
	function imprimir_urgencia($pdf,$query,$q,$medico,$id_medico)
	{
		//if($q)
		$t=0;$pasillo=0;$hidra=0;$ad=0;$sala=0;
	...
	if ($q[$i]['cama_id']==143 || $q[$i]['cama_id']==126){
			$pasillo++;
		}
	if ($q[$i]['cama_id']==125){
			$hidra++;
	...
	if ($q[$i]['cama_id']==127 || $q[$i]['cama_id']==128){
			$sala++;
		}
	...
	$pdf->Cell(268, 10, utf8_decode('Detalle'),0,0,'C');
	$pdf->Ln(15);
	$pdf->SetFont('Arial','B',10);
	$pdf->Cell(50, 5, utf8_decode('Médico Jefe:'),1,0,'R');
	$pdf->Cell(75, 5,$id_medico,1,0,'C');
	$pdf->Cell(83, 5, utf8_decode(''),0,0,'R');
	$pdf->Cell(35, 5, utf8_decode('Sala:'),1,0,'R');
	$pdf->Cell(25, 5,$sala,1,0,'C');
	$pdf->Ln();
	$pdf->Cell(50, 5, utf8_decode('Traumatólogo:'),1,0,'R');
	$pdf->Cell(75, 5, utf8_decode(''),1,0,'R');
	$pdf->Cell(83, 5, utf8_decode(''),0,0,'R');
	$pdf->Cell(35, 5, utf8_decode('Pasillo:'),1,0,'R');
	$pdf->Cell(25, 5,$pasillo,1,0,'C');
	$pdf->Ln();
	$pdf->Cell(50, 5, utf8_decode('Enfermera (o) Turno:'),1,0,'R');
	$pdf->Cell(75, 5, utf8_decode(''),1,0,'R');
	$pdf->Cell(83, 5, utf8_decode(''),0,0,'R');
	$pdf->Cell(35, 5, utf8_decode('Box HIDR:'),1,0,'R');
	$pdf->Cell(25, 5, $hidra,1,0,'C');
	$pdf->Ln();
	$pdf->Cell(208, 5, utf8_decode(''),0,0,'R');
	$pdf->Cell(35, 5, utf8_decode('Box AD:'),1,0,'R');
	$pdf->Cell(25, 5,$ad,1,0,'C');
	$pdf->Ln();
	$pdf->Cell(208, 5, utf8_decode(''),0,0,'R');
...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		GLOBAL $fecha1, $fecha2, $func_nombre, $ac, $medico,$id_medico;
		$this->Image('../../imagenes/logo_hp.jpg',10,8,33);
...
function NbLines($w,$txt)
{
	...
	$pdf->AliasNbPages();
	$pdf->AddPage('L', 'letter');
	dibujar_encabezado_tabla($pdf,$query,$q,$medico,$id_medico);
	$pdf->Output();

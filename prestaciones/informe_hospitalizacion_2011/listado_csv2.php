<?php 
   ...
   $fecha2=pg_escape_string($_POST['fecha2']);
	...
	JOIN doctores on hosp_doc_id=doc_id
	...
	$csv.=$csv_sep."ETNIA".$csv_sep."Domicilio".$csv_sep."Comuna".$csv_sep.utf8_decode("Código-Comuna");
	...
	$csv.=$csv_sep."Diagnostico 5to".$csv_sep.utf8_decode("Código-5to-Diagnostico");
	...
	$csv.=$csv_sep."Nombre-Doctor".$csv_sep."Rut-Doctor";	
	$csv.=$csv_end;
	if($datos)
	{
		for($i=0;$i<count($datos);$i++)
		{
			...
			$csv.=$csv_sep.$datos[$i]['pac_fono'].$csv_sep.$datos[$i]['getn_id']		
			.$csv_sep.$datos[$i]['pac_direccion'].$csv_esp.$datos[$i]['sector_nombre']
			...
			$d=cargar_registros_obj("
				SELECT *, pdiag_fecha::date as pg_diag_fecha1 FROM paciente_diagnostico JOIN diagnosticos USING (diag_cod) WHERE hosp_id=".$datos[$i]['hosp_id']." AND pdiag_tipo=0
				order by pdiag_orden", true);
			if($d)
			{			
				for($x=0;$x<5;$x++) 
				{
					if(isset($d[$x]))
					{
						$csv.=$csv_sep.$d[$x]['diag_desc'].$csv_sep.$d[$x]['diag_cod'];
			...
			{			
				...
				for($a=0;$a<5;$a++)
				{
					if(isset($p[$a]))
					{						
						$csv.=$csv_sep.$p[$a]['presta_fecha1'].$csv_sep.$p[$a]['glosa'].$csv_sep.$p[$a]['codigo'];
					}
					else 
					{
						$csv.=$csv_sep.$csv_sep.$csv_sep;

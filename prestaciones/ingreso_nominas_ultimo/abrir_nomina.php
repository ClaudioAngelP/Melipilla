<?php 
    ...
    else
    {
        ...
        SELECT 
        pacientes.*, nomina_detalle.*, diag_desc,nom_motivo,esp_recurso, 
        date_part('year',age(pac_fc_nac)) as edad, cancela_desc,nom_esp_id
        FROM nomina_detalle
        JOIN nomina USING (nom_id)
        JOIN especialidades ON nom_esp_id=esp_id
        LEFT JOIN pacientes USING (pac_id)
...
<?php
    ...
    if($lista)
	    ...
	    <option value='R' ".($lista[$i]['nomd_tipo']=='R'?'SELECTED':'').">R</option>
            ...
            */
            print("<td style='text-align:center;font-weight:bold;font-size:16px;'>".$lista[$i]['nomd_tipo']."</td>
            <td style='text-align:center;font-weight:bold;font-size:16px;'>".$lista[$i]['nomd_extra']."</td>");
            print("
            <td>
                <center>
                    <input type='text' id='nomd_diag_".$lista[$i]['nomd_id']."' name='nomd_diag_".$lista[$i]['nomd_id']."' style='width:80%;' value='".htmlentities($lista[$i]['nomd_diag'])."' />
                </center>
            </td>
            <td>
                <center>
                    <select id='nomd_sficha_".$lista[$i]['nomd_id']."' name='nomd_sficha_".$lista[$i]['nomd_id']."'>
            ...
            <td style='white-space:nowrap;'>
                <center>");
                    //<select onChange='calcular_totales();' style='width:100px;' id='nomd_diag_cod_".$lista[$i]['nomd_id']."' name='nomd_diag_cod_".$lista[$i]['nomd_id']."'>
                    print("
                    <select onChange='calcular_totales();' style='width:100px;' id='nomd_diag_cod_".$lista[$i]['nomd_id']."' name='nomd_diag_cod_".$lista[$i]['nomd_id']."'>
            ...
            <td style='white-space:nowrap'>
                <center>
                    <select id='nomd_motivo_".$lista[$i]['nomd_id']."' name='nomd_motivo_".$lista[$i]['nomd_id']."'>
                        <option value='S' ".($lista[$i]['nomd_motivo'][0]=='S'?'SELECTED':'').">S</option>
                        <option value='N' ".($lista[$i]['nomd_motivo'][0]!='N'?'SELECTED':'').">N</option>
                    </select>
                    <select id='nomd_motivo2_".$lista[$i]['nomd_id']."' name='nomd_motivo2_".$lista[$i]['nomd_id']."'>
            ...
            <td style='white-space:nowrap;'>
                <center>
                    <select style='width:100px;' onChange='calcular_totales();' id='nomd_origen_".$lista[$i]['nomd_id']."' name='nomd_origen_".$lista[$i]['nomd_id']."'>
                    ...
                    <input type='button' $origen_enabled id='origen_".$lista[$i]['nomd_id']."' name='origen_".$lista[$i]['nomd_id']."' value='[S]' onClick='sel_origen(".$lista[$i]['nomd_id'].");' style='border:0px;margin:0px;'  />
                </center>
            </td>
            <td>
                <center>
                    <select id='nomd_auge_".$lista[$i]['nomd_id']."' name='nomd_auge_".$lista[$i]['nomd_id']."'>
            ...
            if($ficha_clinica)
            {
                ...
                <td>
                    <center>
                        <img src='iconos/table_edit.png'  style='cursor:pointer;' onClick='registrar(".($lista[$i]['nomd_id']).");' />
                    </center>
                </td>
                ");
            }
		...
		$icono='calendar';
            ...
            <center>
                <img src='iconos/phone.png'  style='cursor:pointer;' alt='Gestiones Citaci&oacute;n' title='Gestiones Citaci&oacute;n' onClick='gestiones_citacion(".$lista[$i]['nomd_id'].");' />

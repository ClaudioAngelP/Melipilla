...
<script>
    ...
    calcular_totales=function()
    {
        ...
        for(var i=0;i<dnomina.length;i++)
        {
            ...
            if($('nomd_origen_'+nomd_id).value=='A')
            {
                ...
                if($('nomd_institucion_'+nomd_id).value=='')
                    sel_origen(nomd_id);
            }
            else
            {
	...
	left=Math.round(screen.width/2)-325;
        new_win = window.open('prestaciones/ingreso_nominas/form_paciente.php?'+$('nom_id').serialize()+'&'+$('pac_id').serialize()+'&'+$('nomd_hora').serialize()+'&'+$('nomd_hora_extra').serialize()+'&'+$('duracion').serialize()+'&'+$('pac_rut').serialize(),
    ...
    sel_origen=function(nomd_id)
    {
        ...
        new_win = window.open('prestaciones/ingreso_nominas/seleccionar_opcion.php?tipo=2&valor='+valor+'&nomd_id='+nomd_id,
        'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
        'menubar=no, scrollbars=yes, resizable=no, width=680, height=500, '+
        'top='+top+', left='+left);
        new_win.focus();
...
<center>
    <div class='sub-content' style='width:95%;'>
        <form id='info_nominas' onSubmit='return false;'>
            ...
            <div class='sub-content2' style='height:260px;overflow:auto;' id='listado_nominas'>
            </div>
            <center>
                <table>
                    <tr>

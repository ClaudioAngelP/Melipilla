<?php 
    ...
    $n=cargar_registro("SELECT *, nom_fecha::date,
							(SELECT COUNT(*) FROM nomina_detalle 
							 WHERE nomina_detalle.nom_id=nomina.nom_id) 
							 AS cantidad   						
   						FROM nomina
...
function guardar_copia() {
	...
	var myAjax=new Ajax.Request(
		...
		{
			method:'post',
			parameters:$('nom_id').serialize()+'&'+$('doc_id').serialize()+'&'+$('fecha1').serialize()+'&'+$('datos').serialize(),
			onComplete:function() {
...
<tr><td style='text-align:right;width:30%;'>Nro. de Folio:</td>
<td style='font-weight:bold;font-size:16px;'>
<?php echo $n['nom_folio']; ?>
</td></tr>

<tr><td style='text-align:right;'>Profesional:</td><td>
<?php echo $n['doc_nombres'].' '.$n['doc_paterno'].' '.$n['doc_materno']; ?>
</td></tr>

<tr><td style='text-align:right;'>Especialidad:</td><td>
<?php echo $n['esp_desc']; ?>
</td></tr>

<tr><td style='text-align:right;'>Nro. de Registros:</td>
<td style='font-weight:bold;font-size:16px;'>
<?php echo $n['cantidad']; ?>
</td></tr>
...
<input type='text' name='fecha1' id='fecha1' size=10
  style='text-align: center;' value='<?php echo $n['nom_fecha'] ?>' />
...
<center>
<input type='button' id='copiar' name='copiar' onClick='guardar_copia();'
value='-- Realizar Copia de la N&oacute;mina... --' />
</center>

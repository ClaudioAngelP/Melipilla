<?php 
	...
	$hc=cargar_registros_obj("
		...
		SELECT * FROM (
			...
			(cantidad*art_val_ult) AS subtotal, 0 AS hosphc_id, cod_origen
			FROM cargo_pyxis
			JOIN articulo ON codigo=art_codigo 
			LEFT JOIN bodega_forma ON art_forma=forma_id
			WHERE pac_id=$pac_id AND ctacte=$hosp_id
		...
		(
			...
			art_val_ult AS punit, 
			(hosphc_cantidad*art_val_ult) AS subtotal, hosphc_id, '' AS cod_origen
	...
	for($i=0;$i<sizeof($hc);$i++) {
		...
		<td style='text-align:center;'>".substr($hc[$i]['log_fecha'],0,16)."<br/><b>".$hc[$i]['cod_origen']."</b></td>
		<td style='text-align:right;font-weight:bold;'>".$hc[$i]['art_codigo']."</td>
		<td style='text-align:left;'>".$hc[$i]['art_glosa']."</td>
		<td style='text-align:right;font-weight:bold;'>".$hc[$i]['cantidad']."</td>
		<td style='text-align:left;'>".$hc[$i]['forma_nombre']."</td>
		<td style='text-align:right;'>$".number_format($hc[$i]['punit'],0,',','.').".-</td>
		<td style='text-align:right;font-weight:bold;'>".($hc[$i]['subtotal']<0?'-':'')."$".number_format(abs($hc[$i]['subtotal']),0,',','.').".-</td>
		<td><center>
		");

		$total+=$hc[$i]['subtotal']*1;
	...
	print("
	<tr class='tabla_header'><td colspan=6 style='text-align:right;'>Total:</td><td style='text-align:right;font-weight:bold;font-size:18px;''>$".number_format($total,0,',','.').".-</td><td>&nbsp;</td></tr>	
	</table>
	");

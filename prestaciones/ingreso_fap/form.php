...
<script>
    ...
    agregar_prestacion = function() {
			...
			if($('tipo').value!=5) {
				...
				var myAjax=new Ajax.Request(
					'prestaciones/ingreso_fap/definir_tipo_prestacion.php',
					{
						method:'post',
    ...
    listar_fap = function() {
      ...
      'listado_fap',
      'prestaciones/ingreso_fap/listar_fap.php',
      {
        method:'post',
   ...
   boton_guardar=function(index)
	...
	ver_siguiente=function() {
		...
		if(guardar_fap()) {		
			...
			if(cursor>(datos_fap.length-1)) cursor=0;
			else if(cursor<0) cursor=datos_fap.length()-1;
	...
	eliminar_fap=function() {
		...
		var myAjax=new Ajax.Request(
			'prestaciones/ingreso_fap/sql_eliminar.php',
			{
				method:'post',
				...
				onComplete: function() {
					...
					if(datos_fap.length>0) {					
						if(cursor==datos_fap.length) cursor--;					
						abrir_fap(datos_fap[cursor].fap_id, cursor);
					} else
						listar_fap();		
	...
	quitar_fap = function(fapid){
		
		if(!confirm("&iquest;Desea eliminar el FAP seleccionado?.".unescapeHTML())) return;
		
		var myAjax = new Ajax.Request(
					'prestaciones/ingreso_fap/quitar_fap.php',
		{
		  method:'post',
		  parameters: 'fap_id='+fapid,
		  onComplete: function(resp) {
			   try {
					...
					if(resultado) {
						alert( ('FAP ELIMINADO'.unescapeHTML()) );
						listar_fap();
					}
				}catch(err){
	...
	guardar_fap=function() {
		...
		if($('tipo').value*1==5) {
			...
			var now = new Date(); 
			var fec = now.getDate()+'/'+now.getMonth()+'/'+now.getFullYear();
						
			if($('check_dia').checked){	
		}else{
			for(var i=1;i<9;i++) {
				if(trim($('fap_pab_hora'+i).value)!='') {
					for(var ii=i;ii<9;ii++) {
							
							if($('fap_pab_hora'+ii).value!='' && i!=ii)
							if(!compara_fecha_hora(trim(fec+' '+$('fap_pab_hora'+i).value),trim(fec+' '+$('fap_pab_hora'+ii).value))){
								
								alert("Verifique que el flujo del horario este correcto");
								return;
								break;
							}
					}
				}	
			}}//check_dia.checked=true;

			
			var texto_hora = '';
			if(trim($('fap_pab_hora3').value)=='')
				texto_hora+='\n- Hora de Inicio Anestesia'.unescapeHTML();
			if(trim($('fap_pab_hora4').value)=='')
				texto_hora+='\n- Hora de Inicio Intervenci&oacute;n'.unescapeHTML();
			if(trim($('fap_pab_hora5').value)=='')
				texto_hora+='\n- Hora de T&eacute;rmino Intervenci&oacute;n:'.unescapeHTML();
			if(trim($('fap_pab_hora7').value)=='')
				texto_hora+='\n- Hora de Salida Quir&oacute;fano:'.unescapeHTML()
			
			if($('fap_suspension').value=='')
				if(texto_hora!='')
					alert("Recuerde llenar el Horario Flujo del paciente, Faltan: \n"+texto_hora+"".unescapeHTML());
		...
		var myAjax=new Ajax.Request(
			'prestaciones/ingreso_fap/sql.php',
			{
				method:'post',
		...
		var myAjax=new Ajax.Request(
			...
			{
				method:'post',
                                parameters:$('info_fap').serialize()+params+'&terminar=1',
				onComplete:function(r) {
					if(trim(r.responseText)!='') {
						alert("ERROR: "+r.responseText);
					}
	...
	abrir_fap=function(fap_id, i) {
		...
		$('fap_fechas').innerHTML="<input type='text' name='fap_fecha_p' id='fap_fecha_p' size='15' value='"+r.fap_fecha+"' readOnly onDblClick=$('fap_fecha_p').readOnly=false;>";
		$('nro_ficha').innerHTML=r.pac_ficha;
                if(r.fap_terminado=='t')
                    $('ver_terminado').innerHTML='<font color="red">TERMINADO</font>';
                else
                    $('ver_terminado').innerHTML='&nbsp;';
		
		cursor=i;

      var myAjax = new Ajax.Updater(
      'listado_fap',
      'prestaciones/ingreso_fap/abrir_fap.php',
      {
      	
        method:'post',
        parameters:'fap_id='+fap_id+'&'+$('tipo').serialize()+'&ub='+i,
        evalScripts:true,
        onComplete: function() {
        	...
        	try {
        		...
        		$('listado_fap').style.height='500px';
...
crear_buscadores=function() {
    ...
    autocompletar_centro2 = new AutoComplete(
      ...
      function() {
        ...
        return {
          method: 'get',
          parameters: 'tipo=servicios_hospitalizacion&cadena='+encodeURIComponent($('centro_nombre2').value)
        }
...
actualizar_fecha=function() {
	...
	guardar_lista_equipos=function() {
		...
		for(var n=1;n<=num;n++) {
			...
			datos_equipo[n][11]=new Object();
			datos_equipo[n][11].id=$(n+'_cir4_id').value;
			datos_equipo[n][11].rut=$(n+'_cir4').value;
			var obj2=$(n+'_cir4').next();
			datos_equipo[n][11].nombre=obj2.innerHTML;				
			datos_equipo[n][11].turno=$(n+'_cir4_t').value*1;					
			...
			datos_equipo[n][15]=new Object();
			datos_equipo[n][15].id=$(n+'_enf_id').value;
			datos_equipo[n][15].rut=$(n+'_enf').value;
			var obj2=$(n+'_enf').next();
			datos_equipo[n][15].nombre=obj2.innerHTML;
			...
			datos_equipo[n][12]=new Object();
			datos_equipo[n][12].id=$(n+'_ins2_id').value;
			datos_equipo[n][12].rut=$(n+'_ins2').value;
			var obj2=$(n+'_ins2').next();
			datos_equipo[n][12].nombre=obj2.innerHTML;				
			...
			datos_equipo[n][13]=new Object();
			datos_equipo[n][13].id=$(n+'_pab2_id').value;
			datos_equipo[n][13].rut=$(n+'_pab2').value;
			var obj2=$(n+'_pab2').next();
			datos_equipo[n][13].nombre=obj2.innerHTML;			
			...
			datos_equipo[n][14]=new Object();
			datos_equipo[n][14].id=$(n+'_tec_ane2_id').value;
			datos_equipo[n][14].rut=$(n+'_tec_ane2').value;
			var obj2=$(n+'_tec_ane2').next();
			datos_equipo[n][14].nombre=obj2.innerHTML;				
	...
	generar_lista_equipos = function(guarda) {
		...
		if(typeof(guarda)!='boolean') guardar_lista_equipos();
		
		for(var n=1;n<=num;n++) {
			
		html+='<tr><td class="sub-content" colspan=2><center><u>Equipo <b>'+n+'</b></u></center></td><td class="sub-content" style="display:none;"><center>Tipo</center></td></tr>';
		...
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"cir\");' id='"+n+"_cir' name='"+n+"_cir' size=15 value='"+datos_equipo[n][0].rut+"' onFocus='$(\"listado_fap\").scrollTop=618+(320*"+(n-1)+");' /><span>"+datos_equipo[n][0].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		...
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"cir2\");' id='"+n+"_cir2' name='"+n+"_cir2' size=15 value='"+datos_equipo[n][1].rut+"' /><span>"+datos_equipo[n][1].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		...
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"cir3\");' id='"+n+"_cir3' name='"+n+"_cir3' size=15 value='"+datos_equipo[n][2].rut+"' /><span>"+datos_equipo[n][2].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		...
		html+="Ayudante (3):";
		html+="</td><td class='tabla_fila'>";
		html+="<input type='hidden' id='"+n+"_cir4_id' name='"+n+"_cir4_id' size=10 value='"+datos_equipo[n][11].id+"' />";
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"cir4\");' id='"+n+"_cir4' name='"+n+"_cir4' size=15 value='"+datos_equipo[n][11].rut+"' /><span>"+datos_equipo[n][11].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		html+="<select style='font-size:8px;' id='"+n+"_cir4_t' name='"+n+"_cir4_t'>"+combo_equipo(datos_equipo[n][11].turno)+"</select>";		
		html+="</center></td></tr>";
		...
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"ane1\");' id='"+n+"_ane1' name='"+n+"_ane1' size=15  value='"+datos_equipo[n][3].rut+"' /><span>"+datos_equipo[n][3].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		...
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"ane2\");' id='"+n+"_ane2' name='"+n+"_ane2' size=15  value='"+datos_equipo[n][4].rut+"' /><span>"+datos_equipo[n][4].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		html+="<select style='font-size:8px;' id='"+n+"_ane2_t' name='"+n+"_ane2_t'>"+combo_equipo(datos_equipo[n][4].turno)+"</select>";		
		html+="</center></td></tr>";

		html+="<tr><td class='tabla_fila2' style='text-align:right;'>";
		html+="Enfermera / Matrona:";
		html+="</td><td class='tabla_fila'>";
		html+="<input type='hidden' id='"+n+"_enf_id' name='"+n+"_enf_id' size=10 value='"+datos_equipo[n][15].id+"' />";
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"enf\");' id='"+n+"_enf' name='"+n+"_enf' size=15  value='"+datos_equipo[n][15].rut+"' /><span>"+datos_equipo[n][15].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		html+="&nbsp;";		
		html+="</center></td></tr>";		
		...
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"ins\");' id='"+n+"_ins' name='"+n+"_ins' size=15 value='"+datos_equipo[n][5].rut+"' /><span>"+datos_equipo[n][5].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		html+="&nbsp;";		
		html+="</center></td></tr>";
		...
		html+="Arsenalera (2):";
		html+="</td><td class='tabla_fila'>";
		html+="<input type='hidden' id='"+n+"_ins2_id' name='"+n+"_ins2_id' size=10 value='"+datos_equipo[n][12].id+"' />";
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"ins2\");' id='"+n+"_ins2' name='"+n+"_ins2' size=15 value='"+datos_equipo[n][12].rut+"' /><span>"+datos_equipo[n][12].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		html+="&nbsp;";		
		html+="</center></td></tr>";
		...
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"pab\");' id='"+n+"_pab' name='"+n+"_pab' size=15 value='"+datos_equipo[n][6].rut+"' /><span>"+datos_equipo[n][6].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		html+="&nbsp;";		
		html+="</center></td></tr>";
		...
		html+="Pabellonera (2):";
		html+="</td><td class='tabla_fila'>";
		html+="<input type='hidden' id='"+n+"_pab2_id' name='"+n+"_pab2_id' size=10 value='"+datos_equipo[n][13].id+"' />";
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"pab2\");' id='"+n+"_pab2' name='"+n+"_pab2' size=15 value='"+datos_equipo[n][13].rut+"' /><span>"+datos_equipo[n][13].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		html+="&nbsp;";		
		html+="</center></td></tr>";

		html+="<tr><td class='tabla_fila2' style='text-align:right;'>";
		html+="Aux. Anestesia (1):";
		html+="</td><td class='tabla_fila'>";
		html+="<input type='hidden' id='"+n+"_tec_ane_id' name='"+n+"_tec_ane_id' size=10 value='"+datos_equipo[n][7].id+"' />";
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"tec_ane\");' id='"+n+"_tec_ane' name='"+n+"_tec_ane' size=15  value='"+datos_equipo[n][7].rut+"' /><span>"+datos_equipo[n][7].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		html+="&nbsp;";		
		html+="</center></td></tr>";
		
		html+="<tr><td class='tabla_fila2' style='text-align:right;'>";
		html+="Aux. Anestesia (2):";
		html+="</td><td class='tabla_fila'>";
		html+="<input type='hidden' id='"+n+"_tec_ane2_id' name='"+n+"_tec_ane2_id' size=10 value='"+datos_equipo[n][14].id+"' />";
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"tec_ane2\");' id='"+n+"_tec_ane2' name='"+n+"_tec_ane2' size=15  value='"+datos_equipo[n][14].rut+"' /><span>"+datos_equipo[n][14].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		html+="&nbsp;";		
		html+="</center></td></tr>";

		html+="<tr style='display:none;'><td class='tabla_fila2' style='text-align:right;'>";
		html+="T&eacute;cnico Perf.:";
		html+="</td><td class='tabla_fila'>";
		html+="<input type='hidden' id='"+n+"_tec_perf_id' name='"+n+"_tec_perf_id' size=10 value='"+datos_equipo[n][8].id+"' />";
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"perf\");' id='"+n+"_tec_perf' name='"+n+"_tec_perf' size=15  value='"+datos_equipo[n][8].rut+"' /><span>"+datos_equipo[n][8].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		html+="&nbsp;";		
		html+="</center></td></tr>";
		...
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"tec_rx\");' id='"+n+"_tec_rx' name='"+n+"_tec_rx' size=15  value='"+datos_equipo[n][9].rut+"' /><span>"+datos_equipo[n][9].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		html+="&nbsp;";		
		html+="</center></td></tr>";
		...
		html+="<input type='text' onDblClick='quitar_pp("+n+", \"tec_recu\");' id='"+n+"_tec_recu' name='"+n+"_tec_recu' size=15  value='"+datos_equipo[n][10].rut+"' /><span>"+datos_equipo[n][10].nombre+"</span>";
		html+="</td><td style='width:20px;display:none;' class='tabla_fila2'><center>";
		html+="&nbsp;";		
		html+="</center></td></tr>";
		...
		autocompletar_medicos_cir4=[];

		autocompletar_medicos_ane1=[];
		autocompletar_medicos_ane2=[];

		autocompletar_medicos_enf=[];

		autocompletar_medicos_ins=[];
		autocompletar_medicos_ins2=[];
		autocompletar_medicos_pab=[];
		autocompletar_medicos_pab2=[];
		autocompletar_medicos_tec_ane=[];
		autocompletar_medicos_tec_ane2=[];
      ...
      autocompletar_medicos_cir4[n] = new AutoComplete(
      n+'_cir4', 
      'autocompletar_sql.php',
      function() {
      ...
      autocompletar_medicos_enf[n] = new AutoComplete(
      n+'_enf', 
      'autocompletar_sql.php',
      function() {
        ...
        return {
          method: 'get',
          parameters: 'tipo=personal_pabellon&tpers=6&cadena='+encodeURIComponent($(this.object).value)
        }
      ...
      function() {
        ...
        return {
          method: 'get',
          parameters: 'tipo=personal_pabellon&tpers=3&cadena='+encodeURIComponent($(this.object).value)
        }
     ...
     autocompletar_medicos_ins2[n] = new AutoComplete(
      n+'_ins2', 
      'autocompletar_sql.php',
      function() {
      ...
      function() {
        ...
        return {
          method: 'get',
          parameters: 'tipo=personal_pabellon&tpers=4&cadena='+encodeURIComponent($(this.object).value)
        }
      ...
      function() {
        ...
        return {
          method: 'get',
          parameters: 'tipo=personal_pabellon&tpers=5&cadena='+encodeURIComponent($(this.object).value)
        }
	...
	generar_fap = function() {
      ...
      new_win = 
      window.open('prestaciones/ingreso_fap/generar_fap.php?'+($('tipo').serialize()),
      'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
...
protocolo_fap = function(fap_id,ub) {

      top=Math.round(screen.height/2)-250;
      left=Math.round(screen.width/2)-475;

      new_win =
      window.open('prestaciones/ingreso_fap/protocolo_fap.php?fap_id='+fap_id+'&ub='+ub,
      'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
      'menubar=no, scrollbars=yes, resizable=no, width=950, height=500, '+
      'top='+top+', left='+left);
    ...
    escalas_fap = function(fap_id) {

      top=Math.round(screen.height/2)-250;
      left=Math.round(screen.width/2)-475;

      new_win =
      window.open('prestaciones/ingreso_fap/escalas_fap.php?fap_id='+fap_id,
      'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
      'menubar=no, scrollbars=yes, resizable=no, width=950, height=500, '+
      'top='+top+', left='+left);
    ...
    checklist_fap = function(fap_id,ub) {

      top=Math.round(screen.height/2)-250;
      left=Math.round(screen.width/2)-475;

      new_win =
      window.open('prestaciones/ingreso_fap/checklist_fap.php?fap_id='+fap_id+'&ub='+ub,
      'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
      'menubar=no, scrollbars=yes, resizable=no, width=950, height=500, '+
      'top='+top+', left='+left);
	...
	cargos_fap = function(fap_id) {

      top=Math.round(screen.height/2)-250;
      left=Math.round(screen.width/2)-475;

      new_win =
      window.open('prestaciones/ingreso_fap/hoja_cargo_fap.php?fap_id='+fap_id,
      'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
      'menubar=no, scrollbars=yes, resizable=no, width=950, height=500, '+
      'top='+top+', left='+left);
	...
	adjuntos_fap = function(fap_id) {

      top=Math.round(screen.height/2)-250;
      left=Math.round(screen.width/2)-475;

      new_win =
      window.open('prestaciones/ingreso_fap/adjuntos_fap.php?fap_id='+fap_id,
      'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
      'menubar=no, scrollbars=yes, resizable=no, width=950, height=500, '+
      'top='+top+', left='+left);
	...
	reabrir_fap = function(fap_id) {
      ...
      new_win = 
      window.open('prestaciones/ingreso_fap/generar_fap.php?'+($('tipo').serialize())+'&fap_id='+fap_id,
      'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
      'menubar=no, scrollbars=yes, resizable=no, width=750, height=400, '+
      'top='+top+', left='+left);
	...
	cargo_fap = function(fap_id) {
      ...
      new_win = 
      window.open('prestaciones/ingreso_fap/hoja_cargo.php?fap_id='+fap_id,
      'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
      'menubar=no, scrollbars=yes, resizable=no, width=680, height=380, '+
      'top='+top+', left='+left);
	...
	imprimir_hoja = function(fap_id) {

      top=Math.round(screen.height/2)-165;
      left=Math.round(screen.width/2)-340;

      new_win = 
      window.open('prestaciones/ingreso_fap/imprimir_fap.php?fap_id='+fap_id,
      'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
      'menubar=no, scrollbars=yes, resizable=no, width=680, height=330, '+
      'top='+top+', left='+left);
	...
	imprimir_pausa = function(fap_id) {

      top=Math.round(screen.height/2)-165;
      left=Math.round(screen.width/2)-340;

      new_win = 
      window.open('prestaciones/ingreso_fap/imprimir_pausa.php?fap_id='+fap_id,
      'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
      'menubar=no, scrollbars=yes, resizable=no, width=680, height=330, '+
      'top='+top+', left='+left);
	...
	imprimir_lista_fap=function() {
      ...
      new_win = 
      window.open('prestaciones/ingreso_fap/listado_faps.php?'+$('tipo').serialize()+
		...
		'prestaciones/ingreso_fap/comprueba_pausas.php',
		{
		  ...
		  onComplete: function(resp) {
			   try {
					...
					if(resultado!=''){
						alert('Debe completar la pausa de seguridad'+resultado.unescapeHTML());
						checklist_fap(fap_id,ub);
						return;
					}else{
	...
	compara_fecha_hora = function(fec1,fec2){
		var fecha1 = new Date(fec1);
		var fecha2 = new Date(fec2);
		var estado=true;
		
		//alert(fec1+'|||'+fec2);
		//alert(fecha1.getTime()+'||'+fecha2.getTime());
...
agrega_inf_pabellon=function(fap_id) {

      top=Math.round(screen.height/2)-165;
      left=Math.round(screen.width/2)-340;

      new_win = 
      window.open('prestaciones/ingreso_fap/informacion_pabellon.php?fap_id='+fap_id,
      'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
      'menubar=no, scrollbars=yes, resizable=no, width=680, height=330, '+
      'top='+top+', left='+left);
...
<img src='iconos/table_edit.png'> <b>Registro de Prestaciones Quir&uacute;rgicas</b>
</div>
...
<tr id='opts_pabellon'>
<td style='width:100px;text-align:right;'>Sector:</td>
<td>
<select id='selpab' name='selpab'>
<option value=''>(Todos...)</option>
<?php 
	print(desplegar_opciones("fappab_pabellones", 
	"fapp_id, fapp_desc",-1,'true','ORDER BY fapp_desc')); 
...
<?php if(_cax(209)) { ?>
<input type='button' id='genera_fap' style='display:none;' 
onClick='generar_fap();'
value='Generar Registro Nuevo...'>
<?php } ?>
...
<table style='width:100%;'>
<tr>
        <td id='ver_ubica' style='width:130px;text-align:center;'></td>
        ...
        <td id='nro_ficha' style='text-align:center;font-weight:bold;width:60px;'>
        </td>
        <td style='text-align:right;'>Nombre:</td>
        <td id='ver_paciente' style='width:350px;font-weight:bold;'>
        </td>
        <td id="ver_terminado"></td>
</tr>
</table>
</div>

<?php
	...
	$pdf->Image('../../imagenes/logohpm_small.jpg', 160,5,40,18);

	$pdf->Cell(190,4,'Ministerio de Salud',0,1,'L');
	$pdf->Cell(190,4,'Servicio de Salud de Reloncaví',0,1,'L');
	$pdf->Cell(190,4,'Hospital Dr. Eduardo Schutz Shroeder',0,1,'L');
	...
	$pdf->Cell(45,6,'Horarios Flujo Paciente',1,0,'C',1);
        ...
        $pdf->Cell(143,6,'EQUIPO QUIRÚRGICO    (N:Normal T:Turno C:Convenio P:Privado D:Docencia)',1,1,'C',1);
	...
	*/
	...
	$pdf->Cell(23,5,'Anestesista:',1,0,'R',1);
	for($i=0;$i<2;$i++) {
	...
	$pdf->Cell(190,4,'Ministerio de Salud',0,1,'L');
	$pdf->Cell(190,4,'Servicio de Salud de Reloncaví',0,1,'L');
	$pdf->Cell(190,4,'Hospital Dr. Eduardo Schutz Schroeder',0,1,'L');
	...
	$pdf->Cell(47,6,'Nombre:',1,0,'R',1);
	$pdf->Cell(141,6,($fap['pac_id']!=0?trim($fap['pac_appat'].' '.$fap['pac_apmat'].' '.$fap['pac_nombres']):''),1,1,'L');

	$pdf->Cell(47,6,'Edad:',1,0,'R',1);
	$pdf->Cell(73,6,($fap['pac_id']!=0?$edad:''),1,0,'L');
	$pdf->Cell(32,6,'R.U.T.:',1,0,'R',1);
	$pdf->Cell(36,6,($fap['pac_id']!=0?$fap['pac_rut']:''),1,1,'L');
	...
	$pdf->Cell(47,6,'Modo de Atención:',1,0,'R',1);
	$pdf->SetFont('Arial','', 12);
	$pdf->Cell(73,6,$fap['fap_subtipopab'],1,0,'L');
	$pdf->SetFont('Arial','', 14);


	$pdf->Cell(32,6,'Ficha Clínica:',1,0,'R',1);
	$pdf->Cell(36,6,($fap['pac_id']!=0?$fap['pac_ficha']:''),1,1,'L');
	...
	$pdf->Cell(47,6,'Servicio Orígen:',1,0,'R',1);
	$pdf->Cell(73,6,cut($fap['tcama_tipo'],25),1,0,'L');
	$pdf->Cell(32,6,'Previsión:',1,0,'R',1);
	$pdf->Cell(36,6,($fap['pac_id']!=0?$fap['prev_desc']:''),1,1,'L');
	/* EGF */
    ...
    $pdf->Cell(32,6,'N° Pabellón:',1,0,'R',1);
	$pdf->Cell(36,6,$fap['fapp_desc'],1,1,'L');
	
	if($fap['fap_suspension']!=''){
		$pdf->Cell(47,6,'Suspensión:',1,0,'R',1);
		$pdf->Cell(141,6,$fap['fap_suspension'],1,1,'L');
	}
	...
	$pdf->Cell(47,6,'Diag. Pre. Operatorio:',1,0,'R',1);
	$pdf->SetFont('Arial','', 14);
	$pdf->Cell(141,6,cut($fap['fap_diag_cod'],50),1,1,'L');
	$pdf->SetFont('Arial','', 12);
	$pdf->Cell(47,6,'Diag. Post. Oper. (1):',1,0,'R',1);
	$pdf->SetFont('Arial','', 14);
	if($fap['diag_desc_1']!='')
		$pdf->Cell(141,6,'['.$fap['fap_diag_cod_1'].'] '.cut($fap['diag_desc_1'],45),1,1,'L');
	else 
		$pdf->Cell(141,6,'',1,1,'L');
	...
	for($i=0;$i<sizeof($presta);$i++) {
		if($presta[$i]['fappr_codigo']!=''){
			$pdf->Cell(35,6,$presta[$i]['fappr_codigo'],1,0,'C');
			$pdf->SetFont('Arial','', 12);
			$pdf->Cell(143,6,cut($presta[$i]['glosa'],70),1,0,'L');
			$pdf->Cell(10,6,$presta[$i]['fappr_cantidad'],1,1,'C');
		}
	}
	...
	$pdf->Cell(48,6,'Horarios Flujo Paciente',1,0,'C',1);
        $pdf->SetFont('Arial','B', 10);
        $pdf->Cell(140,6,'EQUIPO QUIRÚRGICO    (N:Normal T:Turno C:Convenio P:Privado D:Docencia)',1,1,'C',1);
	...
	*/
	...
	$pdf->Cell(188,6,'Hallazgos Intraoperatorios',1,1,'C',1);
	...
	$pdf->SetFont('Arial','BU', 12);
	$pdf->Cell(188,6,'Descripción Operación',1,1,'C',1);
	...
	if($fap['fap_indicaciones']!='' OR $fap['fap_indicaciones_anestesia']!='') {
		...
		$pdf->Cell(47,5,'Especialidad:',1,0,'R',1);
		$pdf->Cell(78,5,cut($fap['pp_desc'],25),1,0,'L');
		$pdf->Cell(27,5,'N° Pabellón:',1,0,'R',1);
		$pdf->Cell(36,5,$fap['fapp_desc'],1,1,'L');
	...
	if($fap['fap_suspension']!=''){
		$pdf->Cell(190,5,'Suspendido: '.$fap['fap_suspension'],0,1,'C');
	}

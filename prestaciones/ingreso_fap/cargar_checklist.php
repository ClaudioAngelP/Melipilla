...
<?php
        ...
        $cl=cargar_registros_obj("SELECT *, (SELECT count(*) FROM fap_checklist_detalle AS fcld WHERE fap_id=$fap_id AND fcld.fcl_id=fap_checklist.fcl_id) AS cnt,
									(SELECT fcld_completa FROM fap_checklist_detalle AS fcld WHERE fap_id=$fap_id AND fcld.fcl_id=fap_checklist.fcl_id ORDER BY fcld_fecha DESC LIMIT 1) as estado_completa
									FROM fap_checklist ORDER BY fcl_nombre;", true);
		
        for($i=0;$i<sizeof($cl);$i++) {
		if($cl[$i]['fcl_id']==$id) $sel='SELECTED'; else $sel='';
                if($cl[$i]['estado_completa']=='t') $txt='<i>(Completa)</i>'; else $txt='<i>(Incompleto)</i>';
...
<?php
	...
	for($i=0;$i<sizeof($dr);$i++) {
		$tmp=explode('|',$dr[$i]);
		$digest[$i]=$tmp[1];//$digest[$tmp[0]]=$tmp[1];

	}

	if($d) {
		print("
		<center><table style='width:100%;border:1px solid black;background-color:#dddddd;'><td style='width:20%;text-align:right;'><input type=hidden name='fcld_id' id='fcld_id' value='".$d['fcld_id']."' ><img src='../../iconos/printer.png' onClick='comprueba_pausa(".$fap_id.",".$fcl_id.");' style='cursor:pointer;width:32px;height:32px;display:inline;' /></td>

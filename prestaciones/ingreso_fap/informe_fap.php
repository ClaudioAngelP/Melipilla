<?php
  ...
  $max_presta_cant = cargar_registro("SELECT (max(max)+max(count))-1 as max from (
			SELECT max(fappr_cantidad), count(*) FROM fap_prestacion
			LEFT JOIN fap_pabellon USING(fap_id)
			WHERE fap_fecha::date BETWEEN '$fecha1 00:00:00' AND '$fecha2 23:59:59'
			GROUP BY fap_id
			)as foo");
...
<?php 
for($j=0;$j<(($max_presta_cant['max']));$j++)
...
<?php 
  ...
  for($i=0;$i<count($lista);$i++) {
    ...
    print("<td style='text-align:center;'>".$lista[$i]['pac_rut']."</td>");
	 ...
	 elseif($lista[$i]['fap_tipopab']==5) $lista[$i]['fap_tipopab']='PRIVADA';
	 elseif($lista[$i]['fap_tipopab']==6) $lista[$i]['fap_tipopab']='PAB. ELECTIVO/PAB. EXTENDIDO';
...
/* EGF */ //	 print("<td style='text-align:left;'>".$lista[$i]['fap_tablapab']."</td>");		
	...
	$cantc=0;
	 for($j=0;$j<(($max_presta_cant['max']));$j++){
		 for($p=0;$p<$presta[$j]['fappr_cantidad'];$p++){
			 print("<td style='text-align:left;'>".$presta[$j]['fappr_codigo']."</td>
			 <td style='text-align:center;'>".$presta[$j]['fappr_tipo']."</td>");
			 
			 if($cantc<($presta[$j]['fappr_cantidad']+count($presta)))
				$cantc=$presta[$j]['fappr_cantidad']+count($presta);
		}				
	}
	
	if($cantc==0)
		$cantc=1;
	$cantr=$max_presta_cant['max']-$cantc;
	//print($cantr.'asdas');
	if($cantr>=0){
		for($tt=0;$tt<=$cantr;$tt++){
...
/* EGF */	 //print("<td style='text-align:left;'>".($lista[$i]['fap_sospecha_ges']=='t'?'SI':'NO')."</td>");
	 if ($lista[$i]['fap_reoperado']=='t'){
		 
		 print("<td style='text-align:left;'>".'PROGRAMADA'."</td>");
	 }
	 if ($lista[$i]['fap_reoperado']=='f'){
		 
		 print("<td style='text-align:left;'>".'NO PROGRAMADA'."</td>");
	 }
	 if ($lista[$i]['fap_reoperado']==''){
		 
		 print("<td style='text-align:left;'>".'NO'."</td>");
	 }

<?php 
	...
	COALESCE(
                    (
                        SELECT COALESCE(esp_desc,'ARCHIVO') 
...
<?php 
    if($l)
		...
		<td style='text-align:left;font-weight:bold;'>".$l[$i]['ubic_actual']."</td>");
                if(($l[$i]['am_estado']*1)==3)
                {
                    if($l[$i]['ubic_actual']=="ARCHIVO")
                    {
                        print("<td style='text-align:left;'>".$opts[$l[$i]['am_estado']*1]."-Archivo</td>");
                    }
                    else
                    {
                        print("<td style='text-align:left;'>".$opts[$l[$i]['am_estado']*1]."</td>");
                    }
...
<?php
		...
		WHERE $f1w AND $f2w AND $tw AND NOT pac_id=0 AND $ew AND $pw AND (esp_ficha or esp_ficha is null)
		ORDER BY 
		(CASE 
		WHEN pacientes.pac_ficha='' THEN '0' 
		WHEN pacientes.pac_ficha IS NULL THEN '0' 
...
<?php 
	...
	for($i=0;$i<sizeof($l);$i++) {
		...
		<!--<td style='text-align:left;'>".$l[$i]['esp_desc']."</td>-->
                <td style='text-align:left;'>".$l[$i]['esp']."</td>
		<td style='text-align:left;'>".$l[$i]['doc_paterno']." ".$l[$i]['doc_materno']." ".$l[$i]['doc_nombres']."</td>

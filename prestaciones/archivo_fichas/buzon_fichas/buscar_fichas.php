<?php
        ...
        if($servicio==0)
        {
            ...
            if($tipo_busqueda==1)
            {
                ...
                */
                ...
                case when am_tipo_solicitud='1' then
                (
                    select nomd_hora from nomina_detalle where nomd_id=archivo_movimientos.nomd_id
                )
                when am_tipo_solicitud='0' then
                ( 
            ...
            if($tipo_busqueda==4)
            {
                ...
                */
                ...
                COALESCE((SELECT COALESCE(destino_esp_id,0) FROM archivo_movimientos WHERE archivo_movimientos.pac_id=pacientes.pac_id and am_final ORDER BY am_fecha DESC LIMIT 1),0) as esp_id_actual,
                COALESCE((SELECT COALESCE(destino_doc_id,0) FROM archivo_movimientos WHERE archivo_movimientos.pac_id=pacientes.pac_id and am_final ORDER BY am_fecha DESC LIMIT 1),0) as doc_id_actual,
                COALESCE((SELECT COALESCE(am_estado,0) FROM archivo_movimientos WHERE archivo_movimientos.pac_id=pacientes.pac_id and am_final ORDER BY am_fecha DESC LIMIT 1),0) as am_estado
                FROM nomina_detalle
                ...
                and nomina_detalle.nomd_id not in ((SELECT COALESCE(nomd_id,0) FROM archivo_movimientos WHERE archivo_movimientos.pac_id=pacientes.pac_id and am_final ORDER BY am_fecha DESC LIMIT 1))
                ...
                COALESCE((SELECT COALESCE(am_centro_ruta_destino,null) FROM archivo_movimientos WHERE archivo_movimientos.pac_ficha=pacientes.pac_ficha AND archivo_movimientos.pac_id=pacientes.pac_id and am_final ORDER BY am_fecha DESC LIMIT 1),null) as centro_ruta_actual,
                esp_ficha,
                ...
                and fesp_id not in ((SELECT COALESCE(nomd_id,0) FROM archivo_movimientos WHERE archivo_movimientos.pac_ficha=pacientes.pac_ficha AND archivo_movimientos.pac_id=pacientes.pac_id and am_final ORDER BY am_fecha DESC LIMIT 1))
    ...
    else
    {
        ...
        if(strstr($barras,'-'))
            $tmp=cargar_registros_obj("SELECT * FROM pacientes WHERE ltrim(upper(pac_rut),'0')=upper('$barras')");
        else
            $tmp=cargar_registros_obj("SELECT * FROM pacientes WHERE upper(pac_ficha)=upper('$barras')");
        if(!$tmp)
        {

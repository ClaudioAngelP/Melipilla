<?php
	...
	$esp_ant='';
		...
		ORDER BY am_fecha DESC LIMIT 1
            ...
            nomd_inter_id,COALESCE(nom_estado,0)as nom_estado
            FROM nomina
        ...
        if($tipo==3)
        {
            ...
            upper(pac_rut)as pac_rut,upper(pac_nombres||' '||pac_appat||' '||pac_apmat)AS pac_nombre,
            fesp_estado, 
            COALESCE (especialidades.esp_id::text,fesp_centro_ruta)as esp_id,
            COALESCE(doctores.doc_id,0)as doc_id,
            fesp_id AS nomd_id,
            pac_id,
            amp_nombre,
            '' AS nomd_diag_cod,
	    ...
	    COALESCE((SELECT COALESCE(nomd_id,0) FROM archivo_movimientos WHERE archivo_movimientos.pac_ficha=pacientes.pac_ficha AND archivo_movimientos.pac_id=pacientes.pac_id ORDER BY am_fecha DESC LIMIT 1),0) as nomd_id_sel,
            ...
            ORDER BY fecha_asigna desc,esp desc,doc_nombre,(CASE WHEN pacientes.pac_ficha='' THEN '0' WHEN pacientes.pac_ficha IS NULL THEN '0' ELSE pacientes.pac_ficha END)::bigint";
	...
	case when doc_id!=0 then upper(doc_nombres||' '||doc_paterno||' '||doc_materno) 
        ...
        upper(pac_rut)as pac_rut,upper(pac_nombres||' '||pac_appat||' '||pac_apmat)AS pac_nombre,pacientes.pac_id,'' AS nomd_diag_cod, 
        date_trunc('second',
    ...
    if($salidas)
    {
        for($i=0;$i<count($salidas);$i++)
        {
            ...
            if(($agrupar==0 AND $doc_ant!=$salidas[$i]['doc_nombre']) OR $esp_ant!=$esp_string)
            {
		...
		$motivo='';
                ...
                if($salidas[$i]['esp_id']==0)
                {
                    ...
                    print("<td style='text-align:left;font-size:16px;' colspan=$colspan>Servicio: <b>".htmlentities($salidas[$i]['esp'])."</b><br/>");
                }
                else
                {
                    print("<table style='width:100%;' class='lista_small' cellspacing=0 cellpadding=1>");
                    print("<tr class='tabla_header'>");
                    print("<td style='text-align:left;font-size:16px;' colspan=$colspan>Programa: <b>".htmlentities($salidas[$i]['esp'])."</b><br/>");
                }
                if($agrupar==0)
                {
                ...
                {
                    print("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
                    print("<input type='button' id='imprimir_".$salidas[$i]['esp_id']."' name='imprimir_".$salidas[$i]['esp_id']."' value='Imprimir' onClick='imprimir_especialidad(".$salidas[$i]['esp_id'].",".$agrupar.",".$salidas[$i]['doc_id'].",\"".$fecha."\",0);'/>");
                }
                else
                {
                    if(strstr($salidas[$i]['esp_id'],'.'))
                    {
                        if($tipo==3)
                        {
                            print("&nbsp; -- (");
                            print("<input type='button' id='imprimir_".$salidas[$i]['esp_id']."' name='imprimir_".$salidas[$i]['esp_id']."' value='Imprimir' onClick='imprimir_especialidad(\"".$salidas[$i]['esp_id']."\",".$agrupar.",".$salidas[$i]['doc_id'].",\"".$fecha."\",\"".$salidas[$i]['fesp_func_id']."\");'/>");
                            print(")");
                        }
                        else
                        {
                ...
                print("<tr class='tabla_header'>");
                    ...
                    print("<td style='width:10%;'>Solicitado</td>");
                    if($tipo==2)
                    {
                    ...
                    print("<td style='width:10%;'>RUN</td>");
                    print("<td style='width:25%;'>Nombre Completo</td>");
                    if($tipo==3)
                    {
                        print("<td style='width:10%;'>Motivo Solicitud</td>");
                    }
                    print("<td style='width:15%;'>Ubic. Anterior</td>");
                    if($tipo!=2)
                    {
                        print("<td style='width:15%;'>Ubic. Actual</td>");
                    }
                    print("<td>Estado Actual</td>");
		...
		print("</tr>");
            ...
            if($salidas[$i]['pac_ficha']!='' && $salidas[$i]['pac_ficha']!='0')
            {
                ...
                if(_cax(20002))
                    $ficha.="<input type='button' style='font-size:8px;margin:0px;padding:0px;' id='asigna_".$salidas[$i]['pac_id']."' name='asigna_".$salidas[$i]['pac_id']."' onClick='asignar_ficha(".$salidas[$i]['pac_id'].",1);' alt='Asignar Nuevo Nro de Ficha' title='Asignar Nuevo Nro de Ficha' value='[A]' />";
                
                //<input type='button' $motivo_enabled id='motivo_".$lista[$i]['nomd_id']."' name='motivo_".$lista[$i]['nomd_id']."' value='[S]' onClick='sel_motivo(".$lista[$i]['nomd_id'].");' style='border:0px;margin:0px;'  />
            ...
            else
            {
                ...
                $ficha.="<input type=hidden id='".$salidas[$i]['pac_ficha']."_ficha' name='".$salidas[$i]['pac_ficha']."_ficha' value='".$salidas[$i]['pac_ficha']."' />";
		if(_cax(20002))
                    $ficha.="<input type='button' style='font-size:8px;margin:0px;padding:0px;' id='asigna_".$salidas[$i]['pac_id']."' name='asigna_".$salidas[$i]['pac_id']."' onClick='asignar_ficha(".$salidas[$i]['pac_id'].",0);' value='[ASIGNAR]' />";
		...
		$ficha.="<input type='button' style='font-size:8px;margin:0px;padding:0px;' id='crea_".$salidas[$i]['pac_id']."' name='crea_".$salidas[$i]['pac_id']."' onClick='crear_ficha(".$salidas[$i]['pac_id'].");' value='[CREAR]' /></center>";
            ...
            if(($salidas[$i]['am_estado']*1)==3)
            {
                ...
                {
                    $color_estado=$opts_color[$salidas[$i]['am_estado']*1];
                }
            }
            else
            {
            ...
            print("<tr style='color:".$color_estado.";$color;$tachar' >
            <td style='text-align:right;font-weight:bold;'>".$cont."</td>
            <td style='text-align:center;'>".substr($salidas[$i]['fecha_asigna'],0,16)."</td>
            ");
		
            if($tipo==2)
                print("<td style='text-align:center;'>".substr($salidas[$i]['fecha_envio'],0,16)."</td>");
            
            print("<td style='text-align:center;font-size:14px;font-weight:bold;white-space:nowrap;'>".$ficha."</td>");

            if($tipo==1 OR $tipo==3)
                print("<td style='text-align:center;'>".(($salidas[$i]['peticiones']*1)+($salidas[$i]['peticiones2']*1))."</td>");
            
            print("<td style='text-align:right;'>".$salidas[$i]['pac_rut']."</td>");
            print("<td style='text-align:left;'>".htmlentities($salidas[$i]['pac_nombre'])."</td>");
            ...
            if($tipo==3)
            {
                ...
                print("<td style='text-align:center;'>".$amp_nombre."</td>");
            }
            print("<td style='text-align:center;'>".htmlentities($salidas[$i]['ubic_anterior'])."</td>");
            if($tipo!=2)
            {
                print("<td style='text-align:center;font-weight:bold;'>".htmlentities($salidas[$i]['ubic_actual'])."</td>");
            }
            ...
            if(($salidas[$i]['am_estado']*1)==3)
            {
                if($salidas[$i]['ubic_actual']=="ARCHIVO")
                {
                    print("".$opts[$salidas[$i]['am_estado']*1]."-Archivo");
                }
                else
                {
                    print("".$opts[$salidas[$i]['am_estado']*1]."");
                }
            ...
            print("<td>");
                print("<center>");
                    print("<img src='iconos/printer.png'  style='cursor:pointer;' alt='Imprimir Etiqueta' title='Imprimir Etiqueta' onClick='imprimir_etiqueta(".$salidas[$i]['pac_id'].");' />");
                print("</center>");
            print("</td>");
            print("<td>");
                print("<center>");
                    print("<img src='iconos/magnifier.png'  style='cursor:pointer;' alt='Ver Historial' title='Ver Historial' onClick='historial_ficha(".$salidas[$i]['pac_id'].");' />");
                print("</center>");
            print("</td>");
            print("<td>");
                print("<center>");
                    print("<img src='iconos/find.png'  style='cursor:pointer;' alt='Ver solicitudes' title='Ver solicitudes' onClick='ver_solicitudes(".$salidas[$i]['pac_id'].");' />");
                print("</center>");
            print("</td>");
            print("<td>");
                print("<center>");
                if($tipo==1)
                {
                    if(($salidas[$i]['nomd_inter_id']*1)!=0)
                    {
                        print("<table>");
                            print("<tr>");
                                print("<td>");
                                    print("<img src='iconos/layout.png'  style='cursor:pointer;' alt='Ver Interconsulta' title='Ver Interconsulta' onClick='abrir_ficha(".$salidas[$i]['nomd_inter_id'].");' />");
                                print("</td>");
                                print("<td>");
                                    print("<img src='iconos/printer.png'  style='cursor:pointer;' alt='Imprimir Interconsulta' title='Imprimir Interconsulta' onClick='print_inter_ficha(".$salidas[$i]['nomd_inter_id'].");' />");
                                print("</td>");
                            print("</tr>");
                        print("</table>");
                    }
            ...
            print("<td>");
                print("<center>");
                    print("<img src='iconos/report_user.png'  style='cursor:pointer;' alt='Consultar Paciente' title='Consultar Paciente' onClick='consultar_paciente(\"".$salidas[$i]['pac_ficha']."\");' />");

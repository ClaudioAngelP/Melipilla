<?php 
    ...
    if(isset($_POST['ficha']))
    {
        ...
        if($pac_ficha!=0 and $pac_ficha!="0")
        {
            ...
            $tmp=cargar_registro("SELECT * FROM pacientes WHERE pac_id=$pac_id LIMIT 1");
            pg_query("update archivo_movimientos set pac_ficha='".$tmp['pac_ficha']."' WHERE pac_id=$pac_id;");
            pg_query("update ficha_espontanea set pac_ficha='".$tmp['pac_ficha']."' WHERE pac_id=$pac_id;");
            pg_query("COMMIT");
        ...
        else
        {
            ...
            $tmp=cargar_registro("SELECT * FROM pacientes WHERE pac_id=$pac_id LIMIT 1");
            pg_query("update archivo_movimientos set pac_ficha=null WHERE pac_id=$pac_id;");
            pg_query("update ficha_espontanea set pac_ficha=null WHERE pac_id=$pac_id;");
            pg_query("COMMIT");

<?php
    ...
    if(!isset($_POST['ultimo']))
    {
        ...
        if(isset($_POST['select_busqueda']))
        {
            ...
            {
                $tmp=cargar_registros_obj("SELECT * FROM pacientes WHERE pac_id='$barras'");
            }
        }
        else
        {
        ...
        if($tmp)
        {
            if(count($tmp)>1)
            {
                if($buzon)
                {
                    exit("<table><tr><td>NO SE PUEDE MOSTRAR DATOS DEL PACIENTE INGRESADO YA QUE SE ENCUENTRA DUPLICADO</td></tr></table>");
                }
                else
                {

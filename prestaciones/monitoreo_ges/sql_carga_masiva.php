...
drop table patologias_sigges_traductor;
	...
	for($i=0;$i<$totalr;$i++) {
		...
		$espnombre=trim($reg[13]);

		$tmp3=cargar_registro("SELECT * FROM lista_dinamica_especialidades WHERE esp_nombre='$espnombre';");

		if($tmp3) {
			$codesp=$tmp3['esp_codigo'];
		} else {
			$codesp='';
		}

		$observaciones=trim($reg[14]);
		
		if($tipo_carga==0 OR $tipo_carga==2 OR $tipo_carga==3) {
		...
		if($chk) {		
			...
			pg_query("UPDATE monitoreo_ges SET mon_cod_especialidad='$codesp' WHERE mon_id=$mon_id");
			...
			if($tipo_carga<=1) {
				...
				if($valores_sda!='|')
						pg_query("INSERT INTO monitoreo_ges_registro VALUES (
							DEFAULT, $mon_id, $func_id, $fevento, '$id_pendiente_oc', '$id_bandeja_sda', '', null, 'CARGA MASIVA COMPRAS', '$cual', $fevento, 1, '', '$valores_sda'
						);");
				...
				if($valores_abast!='|')
						pg_query("INSERT INTO monitoreo_ges_registro VALUES (
							DEFAULT, $mon_id, $func_id, $fevento, '$id_pendiente_oc', '$id_bandeja_abast', '', null, 'CARGA MASIVA COMPRAS', '$cual', $fevento, 1, '', '$valores_abast'
						);");

...
<?php 
  ...
  for($i=0;$i<count($regs);$i++) {
	...
	if($regs[$i]['pac_ficha']!='')
		...
		$ficha="<center>
		<input type='button' style='font-size:8px;margin:0px;padding:0px;' id='asigna_".$regs[$i]['pac_id']."' name='asigna_".$regs[$i]['pac_id']."' onClick='asignar_ficha(".$regs[$i]['pac_id'].");' value='[ASIGNAR]' />
    ...
    print("<tr class='$clase' style='color:".$opts_color[$regs[$i]['am_estado']*1].";$color'
			...
			".$opts[$regs[$i]['am_estado']*1]."
			</td><td>
			 <center><img src='iconos/printer.png'  style='cursor:pointer;'
                alt='Imprimir Etiqueta' title='Imprimir Etiqueta' onClick='imprimir_etiqueta(".$regs[$i]['pac_id'].");' /></center>
			</td><td>
			 <center><img src='iconos/magnifier.png'  style='cursor:pointer;'
                alt='Ver Historial' title='Ver Historial' onClick='historial_ficha(".$regs[$i]['pac_id'].");' /></center>
			</td></tr>");

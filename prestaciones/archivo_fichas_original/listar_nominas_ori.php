...
if($tipo==1 OR $tipo==3){
	...
	$salidas = cargar_registros_obj("SELECT nomina.nom_id,nom_fecha::date AS fecha,nomd_hora, upper(esp_desc)AS esp,doc_rut,pacientes.pac_ficha,
			...
			(SELECT COUNT(*) FROM ficha_espontanea AS fesp WHERE fesp.pac_id=pacientes.pac_id AND fesp.fesp_fecha::date=nomina.nom_fecha::date AND fesp_estado=0) AS peticiones2
				...
				WHERE esp_ficha AND nom_fecha BETWEEN '$fecha 00:00:00' AND '$fecha 23:59:59' AND nomd_diag_cod NOT IN ('B')
				AND $esp AND $doc
		...
		if($tipo==3)
	   $salidas=cargar_registros_obj("SELECT fesp_fecha::date AS fecha_asigna,upper(esp_desc)AS esp,doc_rut,
       ...
       JOIN pacientes USING (pac_id)
       WHERE esp_ficha AND fesp_fecha BETWEEN '$fecha 00:00:00' AND '$fecha 23:59:59' AND fesp_estado=0 AND $esp AND $doc
	...
	if(($agrupar==0 AND $doc_ant!=$salidas[$i]['doc_id']) OR $esp_ant!=$salidas[$i]['esp_id']){
			...
			print("<table style='width:100%;' class='lista_small' cellspacing=0 cellpadding=1>
				<tr class='tabla_header'>
					<td style='text-align:left;font-size:16px;' colspan=11>
					Programa: <b>".htmlentities($salidas[$i]['esp'])."</b><br/>
");

				if($agrupar==0)
				print("
					Profesional/Servicio: <b>".htmlentities($salidas[$i]['doc_nombre'])."</b>");
	...
	if($salidas[$i]['pac_ficha']!='')
		...
		if(_cax(20002))
		$ficha.="
		<input type='button' style='font-size:8px;margin:0px;padding:0px;' id='asigna_".$salidas[$i]['pac_id']."' name='asigna_".$salidas[$i]['pac_id']."' onClick='asignar_ficha(".$salidas[$i]['pac_id'].");' value='[ASIGNAR]' />";

		$ficha.="
		<input type='button' style='font-size:8px;margin:0px;padding:0px;' id='crea_".$salidas[$i]['pac_id']."' name='crea_".$salidas[$i]['pac_id']."' onClick='crear_ficha(".$salidas[$i]['pac_id'].");' value='[CREAR]' />
		</center>";

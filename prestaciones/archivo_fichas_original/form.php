<?php	
	...
	for($l=0;$l<sizeof($opts);$l++) {
		if($l==3 OR $l==4) continue; 
		if($l==1) $sel='SELECTED'; else $sel='';
...
asignar_ficha = function(pac_id) {
	...
	var myAjax=new Ajax.Request(
		'prestaciones/archivo_fichas/sql_ficha.php',
		{
			method:'post',
			parameters:'pac_id='+pac_id+'&ficha='+encodeURIComponent(ficha),
...
enviar_nominas=function() {
	var myAjax=new Ajax.Request(
		'prestaciones/archivo_fichas/sql_enviar_retiradas.php',
		{
...
<table style='width:100%;'>
	<tr>
		...
		<td style='font-size:14px;'><select id='tipo_inf' name='tipo_inf' onChange='actualizar_list();' style='font-size:18px;'>
...
<div class='sub-content' id='buscar_nominas'><center>
	...
	<tr id='tr_estado' ><td style='text-align:right;'>Estado:</td>
        <td colspan=5>                         
	 ...
	 <tr id='' ><td style='text-align:right;'>Agrupar:</td>
        <td colspan=5>
	...
	<tr><td style='text-align:right;'><img src='abastecimiento/hoja_cargo/barras.png' /></td>
        <td colspan=5>                         
		<input type='text' id='barras' name='barras' size=25 
		style='font-size:16px;text-align:center;' onKeyUp='if(event.which==13) pistolear_ficha();' 
		onFocus='this.style.border="3px dashed red";this.select();' onBlur='this.style.border="";this.style.background="";' />
		<input type='text' id='texto_barras' name='texto_barras' size=60 READONLY DISABLED 
		style='font-size:16px;text-align:left;border:none;' value='&lt;&lt; Seleccione Fichas con C&oacute;digo de Barras' />

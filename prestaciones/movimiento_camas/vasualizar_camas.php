	...
	*
	...
	for($i=0;$i<sizeof($ccamas);$i++) {
		...
		for($k=0;$k<sizeof($tc);$k++) {		
			...
			$chqb=pg_query("
			SELECT * FROM bloqueo_camas WHERE bloq_numero_cama=$n AND
			(
				bloq_fecha_ini<=CURRENT_DATE AND 
				(
					bloq_fecha_fin IS NULL OR 
			...
			if(pg_num_rows($chqb)==0) {
				$bloqueada='';
			} else {
				$bloqueada=" style='background-image:url(../../icons/lock.png)' ";
			}
			...
			$html.="
				...
				<tr>
				<td class='ucama' id='icama_$n' ".$bloqueada."></td>
				</tr><tr>

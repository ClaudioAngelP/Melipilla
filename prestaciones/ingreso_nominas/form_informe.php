<?php
    ...
    if(isset($_GET['sol_examd_id']))
    {
        $informe_examen=true;
        $sol_examd_id=$_GET['sol_examd_id']*1;
    }
    else
    {
        $informe_examen=false;
        $sol_examd_id=0;
    }
    ...
    $ndet=cargar_registro($consulta, true);

    $pac_id=$ndet['pac_id']*1;
    if(!$informe_examen)
    {
        $inf=cargar_registro("SELECT * FROM nomina_detalle_informe LEFT JOIN doctores ON nomdi_doc_id=doc_id WHERE nomd_id=$nomd_id", true);
    }
    else
    {
        $inf=cargar_registro("SELECT *,date(sol_fecha)as fecha_solicitud FROM solicitud_examen_detalle
        ...
        WHERE sol_examd_id=$sol_examd_id",true);
    }
?>
<html>
	...
	*/
	...
	function listar_adjuntos()
        {
            var myAjax=new Ajax.Updater('lista_adjuntos','listar_adjuntos.php',
            {
                method:'post',
	...
	function eliminar_adjunto(nomda_id)
        {
            ...
            if(!conf)
                return;
            var myAjax=new Ajax.Request('eliminar_adjunto.php',
            {
                method:'post',
	...
	function guardar_informe()
		...
		return false;
            ...
            var myAjax=new Ajax.Request('sql_informe.php',
            {
                method:'post',
                parameters: params,
                onComplete:function()
...
<body class='fuente_por_defecto popup_background' onbeforeunload="mymessage()">
    ...
    <form id='datos' name='datos' onSubmit='return false'>
        ...
        <input type='hidden' id='sol_examd_id' name='sol_examd_id' value='<?php echo $sol_examd_id; ?>' />
    ...
    <div class='sub-content'>
        <table style='width:100%;'>
            <tr>
                <td style='width:150px;text-align:right;' class='tabla_fila2'>
                    R.U.T.:
                </td>
                <td colspan=3 class='tabla_fila' style='text-align:left;font-size:16px;'>
            ...
            <tr>
                ...
                <td colspan=5 class='tabla_fila' style='text-align:left;font-weight:bold;font-size:16px;'>
            ...
            <tr>
                ...
                <td class='tabla_fila'>
                    ...
                    {
                    ?>
                        <input type='text' name='fecha1' id='fecha1' size=10 style='text-align: center;' value='<?php echo $inf['fecha_solicitud'] ?>'>
                    <?php
                    }
                    else
                    {
                        ...
                        {
                    ?>
                            <input type='text' name='fecha1' id='fecha1' size=10 style='text-align: center;' value='<?php echo $inf['nomdi_fecha_solicitud'] ?>'>
                    <?php
                        }
                        else
                        {
                ...
                <td class='tabla_fila'>
                    ...
                    if($informe_examen)
                    {
                        if($inf['sol_examd_digitacion']=="" || $inf['sol_examd_digitacion']==null)
                        {
                            $fecha_digitacion=date("d/m/Y");
                        }
                        else
                        {
                            $fecha_digitacion=$inf['sol_examd_digitacion'];
                        }
                    }
                    else
                    {
                        if($inf['nomdi_fecha_digitacion']=="" || $inf['nomdi_fecha_digitacion']==null)
                        {
                            $fecha_digitacion=date("d/m/Y");
                        }
                        else
                        {
                            $fecha_digitacion=$inf['nomdi_fecha_digitacion'];
                        }

                    }
                    ?>
                    <input type='text' name='fecha2' id='fecha2' size=10 style='text-align: center;' value='<?php echo $fecha_digitacion; ?>'>
                ...
                <td class='tabla_fila'>
                    ...
                    if($informe_examen)
                    {
                        if($inf['sol_examd_entrega']=="" || $inf['sol_examd_entrega']==null)
                        {
                            $fecha_entrega=date("d/m/Y");
                        }
                        else
                        {
                            $fecha_entrega=$inf['sol_examd_entrega'];
                        }
                    }
                    else
                    {
                        if($inf['nomdi_fecha_entrega']=="" || $inf['nomdi_fecha_entrega']==null)
                        {
                            $fecha_entrega=date("d/m/Y");
                        }
                        else
                        {
                            $fecha_entrega=$inf['nomdi_fecha_entrega'];
                        }

                    }
                    ?>
                    <input type='text' name='fecha3' id='fecha3' size=10 style='text-align: center;' value='<?php echo $fecha_entrega; ?>'>
            ...
            ?>
                <tr>
                    <td style='width:100px;text-align:right;' class='tabla_fila2'>
                        Tipo de Ex&aacute;men:
                    </td>
                    <td class='tabla_fila'>
                        <b><?php echo $inf['sol_tipo_examen']." -- ".$inf['pc_codigo']." -> ".$inf['pc_desc']." (".$inf['pc_grupo'].")";?> </b>
                    </td>
                </tr>
            <?php
            }
            ...
            <tr>
                ...
                <td colspan=5 class='tabla_fila' style='text-align:left;'>
                    <input type='hidden' id='doc_id' name='doc_id' value='<?php echo $inf['doc_id']; ?>'>
                    <input type='text' id='rut_medico' name='rut_medico' size=10 style='text-align: center;' value='<?php echo $inf['doc_rut']; ?>' disabled>
                    <input type='text' id='nombre_medico' value='<?php echo trim($inf['doc_paterno'].' '.$inf['doc_materno'].' '.$inf['doc_nombres']); ?>' name='nombre_medico' size=45>
                </td>
            </tr>
        </table>
    </div>
    ...
    if($informe_examen)
    {
        if($inf['sol_examd_informe']=="" || $inf['sol_examd_informe']==null)
        {
            $informe="";
        }
        else
        {
            $informe=$inf['sol_examd_informe'];
        }
    }
    else
    {
        if($inf['nomdi_html']=="" || $inf['nomdi_html']==null)
        {
            $informe="";
        }
        else
        {
            $informe=$inf['nomdi_html'];
        }
    }
    ?>

    <textarea id='informe' name='informe' style='width:100%;height:350px;'><?php echo $informe; ?></textarea>
    <div class='sub-content'>
        <form id='adjuntar' name='adjuntar' action='adjuntar.php' onSubmit='return false;' method='post' enctype="multipart/form-data">
            <input type='hidden' id='nomd_id' name='nomd_id' value='<?php echo $nomd_id; ?>' />
            <input type='hidden' id='sol_examd_id' name='sol_examd_id' value='<?php echo $sol_examd_id; ?>' />
            <table style='width:100%;'>
                <tr>
                ...
                <tr>
                    <td colspan=4 id='lista_adjuntos'>
                    </td>
                </tr>
            </table>
        </form>

<?php
    ...
    COALESCE(pac_clave, md5(substr(md5(pac_id::text),1,5))) AS pac_clave

    FROM pacientes
    LEFT JOIN comunas USING (ciud_id)
    LEFT JOIN prevision USING (prev_id)
	LEFT JOIN solicitud_examen ON sol_pac_id = pac_id
    ...
    class PDF extends FPDF
    {
        function header()
        {
            ...
            $this->Ln();
            $this->SetX(40);
            $this->Cell(150,4,utf8_decode('Hospital San José de Melipilla'),0,0,'L');
            $this->Ln();
            $this->SetX(40);
            $this->Cell(150,4,utf8_decode('Admisión Atención Abierta'),0,0,'L');
            $this->Ln();
            $this->SetFontSize(14);
            $this->SetY(30);
	}
    ...
    if($lista)
    {
        ...
        for($i=0;$i<count($lista);$i++)
        {
            ...
            $pdf->Cell(140,7,utf8_decode('COMPROBANTE DE CITACIÓN'),1,0,'C',1);
            $pdf->Ln();
            ...
            //$pdf->SetFillColor(240,240,240);
            //$pdf->Cell(40,6,'Tel�fono(s):',1,0,'R',1);
            //$pdf->SetFillColor(255,255,255);
		...
		if($pac['pac_recados']!='') $fonos.=' / '.$pac['pac_recados'];
            ...
            //$pdf->Ln();
            $pdf->SetFillColor(240,240,240);
            $pdf->Cell(40,6,'Fecha Nacimiento:',1,0,'R',1);
            $pdf->SetFillColor(255,255,255);
            $pdf->SetFont('Arial','', 10);
            $pdf->Cell(100,6,$pac['pac_fc_nac'],1,0,'L',1);
            $pdf->Ln();
            $pdf->SetFillColor(240,240,240);
            $pdf->Cell(40,6,'Edad:',1,0,'R',1);
            $pdf->SetFillColor(255,255,255);
            $pdf->SetFont('Arial','', 10);
            $pdf->Cell(100,6,utf8_decode($pac['edad_anios'].' AÑOS '.$pac['edad_meses'].' Meses '.$pac['edad_dias'].' días'),1,0,'L',1);
            $pdf->SetFont('Arial','', 14);
            $pdf->Ln();
            $pdf->SetFillColor(240,240,240);
            $pdf->Cell(40,6,utf8_decode('Ficha Clínica:'),1,0,'R',1);
            $pdf->SetFillColor(255,255,255);
            $pdf->Cell(40,6,$pac['pac_ficha'],1,0,'L',1);
            $pdf->SetFillColor(240,240,240);
            $pdf->Cell(30,6,utf8_decode('Previsión:'),1,0,'R',1);
            $pdf->SetFillColor(255,255,255);
            ...
            /* $pdf->SetFillColor(200,200,200);
            $pdf->Cell(40,7,'Prestaci�n:',1,0,'R',1);
            $pdf->SetFillColor(255,255,255);
            $pdf->SetFont('','B',12);
            ...
            ."4) Si por algún motivo no pudiese asistir debe dar aviso a los numeros 25747622-25747621-25745638  para asignar el cupo a otra persona")))),1);


            $pdf->Ln(8);
            $pdf->SetFont('Arial','BU', 10);
            $pdf->Cell(160,6,utf8_decode('USO EXCLUSIVO RECAUDACION'),0,1,'C');
            $pdf->Ln(1);
            $pdf->SetFont('Arial','', 12);
            $pdf->Cell(40,6,utf8_decode('Previsión:'),0,0,'L');
            ...
            $pdf->SetFont('','BU','16');
            $pdf->Cell(160,10,'Indicaciones',1,1,'C');
            ...
            if($cargar_prestacion)
            {
              for($i=0;$i<count($cargar_prestacion);$i++)
              {
                $pdf->SetFont('Arial','', 12);
                $pdf->SetFillColor(255,255,255);
                $pdf->Cell(60,6,$cargar_prestacion[$i]['pc_codigo'],1,0,'L',1);
                $pdf->SetFont('Arial','', 12);
                $pdf->SetFillColor(255,255,255);
      					$pdf->Cell(100,6,$cargar_prestacion[$i]['pc_desc'],1,0,'L',1);
                $pdf->Ln();

                //$pdf->Cell(40,6,utf8_decode($cargar_prestacion[$i]['pc_desc']),1,0,'R',1);

              }
              $pdf->SetX(185);
              $pdf->Cell(160,150,'',1,0,'C');
            }

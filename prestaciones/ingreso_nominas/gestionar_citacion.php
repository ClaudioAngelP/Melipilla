<?php 
    ...
    else
    {
        if(isset($_POST['observaciones']) AND trim($_POST['observaciones'])!='')
        {
            ...
            $func_id=$_SESSION['sgh_usuario_id'];
            $observaciones=pg_escape_string($_POST['observaciones']);
            pg_query("INSERT INTO interconsulta_detalle_Gestion values (DEFAULT, $inter_id, $func_id, CURRENT_TIMESTAMP, '$observaciones');");
            header('Location: gestionar_citacion.php?inter=1&inter_id='.$inter_id);
            
            exit();
        }
        $g=cargar_Registros_obj("SELECT * FROM interconsulta_detalle_Gestion JOIN funcionario USING (func_id) WHERE inter_id=$inter_id ORDER BY reg_fecha DESC;");
        $lista = cargar_registros_obj("
        SELECT * from interconsulta
        LEFT JOIN pacientes on inter_pac_id=pac_id
        LEFT JOIN especialidades ON inter_unidad=esp_id
...
{
?>
    <input type='hidden' id='inter' name='inter' value=1 />
<?php
}
?>
<input type='hidden' id='inter_id' name='inter_id' value='<?php echo $inter_id; ?>' />

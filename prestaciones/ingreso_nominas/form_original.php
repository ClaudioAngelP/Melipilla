...
<script>
	...
	calcular_totales=function() {
		...
		for(var i=0;i<dnomina.length;i++) {
			...
			if($('nomd_origen_'+nomd_id).value=='A') {
				...
				if($('nomd_institucion_'+nomd_id).value=='') sel_origen(nomd_id);
            } else {
                $('origen_'+nomd_id).disabled=true;
                $('nomd_institucion_'+nomd_id).value='';
            }

			var myAjax=new Ajax.Request('prestaciones/ingreso_nominas/sql_ligero.php', {method:'post',parameters:$('info_nominas').serialize()});
	...
	if(!comprobar_rut($('pac_rut').value))
	{
		...
		return;
					  ...
					  new_win = 
					  window.open('prestaciones/ingreso_nominas/form_paciente.php?'+$('nom_id').serialize()+'&'+$('pac_id').serialize()+'&'+$('nomd_hora').serialize()+'&'+$('nomd_hora_extra').serialize()+'&'+$('duracion').serialize()+'&'+$('pac_rut').serialize(),
...
sel_motivo=function(nomd_id) {
	...
	if(op=='T'){ valor=encodeURIComponent($('nomd_codigo_susp_'+nomd_id).value); tipo=0; }
      ...
      new_win =
      window.open('prestaciones/ingreso_nominas/seleccionar_opcion.php?tipo='+tipo+'&valor='+valor+'&nomd_id='+nomd_id,
      'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
      'menubar=no, scrollbars=yes, resizable=no, width=680, height=500, '+
      'top='+top+', left='+left);
...
sel_origen=function(nomd_id) {
      ...
      new_win =
      window.open('prestaciones/ingreso_nominas/seleccionar_opcion.php?tipo=2&valor='+valor+'&nomd_id='+nomd_id,
      'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
      'menubar=no, scrollbars=yes, resizable=no, width=680, height=500, '+
      'top='+top+', left='+left);

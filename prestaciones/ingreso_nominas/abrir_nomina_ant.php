...
<?php
    ...
    if($lista){
		...
		<td style='text-align:right;font-weight:bold;font-size:14px;' class='tabla_header'>".($i+1)."</td>
		<td style='text-align:center;font-weight:bold;font-size:20px;'>".substr($lista[$i]['nomd_hora'],0,5)."</td>
		<td style='text-align:center;font-weight:bold;font-size:16px;' colspan=$colspan><i>$nombrecupo  $cestado</i></td>
                <td>
                <center>
                    $boton1
		</center>
                        ...
                        if(_cax(313))
                        {
                            ...
                            if($encontrado)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        print("<img src='iconos/table_edit.png'  style='cursor:pointer;' onClick='registrar(".($lista[$i]['nomd_id']).");' />");
                                        print("<img src='iconos/table_edit.png'  style='cursor:pointer;' onClick='imprimir_nomina_detalle(".($lista[$i]['nomd_id']).");' />");
                                    }
                                    else
                                    {
                                        print("<img src='iconos/table_edit.png'  style='cursor:pointer;' onClick='imprimir_nomina_detalle(".($lista[$i]['nomd_id']).");' />");
	...
	$horas_extra_html="<select id='nomd_hora_extra' name='nomd_hora_extra'>";
        ...
        if($agrupar_cupos)
        {
            $consulta="SELECT DISTINCT on (nomd_hora) nomd_hora,nom_id FROM nomina_detalle
            JOIN nomina USING (nom_id)
            WHERE nom_fecha='$fecha' and nom_doc_id=$doc_id and nom_esp_id=$esp_id AND nomd_hora IS NOT NULL AND NOT nomd_hora='00:00:00' ORDER BY nomd_hora";
        }
        else
        {
            $consulta="SELECT DISTINCT nomd_hora FROM nomina_detalle WHERE nom_id=$nom_id AND nomd_hora IS NOT NULL AND NOT nomd_hora='00:00:00' ORDER BY nomd_hora";
        ...
        if($hs)
            for($k=0;$k<sizeof($hs);$k++)
            {
                if($agrupar_cupos)
                {
                    $horas_extra_html.="<option value='".substr($hs[$k]['nomd_hora'],0,5)."_".$hs[$k]['nom_id']."'>".substr($hs[$k]['nomd_hora'],0,5)."</option>";
                }
                else
                {
                    //if($lote)

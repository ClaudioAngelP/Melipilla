...
<script>
        ...
        if(index==0)
        {
            ...
            $j.ajax(
            {
                ...
                success: function(data)
                {
                    ...
                    if(tipo_examen[1]!=false)
                    {
                        $j('#tipo_exam').append('<option value="KITS">****KITS DE EXAMENES****</option>');
                    }
                }
            });
        }
    ...
    agregar_prestacion_examen = function(index,llamada,tipo)
    {
        ...
        if(encontrado==false)
        {
            ...
            else
            {
                var kit_id="";
                if(llamada==1)
                    kit_id=registros_prestaciones[0][index]['pc_id'];
                else
                    kit_id=pc_id;
    ...
    dibujar_favoritos=function()
    {
        ...
        else
        {
            ...
            for(var i=0;i<registros_prestaciones[0].length;i++)
            {
                ...
                var color='';
                var string_kit='';
                if(registros_prestaciones[0][i]['tipo']=='1')
                {
                    var color="font-weight:bold;color:red;";
                    var string_kit=" - (KIT)";
                }
                if(i%2==0) clase='tabla_fila'; else clase='tabla_fila2';
                html+="<tr class="+clase+" style='cursor: pointer;' onMouseOver=this.className=\'mouse_over\' onMouseOut=this.className=\'"+clase+"\' ondblclick='agregar_prestacion_examen("+i+",1,"+registros_prestaciones[0][i]['tipo']+");'>";
                    /*
                    ...
                    */
                    html+='<td style="font-size:12px;text-align:left;'+color+'">'+registros_prestaciones[0][i]['codigo']+'</td>';
                    html+='<td style="font-size:12px;text-align:left;'+color+'">'+descr+string_kit+'</td>';
                    html+='<td style="font-size:12px;text-align:center;'+color+'">'+registros_prestaciones[0][i]['pc_grupo']+'</td>';
                    html+='<td style="font-size:12px;text-align:center;'+color+'">'+registros_prestaciones[0][i]['pc_grupo_examen']+'</td>';
                    html+='<td><center><img src="../../iconos/add.png" style="cursor: pointer;" onClick="agregar_prestacion_examen('+i+',1,'+registros_prestaciones[0][i]['tipo']+');"></center></td>';
    ...
    limpiar_examenes=function(index)
    {
        presta_examen.length = 0;
        listar_prestaciones_examen();
    }
                    ...
                    <div class='tabbed_content' id='tab_examenes_solicitud_content' style='height: 470px; overflow:auto;display:none;'>
                        <div class='sub-content2' id='list_examenes_solicitud' name='list_examenes_solicitud' style='height: 450px; overflow:auto;'>
                                ...
                                <tr>
                                    <td>
                                        <select id='esp_exam' name='esp_exam' onChange='mod_tipo_examen(0);' style='display:block;'>
                                            ...
                                            {
                                            ?>
                                            <option value='99999' >LABORATORIO HME V2</option>
                                            <?php
                                            }
                                            ?>
                                            <!--<?php //echo $espechtml; ?>-->
                            ...
                            <div class='sub-content' style="display: block">
                                <table style='width:100%;' cellpadding=0 cellspacing=0>
                                    <tr>
                                        ...
                                        <td>
                                            <input type='text' id='cantidad_examen' name='cantidad_examen_examen' onKeyUp="if(event.which==13) agregar_prestacion_examen(0,2,($('kit_exam').value*1));" size=3 style="display: none;"/>
                                            <input type='text' id='obs_examen' name='obs_examen_examen' onKeyUp="if(event.which==13) agregar_prestacion_examen(0,2,($('kit_exam').value*1));" size=100 />
                                            <br>
                                        </td>
                                    </tr>
                                    <tr>
                            ...
                            <center>
                                ...
                                <input type='button' value='[ Limpiar Lista ]' onClick='limpiar_examenes();'>
                            </center>
                        </div>
                    </div>
                </td>
...
<script>
    ...
    seleccionar_prestacion_examen = function(presta_examen)
    {
        ...
        //{
            ...
            $('kit_exam').value=(presta_examen[5]*1);
            $('obs_examen').value='';
            $('obs_examen').select();
            $('obs_examen').focus();
    ...
    return {
        method: 'get',
        parameters: 'tipo=proc_prestacion_examen&cod_presta='+encodeURIComponent($('cod_presta_examen').value)+'&esp_id='+encodeURIComponent($('esp_exam').value)
    }

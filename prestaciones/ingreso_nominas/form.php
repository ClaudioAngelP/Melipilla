<?php
    ...
    //{
        ...
        $especialidades=desplegar_opciones_sql("SELECT esp_id, esp_desc FROM especialidades where (esp_codigo_ifl_usuario = '' or esp_codigo_ifl_usuario is null) ORDER BY esp_desc", NULL, '', '');
...
<script>
	...
	left=Math.round(screen.width/2)-325;
		...
		'menubar=no, scrollbars=yes, resizable=no, width=1100, height=700, '+
        'top='+top+', left='+left);
        new_win.focus();					
	return;
    ...
    informe=function(nomd_id)
    {
        ...
        'win_informe', 'toolbar=no, location=no, directories=no, status=no, '+
    ...
    {
        var div=($('encabezado_div').value*1)
        if(div==0)
        {
            obj_esp = document.getElementById('esp_id');
            doc_esp = document.getElementById('doc_id');
            //$('btn_back_fecha').style.display='none';
            //$('btn_fecha').style.display='none';
            //$('btn_next_banco').style.display='none';
            //_general = $('buscar_nominas').innerHTML;
            
            _general="<table><tr><td>Fecha:</td><td>"+$('fecha1').value+"<td></td></tr><tr><td>Especialidad:</td><td>"+obj_esp.options[obj_esp.selectedIndex].text+"</td></tr><tr><td>Doctor / Recurso:</td><td>"+doc_esp.options[doc_esp.selectedIndex].text+"</td></tr></table>"
            //$('btn_back_fecha').style.display='';
            //$('btn_fecha').style.display='';
            //$('btn_next_banco').style.display='';
        ...
        var obj = document.getElementById("td_registro");
        if (obj) {
            $('td_registro').hide();
        }
        obj = document.getElementById("td_iconos");
        if (obj) {
            $('td_iconos').hide();
        }
        obj = document.getElementById("td_tipo_contrato");
        if (obj) {
            $('td_tipo_contrato').show();
        }
        obj = document.getElementById("txt_cant");
        if (obj) {
            for(var i=0;i<($('txt_cant').value*1);i++) {
                obj = document.getElementById("td_registro_"+i);
                if (obj) {
                    $('td_registro_'+i).hide();
                }
                obj = document.getElementById("td_iconos_"+i);
                if (obj) {
                    $('td_iconos_'+i).hide();
                }
                obj = document.getElementById("td_tipo_contrato_"+i);
                if (obj) {
                    $('td_tipo_contrato_'+i).show();
                }
            }
        }
        ...
        obj = document.getElementById("td_registro");
        if (obj) {
            $('td_registro').show();
        }
        obj = document.getElementById("td_iconos");
    ...
    copiar_nomina=function()
    {
        if($j('#nom_id').length)
        {
            ...
            'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
            'menubar=no, scrollbars=yes, resizable=no, width=680, height=430, '+
            'top='+top+', left='+left);
            new_win.focus();
        }
        else
    ...
    imprimir_nomina_detalle=function(nomd_id)
    {
        
        top=Math.round(screen.height/2)-250;
        left=Math.round(screen.width/2)-340;
        new_win = window.open('prestaciones/ingreso_nominas/nomina_detalle.php?nomd_id='+nomd_id,
        'win_nomina', 'toolbar=no, location=no, directories=no, status=no, '+
        'menubar=no, scrollbars=yes, resizable=no, width=680, height=500, '+
        'top='+top+', left='+left);
        new_win.focus();
    ...
    abrir_referencia=function(nomd_id,pac_id,esp_id) {
        <?php if(!_cax(315)){?>
        if (!compare_dates($('fecha1').value, $('fecha_actual').value))
        {
            ...
            top=Math.round(screen.height/2)-250;
            left=Math.round(screen.width/2)-340;
            new_winreferencia_pdf = window.open(ruta+'?nomd_id='+nomd_id,
        ...
        win.setAjaxContent('prestaciones/ingreso_nominas/hoja_contrarreferencia.php', 
        {
            method: 'post',
            async:false,
            dataType: 'json',
            parameters: 'nomd_id='+nomd_id+'&pac_id='+pac_id+'&esp_id='+esp_id+'&llamada=1',
        });
    ...
    function abrir_detalle_nomina(logn_id) {
        l=(screen.availWidth/2)-250;
        t=(screen.availHeight/2)-200;
        win = window.open('visualizar.php?logn_id='+logn_id, 'ver_detale_nomina','scrollbars=no, toolbar=no, left='+l+', top='+t+', '+'resizable=no, width=800, height=600');
        win.focus();
    }
    
    mensaje_nomina=function(index,id)
    {
        if(index=="-1")
        {
            abrir_detalle_nomina(id);
        }
...
<center>
    <div class='sub-content' style='width:95%;'>
        <form id='info_nominas' onSubmit='return false;'>
            ...
            <input type='hidden' id='encabezado_div' name='encabezado_div' value='0' />
            <input type='hidden' id='cant_nominas' name='cant_nominas' value='0' />
            <?php
            if(_cax(314))
            {
            ?>
                <input type='hidden' id='crear_nomina' name='crear_nomina' value='1' />
            <?php
            }
            else
            {
            ?>
                <input type='hidden' id='crear_nomina' name='crear_nomina' value='0' />
            <?php
            }
            ?>
            ...
            <div class='sub-content' id='buscar_nominas' name="buscar_nominas">
                <table>
                    <tr>
                        <td>
                        ...
                        <td>
                            ...
                            <input type='button' id="btn_fecha" name="btn_fecha" value='[VER HOY: <?php echo date('d/m/Y'); ?>]' onClick='$("fecha1").value="<?php echo date("d/m/Y"); ?>";$("esp_id").value="-1";listar_nominas();' >
                        </td>
                        <td>
            ...
            <div class='sub-content' id='datos_nomina' style='display:none;'>
                <table style='width:100%;' border="0">
                    <tr>
                        <td style='width:100px;text-align:right;'>Nro. N&oacute;mina:</td>
                        <td id='nro_nomina' style='width:500px;font-size:12px;font-weight:bold;'></td>
                ...
                <center>
                    ...
                    <input type='button' id='btn_actualizar' onClick='abrir_nomina($("folio_nomina").value, 1);' value='Actualizar Lista'>
                </center>
            </div>

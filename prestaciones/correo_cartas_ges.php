	...
	*/
	...
	$lista=cargar_registros_obj("
		...
		*,
		...
		WHERE NOT mon_estado AND 
		monr_estado=0 AND 
		(
			(monr_clase = '8' AND (mon_fecha_limite-mon_fecha_inicio)<=40) OR
			monr_clase IN ('9','34','35','36','39','6','48')
		)
		ORDER BY mon_id) AS foo
		LEFT JOIN pacientes USING (pac_id)
		left join comunas using (ciud_id)
		ORDER BY pac_ficha,mon_rut;
	");
		...
		$objPHPExcel->getActiveSheet()->setCellValue('C2', 'NOMINA DE CORREOS GES ENVIADOS');
	...
	$objWriter->save('/tmp/nomina_cartas_excepciones.xls');
	
	


	// GENERACIÓN DE PDF CON LAS CARTAS
	
	require_once("correo_cartas_ges_generar_pdf.php");
...
<i>Estimados, se adjunta n&oacute;mina (XLS) y cartas de Monitoreo GES en formato imprimible (PDF) a este e-mail.</i>

<br /><br />

<b><u>Pacientes clasificados para EXCEPCI&Oacute;N Y CORREO</u></b><br /><br />

<?php echo $resumen1; ?>

<br /><br />

<i>Con esta acci&oacute;n, todos estos pacientes han sido reclasificados como "Correo Enviado".<br /><br />Saludos cordiales.</i>
...
<i>Estimados, no se detectaron cartas pendientes. Se notifica como aviso de que el proceso si est&aacute; siendo ejecutado.<br /><br />Saludos cordiales.</i>
...
error_reporting(E_ALL);
 ...
 if($attach) {
	$mime->addAttachment('/tmp/nomina_cartas_excepciones.xls','application/xlsx');
	$mime->addAttachment('/tmp/cartas_y_excepciones.pdf','application/pdf');
 }
...
//$mails='rodrigo.carvajal@sistemasexpertos.cl';
 ...
 for($i=0;$i<sizeof($mail_array);$i++) {
	...
	send_mail($email,'sistemagis.hgf@redsalud.gov.cl',utf8_decode('Monitoreo GES - Excepciones y Correos '.date('d/m/Y H:i')),$html,$lista);
 }
 
 $id_condicion=11;  // CORREO ENVIADO
 ...
 for($i=0;$i<sizeof($lista);$i++) {
	...
	$comentarios=utf8_decode('CORREO ENVIADO EL '.date('d/m/Y').' A LAS '.date('H:i:s')).' - CAUSAL: '.$lista[$i]['nombre_condicion'];
	...
	if($lista)
	echo 'Enviadas '.sizeof($lista).' cartas por email.';
	else	echo 'No hay cartas pendientes.';

	if(file_exists('/tmp/cartas_y_excepciones.pdf')) unlink('/tmp/cartas_y_excepciones.pdf');
	if(file_exists('/tmp/nomina_cartas_excepciones.xls')); unlink('/tmp/nomina_cartas_excepciones.xls');

  ...
  ", false);
  	...
  	class PDF extends FPDF {
		function header() {
			...
			$this->Ln();
			$this->SetX(50);
			$this->Cell(150,7,('SS Viña del Mar Quillota'),0,0,'L');	
			$this->Ln();
			$this->SetX(50);
			$this->Cell(150,7,('Hospital Dr. Gustavo Fricke Viña del Mar'),0,0,'L');	
			$this->Ln();
			$this->SetX(50);
...
Avise de inmediato cambios de dirección o teléfono.<br><br>"))));
	...
	$pdf->Cell(160,7,$lista[$i]['nom_fecha'].' A LAS '.substr($lista[$i]['nomd_hora'],0,5).' HORAS.',1,0,'L',1);	
	$pdf->Ln();
	...
	$pdf->SetFillColor(255,255,255);
	$pdf->Cell(160,7,$lista[$i]['esp_nombre_especialidad'],1,0,'L',1);	
	$pdf->Ln();
	...
	$pdf->SetFillColor(255,255,255);
	$pdf->Cell(160,7,strtoupper($pac['pac_direccion']),1,0,'L',1);	
	$pdf->Ln();
	...
	$pdf->Output('CITACIONES_'.strtoupper(trim($rut)).'.pdf','I');	

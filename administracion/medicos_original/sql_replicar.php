<?php
  ...
  for($i=0;$i<7;$i++)
    if(isset($_POST['dia_'.$i])) $dia[$i]=true; else $dia[$i]=false;
  
  $fechas = cargar_registros_obj("
    SELECT DISTINCT
      date_trunc('day', cupos_fecha) AS cupos_fecha,
      cupos_horainicio, cupos_horafinal, cupos_id, 
      cupos_cantidad_n, cupos_cantidad_c, 
      cupos_cant_i, cupos_cant_h, cupos_cant_e,
      cupos_extras, cupos_cant_r,
      esp_desc, esp_id
    FROM cupos_atencion
  ...
  for(;$di<=$df;$di+=86400) {
    ...
    $d=date('d/m/Y', $di);
    if(array_search($d, $fechas_ausente)) continue;
    ...
    for($i=0;$i<count($fechas);$i++) {
	  ...
	  pg_query("INSERT INTO nomina VALUES (DEFAULT, 'AGENDA' || CURRVAL('nomina_nom_id_seq'), $esp_id, $doc_id, '', 0, false, '$d');");

<?php
    ...
    SELECT *, 
    (SELECT COUNT(*) FROM convenio_detalle WHERE convenio_detalle.convenio_id=fooo.convenio_id and art_id!=0) AS arts, 
    (SELECT COUNT(*) FROM orden_compra WHERE orden_licitacion=convenio_licitacion AND fooo.prov_id=orden_prov_id) AS ocs 
    FROM (
        SELECT DISTINCT c1.*, (convenio_fecha_final-CURRENT_DATE) AS dias_vigencia,prov_glosa,upper(prov_rut)as prov_rut 
        FROM convenio AS c1
        JOIN proveedor USING (prov_id)
        $art_w
        WHERE $lic_w AND $prov_w
        ORDER BY c1.convenio_nombre
    ) AS fooo WHERE $estado_w

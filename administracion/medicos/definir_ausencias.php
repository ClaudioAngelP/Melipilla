<?php
  ...
  '1', 'true', 'ORDER BY motivo_id')); 

  $horashtml='';
  for($i=0;$i<24;$i++) 
  {
    ($i<10) ? $h='0'.$i : $h=$i;
    if($h=='08') $sel='SELECTED'; else $sel='';
...
listar_ausencias=function() {
  ...
  html+='<td>Hora T&eacute;rmino</td>';
  ...
  for(var i=0;i<ausencias.length;i++) {
    ...
    if(ausencias[i].hora_final!=null) {
      html+='<td style="text-align:center;" valign="top">'+ausencias[i].hora_inicio+'</td>';
      html+='<td style="text-align:center;" valign="top">'+ausencias[i].hora_final+'</td>';
    } else {
      html+='<td style="text-align:center;" valign="top" colspan=2>'+ausencias[i].hora_inicio+'</td>';
    }
  ...
  html+='<td style="text-align:center;" valign="top"><select id="hr_desde" name="hr_desde" style="width:70px;">';
  html+='<?php echo $horashtml; ?></select></td>';
  html+='<td style="text-align:center;" valign="top"><select id="hr_hasta" name="hr_hasta" style="width:70px;">';
  html+='<?php echo $horashtml; ?></select></td>';
  html+='<td style="text-align:center;"><select id="motivo" name="motivo" style="width:200px;">';
...
agregar_ausencia = function() {
    ...
    if($('hr_hasta').value=="")
    {
        alert("Debe ingresar Hora Final");
        return;
    }
  ...
  params+=$('hr_desde').serialize()+'&';
  params+=$('hr_hasta').serialize()+'&';

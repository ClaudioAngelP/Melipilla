...
<table style='width: 100%;' cellspacing=0>
    ...
    <?php
        ...
        $gastos = cargar_registros_obj("
        			...
        			SELECT foo.centro_ruta, centro_nombre, art_item, item_glosa, gasto, ppto.ppto_monto AS ppto_monto, ppto2.ppto_monto AS monto_cc FROM (
					...
					LEFT JOIN centro_costo_presupuesto AS ppto2 ON foo.centro_ruta=ppto2.centro_ruta AND ppto2.ppto_item='' AND ppto2.ppto_fecha='$f1'
					LEFT JOIN centro_costo_presupuesto AS ppto ON foo.centro_ruta=ppto.centro_ruta AND foo.art_item=ppto.ppto_item AND ppto.ppto_fecha='$f1'
        ...
        $suma_monto=0; $suma_gasto=0; $suma_saldo=0;
        ...
        for($i=0;$i<sizeof($gastos);$i++)
        {
            ...
            if($tmp!=$gastos[$i]['centro_nombre']) {
				...
				if($i>0) {
					...
					if($gastos[$i-1]['monto_cc']!='') {
						$suma_monto=$gastos[$i-1]['monto_cc']*1;
					}
					
					if($suma_monto<=$suma_gasto)
					$grafico=graficar_item(
							array(	$suma_monto,
							0,
							$suma_gasto,
							$suma_monto-$suma_gasto));
					else
					$grafico=graficar_item(
							array(	$suma_monto,
							$suma_gasto,
					...
					print("<tr class='tabla_header'><td colspan=2 style='text-align:right;'>Total:</td><td style='font-size:16px;'>$".number_format($suma_monto,0,',','.').".-</td>
					<td>
					<center>
						<img src='iconos/pencil.png' onClick='modificar_ppto(\"".$gastos[$i-1]['centro_ruta']."\", \"".$gastos[$i-1]['centro_nombre']."\", \"\");' style='cursor:pointer;' />
					</center>
					</td>
					<td style='font-size:16px;'>$".number_format($suma_gasto,0,',','.').".-</td><td style='font-size:16px;'>$".number_format($suma_saldo,0,',','.').".-</td><td style='border:0px;'><center>$grafico</center></td><td>&nbsp;</td></tr></table><hr><table style='width:100%;' cellspacing=0>");
				...
				<td colspan=8 class='tabla_header' style='font-size:24px;padding:5px;'><u>".$gastos[$i]['centro_nombre']."</u></td>
				</tr>
            ...
            $suma_monto+=$gastos[$i]['ppto_monto']*1;
            $suma_gasto+=$gastos[$i]['gasto']*1;
            $suma_saldo+=$saldo*1;

...
function vdevolucion($dev_id,$xls=false,$ruta='') {
			...
			if(!$xls) {
				return "<span style='cursor:pointer;white-space:nowrap;text-decoration:underline;font-weight:bold;color:blue;' onClick='imprimir_dev_boletin($dev_id);'>
	...
	$d=cargar_registros_obj("SELECT *, date_trunc('second',dev_fecha) AS dev_fec FROM devolucion_boletines 
...
imprimir_dev_boletin=function(dev_id) {
		var valor=true;
...
<td style='font-weight:bold;color:red;'><?php 
		for($i=0;$i<sizeof($d);$i++)
			echo vdevolucion($d[$i]['devol_id'],0,'../').'<br />';	
	?></td>
</tr>
<?php } ?>
...
<center>
<?php if((_cax(325) OR _cax(322)) AND $r['anulacion']=='') { ?>

<?php 
	...
	$prev_id=$p['prev_id']*1;
	$origen_id=pg_escape_string($_POST['filtro_origen']*1);
	$prev_desc=pg_escape_string($_POST['prevision']);
	$isapre=pg_escape_string($_POST['isapre_select']*1);
	...
	pg_query("INSERT INTO boletines VALUES (
		...
		$nboleta, '$datos_pagare', '$modalidad', '', $garantia,$isapre,$origen_id
	);");
	...
	for($i=0;$i<sizeof($presta);$i++) {
		...
		if(($presta[$i]->presta_id*1)>0 AND isset($presta[$i]->ptipo)) {
			...
			if($presta[$i]->ptipo=='R') {
				pg_query("UPDATE recetas_pago SET rp_desp_pago=1 WHERE rp_stock_id=".($presta[$i]->presta_id*1));
			}
		}
...
*/	
		...
		pg_query("INSERT INTO seguros VALUES (
			DEFAULT, $currval, 
			".($seguros[$i]->tipo*1).", 
			'".pg_escape_string($seguros[$i]->compania)."',			
			...
			0,
			current_timestamp,
			'".pg_escape_string($seguros[$i]->prev_id)."',
			".($seguros[$i]->esp_id*1).",
			'".pg_escape_string($seguros[$i]->fono)."');");
	...
	for($i=0;$i<sizeof($otras);$i++) {
		...
		if($c[3]=='')		
			...
			switch(strlen($c[3])) {			
				case 8:
				$c[3]=substr($c[3],0,2).'/'.substr($c[3],2,2).'/'.substr($c[3],4,5);
				break;
				case 6:
				$c[3]=substr($c[3],0,2).'/'.substr($c[3],2,2).'/20'.substr($c[3],4,2);
				break;
			}
		}		
		$num=($c[5])*1;
	...
	print(json_encode(array(true,$boletin['bolnum'],$garantia)));

			...
			for(var i=0;i<prestaciones.length;i++) {
				...
				if($('pagare').value*1==1 || $('pagare').value*1==2 )
					...
					if(!pabdental2) {
						...
						if(prev_id==12 || pac_edad>=60) {factor_pab2=0; }
						else if(prev_id==10) {factor_pab2=0; }
						else if(prev_id==11){ factor_pab2=0.1; }
						else if(prev_id==15){ factor_pab2=0.2;}
						else{ factor_pab2=1; }
						
					} else {
						if(prev_id==12 || pac_edad>=60) factor_pab2=0;
						else if(prev_id==10) factor_pab2=0.3;
						else if(prev_id==11) factor_pab2=0.5;
						else if(prev_id==15) factor_pab2=0.8;
						else factor_pab2=1;
					}
		...
		if(cod!=undefined){
			...
			if($('convenios_hosp').value==3 || $('convenios_hosp').value==4 || $('convenios_hosp').value==5 || $('convenios_hosp').value==6 || $('convenios_hosp').value==7 || $('convenios_hosp').value==8 || $('convenios_hosp').value==9) {
				...
				if($('frec_id').value*1>0) {
					...
					if(prev_id>=10 && prev_id<=15) { total=cod.transferen*1; valor=cod.transferen*1; }

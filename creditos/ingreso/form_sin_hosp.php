		...
		'ingresos/certificar_paciente.php',
		{
			...
			onComplete:function(r) {
				...
				if(d) {
					...
					if(d['prais']!='000') $('prevision').value='PRAIS';
				cargar_prestaciones();
...
<td style='font-weight:bold;'>
<input type='text' id='cliid' name='cliid' style='font-size:14px;text-align:center;'
 onKeyUp=' if(event.which==13 ) validar_id();' size=12 />
</td>
...
<script> 
	...
	if($('pagare').value!=3 || $('pagare').value!=4) {
	...
	var cod=info_codigo2(codigo, modalidad);
		...
		if( cod.pago_fijo=='t' ) {
			...
			if($('pagare').value==3 || $('pagare').value==4) {
					if($('pagare').value==3 )//capedena copago d
					{
					...
					if($('pagare').value==4 )//dipreca copago c
					{
				...
				if($('frec_id').value*1>0) {
					...
					if(prev_id>=10 && prev_id<=15) { total=cod.precio*1; valor=cod.precio*1; }
					else if(prev_id==5) { total=cod.precio*1; valor=cod.precio*1; }
					else { total=cod.precio*1; valor=cod.precio*1; }

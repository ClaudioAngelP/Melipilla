...
validar_bolnum = function(){
	...
	var myAjax=new Ajax.Request('ingresos/datos_boletin.php',
	{
		method:'post',
		...
		onComplete: function(resp) {
			...
			if(d) {
				if(d['anulacion']=='' && (d['pagare']=='t' || d['garantia']=='t')){
					$('pac_id').value=d['pac_id']*1;
					$('clirut').value=d['pac_rut'];
					$('cliid').value=d['id_sidra']*1;
					if(d['pac_rut']!=''){
					...
					alert('El Boletin ingresado no puede ser Regularizado.');
					cambiar_pagina('creditos/ingreso/form_regula.php');
...
validar_rut = function() {
	...
	$rut=$('clirut').value;
	if($('clirut').value==''){$rut=$('cliid').value;}

	var myAjax=new Ajax.Request('ingresos/datos_paciente.php',
	{
		method:'post',
		parameters: 'str='+encodeURIComponent($rut),
		onComplete: function(resp) {
...
actualizar_prevision_sn=function(sn_v) {

	$('prevision').value='Cargando...';

	var myAjax=new Ajax.Request(
		'ingresos/certificar_paciente_sn.php',
		{
			method:'post',parameters:'rut='+encodeURIComponent(sn_v),
...
cargar_prestaciones=function() {
	...
	var myAjax=new Ajax.Request(
		'ingresos/prestaciones_regula.php',
		{
			method:'post',
			parameters:$('pac_id').serialize()+'&'+$('filtro_presta').serialize()+'&'+$('modalidad').serialize()+'&'+$('frec_id').serialize()+'&'+$('pagare').serialize()+'&'+$('nboletin').serialize(),
			onComplete:function(resp) {
...
calcular_cuota=function() {
	...
	if(pie>=proval) {
		$('ingresa').value='Regularizar Recaudaci&oacute;n ... >>'.unescapeHTML();
	...
	'creditos/sql_regula.php',
	{
		...
		onComplete: function(resp) {
			...
			try {		
				...
				alert( 'ERROR:\n\n' + resp.responseText.unescapeHTML() +'error');			
				$('ingresa').disabled=false;	
...
<b>Regularizaci&oacute;n de Prestaciones</b>
</td>
...
<td style='text-align:right;width:10%;'>R.U.N.:</td>
<td style='font-weight:bold;'>
<input type='text' id='clirut' name='clirut' style='font-size:14px;text-align:center;' size=12 />
</td>

<td style='text-align:right;width:0%;'>ID.:</td>
<td style='font-weight:bold;'>
<input type='text' id='cliid' name='cliid' style='font-size:14px;text-align:center;' size=12 />
</td>
...
value='Regularizar Recaudaci&oacute;n ... >>'>
</center>

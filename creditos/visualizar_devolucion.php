<?php 
	...
	$r=cargar_registro("SELECT *, date_trunc('second',dev_ejecuta) AS bolfec FROM boletines 
	LEFT JOIN funcionario func_id=func_id_ejecuta 	
	LEFT JOIN pacientes USING (pac_id)
	WHERE bolnum=".$devol, true);
	
	echo "SELECT *, date_trunc('second',bolfec) AS bolfec FROM boletines 
	LEFT JOIN funcionario USING (func_id) 	
	LEFT JOIN pacientes USING (pac_id)
	WHERE bolnum=".$devol;
	
	$garantia=$r['garantia']=='t'?true:false;

	$a=cargar_registros_obj("SELECT * FROM boletines WHERE bolnumx=".$devol);
	...
	$chq=cargar_registros_obj("SELECT * FROM cheques WHERE bolnum=".$devol);
...
<b>Datos del Comprobante de Pago <u>#<?php echo number_format($devol, 0,',','.'); ?></u></b>
</div>
...
<center>
<?php if((_cax(302) OR _cax(305)) AND $r['anulacion']=='') { ?>
<input type='button' value='--- <?php if($garantia) echo 'Reguralizar Garant&iacute;a'; else echo 'Anulaci&oacute;n de Comprobante ...'; ?> ---' 
onClick='eliminar_boletin(<?php echo $devol; ?>);' />
<?php } ?>
<input type='button' value='--- Imprimir Bolet&iacute;n... ---' 
onClick='imprimir_boletin(<?php echo $devol; ?>);' />
<br/><br/>
</center>

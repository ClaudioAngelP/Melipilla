...
<div class='sub-content3' style='overflow:auto;'>
    ...
    WHERE pac_id=$paciente AND nomd_observaciones!='' and (nomd_diag_cod NOT IN ('H','T','B'))
        ...
        else
        {
            ...
            for($k=0;$k<sizeof($obs);$k++)
            {
                ...
                print('<td style="width:10%;font-weight:bold;";>'.$obs[$k]['fecha'].'</td>
                    <td style="width:30%;font-size:10px;">'.htmlentities($obs[$k]['doc']).'<br><b>('.$obs[$k]['esp_desc'].')</b></td>
                    <td>'.htmlentities($obs[$k]['nomd_observaciones']).'</td>
                    <td>
                        <center>
                ');
                if($obs[$k]['nomd_id']==$nomd_id)

...
<script>
    ...
    cargar_ultima_receta=function(receta_id)
    {
        ...
        {
            method:'post',
            parameters:'pac_id=<?php echo $paciente; ?>&'+$('bodega_id').serialize()+params,
            onComplete:function(datos)
            {
                
                var r=datos.responseText.evalJSON(true);
                if(r.receta_cronica=='t')
                {
                    $('cronica').checked=true;
                    actualizar_cronica();
                }
                $('receta_id').value=r.id;
                $('fecha_retro').value=r.fecha_retro;
		$('rut_medico').value=r.doc_rut;
		$('nombre_medico').value=(r.doc_paterno+' '+r.doc_materno+' '+r.doc_nombres).unescapeHTML();
                
		$('centro_servicio').value=r.receta_centro_ruta;
                $('centro_costo').value=r.centro_nombre.unescapeHTML();
                
                $('observaciones').value=r.receta_comentarios.unescapeHTML();
                
                if((r.receta_tipotalonario_id*1)!=0){
                    $('nro_receta').value=(r.receta_numero*1);
                    document.getElementById("nro_receta").readOnly = true;
                    $('tipo_talonario').value=(r.receta_tipotalonario_id*1);
                }
                if(r.receta_prov_alta=='t'){
                    document.getElementById("prov_alta").checked = true;
                } else {
                    document.getElementById("prov_alta").checked = false;
                }
		...
		for(var i=0;i<r.detalle.length;i++)
                {
                    ...
                    medicamentos[conta_med][5]=r.detalle[i].art_glosa.unescapeHTML();
                    ...
                    medicamentos[conta_med][11]=(r.detalle[i].art_tipo_adm*1);
                    if((r.detalle[i].art_tipo_adm*1)!=2)
                    {
                    ...
                    {
                        var tmp=r.detalle[i].recetad_indicaciones.split(']');
                        var tmp2=tmp[0].split('|');
                        medicamentos[conta_med][12]=tmp2[0].replace('[M:','')*1;
                        medicamentos[conta_med][13]=tmp2[1].replace('T:','')*1;
                        medicamentos[conta_med][14]=tmp2[2].replace('N:','')*1;
                    }
                    
                    medicamentos[conta_med][15]=medicamentos[conta_med][12]+'-'+medicamentos[conta_med][13]+'-'+medicamentos[conta_med][14];
                    medicamentos[conta_med][16]=(r.detalle[i].recetad_observacion.unescapeHTML());
                    //medicamentos[conta_med][17]=($('rangodosis').value*1);
                    //medicamentos[conta_med][18]=($('rangodosis2').value*1);
                    medicamentos[conta_med][17]=1;
                    medicamentos[conta_med][18]=1;
                    medicamentos[conta_med][19]=true;
                    
                    medicamentos[conta_med][20]=(r.detalle[i].cant_lotes*1);
                ...
                if(receta_control==1)
                {
                    ...
                    if(opciones_talonario.charAt(0)=='t') {
                        ...
                        if(r.adq_rut.toUpperCase()==r.pac_rut.toUpperCase()) {
                            $('adq_retira').value=0;
                        } else {
                            $('adq_retira').value=1;
                        }
                        $('adq_rut').value=r.adq_rut.toUpperCase();
                        $('adq_appat').value=r.adq_appat.unescapeHTML();
                        $('adq_apmat').value=r.adq_apmat.unescapeHTML();
                        $('adq_nombres').value=r.adq_nombres.unescapeHTML();
                        $('adq_direccion').value=r.adq_direccion.unescapeHTML();
                    } else {
            ...
            var myAjax=new Ajax.Request('ficha_clinica/datos_adquiriente.php',
            {
                ...
                onComplete:function(r)
                {
                    try 
                    {
                        ...
                        {
                            $('adq_nombres').value=datos.adq_nombres.unescapeHTML();
                            $('adq_appat').value=datos.adq_appat.unescapeHTML();
                            $('adq_apmat').value=datos.adq_apmat.unescapeHTML();
    ...
    agregar_art = function()
    {
        ...
        if($('art_tipo_adm').value*1!=2)
        {
            ...
            */
            ...
            if($('fraccion').value=='')
                medicamentos[conta_med][1]=$('cant').value*1;
            else
                medicamentos[conta_med][1]=$('fraccion').value*1;
            
            medicamentos[conta_med][2]=(($('horas').value*1)*($('rangodosis2').value*1));
            medicamentos[conta_med][3]=(($('dias').value*1)*($('rangodosis').value*1));
        ...
        medicamentos[conta_med][15]=medicamentos[conta_med][12]+'-'+medicamentos[conta_med][13]+'-'+medicamentos[conta_med][14];
	medicamentos[conta_med][16]=($('txt_observacion').value);
	medicamentos[conta_med][17]=($('rangodosis').value*1);
	medicamentos[conta_med][18]=($('rangodosis2').value*1);
	medicamentos[conta_med][19]=true;
        medicamentos[conta_med][20]=($('articulo_lotes').value*1);
        ...
        //}
        redibujar_tabla();
        $('lista_medicamentos').scrollTop=0;
        //$('lista_medicamentos').scrollTop=$('lista_medicamentos').scrollHeight;
        conta_med++;
        $('articulo_id').value='';
        $('cant').value='';
        $('cant1').value='';
    ...
    abrir_articulo = function(d)
    {
        ...
        /*
        var myAjax = new Ajax.Updater('detalle_prod', 'mostrar.php', 
        {
            method: 'get', 
            evalScripts: true,
        ...
        */
	var myAjax=new Ajax.Request('ficha_clinica/datos_medicamento.php',
        {
            method:'post',
			...
			$('cant_normal').show();
                    ...
                    $('articulo_lotes').value=d.cant_lotes*1;
    ...
    {
        /*
        if($('rangodosis').value*1==2)
        {
            dias_q=$('dias').value*30;
        }
        else
        {
            dias_q=$('dias').value*1;
        ...
        */
        dias_q=(($('dias').value*1)*($('rangodosis').value*1));
        /*
        if($('rangodosis2').value*1==2)
        {
            horas_q=$('horas').value*24;
        }
        else
        {
            horas_q=$('horas').value*1;
        }
        */
        horas_q=(($('horas').value*1)*($('rangodosis2').value*1));
        var cantidad=0;
        ...
        if($('fraccion').value=='')
        {
            $('cant').disabled=false;
            if($('art_tipo_adm').value*1==2)
                cantidad=($('cant1').value*1)+($('cant2').value*1)+($('cant3').value*1);
            else
                cantidad=$('cant').value;
        }
        else
        {
            cantidad=$('fraccion').value*1;
            $('cant').value='';
            $('cant').disabled=true;
        }
        ...
        //horas=(($('horas').value*1)*$('rangodosis2').value);
        /*
        if($('art_tipo_adm').value*1==2)
            cantidad=($('cant1').value*1)+($('cant2').value*1)+($('cant3').value*1);
        else
      ...
      check_despachable = function(x){
			
			//console.log($('despa_'+x).checked);
			
			if($('despa_'+x).checked)
				medicamentos[x][19]=true;
			else
				medicamentos[x][19]=false;
				
		  console.log(medicamentos[x]);
	}
	
	check_articulo = function(){
		
		for(i=conta_med;i>=0;i--) {
      //console.log(medicamentos[i]);
          ...
          if( medicamentos[i][1] >= 1 ) {
				cant =  medicamentos[i][1];
		  } else {
			if( medicamentos[i][1] == 0.25 ) 
				cant='1/4';  
			else if ( medicamentos[i][1] == 0.5 )
				cant='1/2';
			else if ( medicamentos[i][1] == 0.75 )
				cant='3/4';
			else
				cant= medicamentos[i][1];
		  }
	      //cant_2=(medicamentos[i][2]*1);
	      horas=(medicamentos[i][2]*1);
	      //dias_q=medicamentos[i][3]*1;
	      dias=medicamentos[i][3]*1;

		totalua = Math.ceil(1*((medicamentos[i][3]*24))/(medicamentos[i][2])*(medicamentos[i][1])/(medicamentos[i][8]*1));
		
		if($('cronica').checked && medicamentos[i][3]>30)
				totaldesp=Math.floor(totalua/(medicamentos[i][3]/30));
		else totaldesp=totalua;
        
        totalcant=totaldesp;  
        if(totaldesp>art_stock){ medicamentos[i][9]=false; $('despa_'+i).checked=false;}
        else{ medicamentos[i][9]=true; $('despa_'+i).checked=true;}
		
		if(art_stock<=0){ medicamentos[i][9]=false; $('despa_'+i).checked=false;}
		
		if(!$('despachar').checked){ $('despa_'+i).checked=false; }
    ...
    {
        tabla_html='<table width=100% id=\"seleccion\"><tr class=\"tabla_header\" style=\"font-weight: bold;\"> <td>C&oacute;digo Int.</td><td>Nombre</td><td colspan=2>Cant.</td><td>U.A.</td><td>Despachar</td><td>Quitar</td></tr>';
        for(i=conta_med;i>=0;i--)
        {
            //console.log(medicamentos[i]);
            if(medicamentos[i]==null)
            ...
            //dias_q=medicamentos[i][3]*1;
            dias=medicamentos[i][3]*1;
            dosis = cant + ' <b>' + medicamentos[i][9] + '</b> <i>cada</i> ' + horas + '<i> horas,  durante</i> ' + dias + ' <i>d&iacute;as.</i>';
            ...
            {
                if($('cronica').checked && medicamentos[i][3]>30)
                    totaldesp=Math.ceil(totalua/(medicamentos[i][3]/30));
                else
                    totaldesp=totalua;
            }
            else
            ...
            tabla_html+='<tr class=\"tabla_fila\">';
                tabla_html+='<td style=\"text-align:right;font-size:12px;\"><b>'+codigo+'</b></td>';
                tabla_html+='<td style=\"font-size:14px;color:blue;\">'+nombre+'</td>';
                tabla_html+='<td style="text-align:right;">'+number_format(totaldesp,2)+'.- / ('+number_format(totalua,2)+'.-)</td>';
                tabla_html+='<td>';
                tabla_html+='<input type="text" name="totalcant_'+i+'" id="totalcant_'+i+'" value="'+totalcant+'"';
                tabla_html+='onKeyUp="this.value=(this.value*1); if('+medicamentos[i][0]+'!=116562 && '+medicamentos[i][0]+'!=116654 && '+medicamentos[i][0]+'!=116841 && '+medicamentos[i][0]+'!=116563 && '+medicamentos[i][0]+'!=116772 && '+medicamentos[i][0]+'!=116671 && '+medicamentos[i][0]+'!=116706){if(this.value>'+totaldesp+') this.value='+totaldesp+';}else{if(this.value>'+totalua+') this.value='+totaldesp+';}if(this.value>'+art_stock+') this.value='+art_stock+';" size=7 style="text-align:center;" >';
                tabla_html+='</td>';
                tabla_html+='<td style="font-weight:bold;">'+medicamentos[i][10]+'</td>';
                tabla_html+='<td rowspan=3><center><input type="checkbox" name="despa_'+i+'" id="despa_'+i+'" '+chkd+' onChange="check_despachable('+i+');"></center></td>';
                tabla_html+='<td rowspan=3><center><img src=\"iconos/delete.png\" onClick=\"quitar_art('+i+');\" alt=\"Quitar Medicamento...\" title=\"Quitar Medicamento\" style="cursor: pointer;"></center></td>';
            tabla_html+='</tr>';
            tabla_html+='<tr class=\"tabla_fila2\">';
                tabla_html+='<td style="text-align:right;"><b>Dosis/Stock(Lotes Vigentes):</b></td>';
                tabla_html+='<td>'+dosis+'</td>';
                
                var color_lote='';
                if((medicamentos[i][20]*1)>0)
                    color_lote='blue';
                else
                    color_lote='red';
                
                tabla_html+='<td style=\"text-align: right; '+texto_color+'\">'+number_format(art_stock,2)+'.- / (<font color='+color_lote+'>'+medicamentos[i][20]+'</font>)</td>';
                //tabla_html+='<td style=\"text-align: right; '+texto_color+'\">'+number_format(art_stock,2)+'.-</td>';
                tabla_html+='<td>&nbsp;</td>';
                tabla_html+='<td style="font-weight:bold;">'+medicamentos[i][10]+'</td>';
            tabla_html+='</tr>';
            tabla_html+='<tr class=\"tabla_fila\">';
                tabla_html+='<td style=\"font-weight:bold;text-align:right;\">Observaci&oacute;n:</td>';
                tabla_html+='<td colspan=4>'+medicamentos[i][16]+'</td>';
            tabla_html+='</tr>';
        }
    ...
    verifica_receta_tabla = function () {
		...
		if(($('paciente').value*1)==0){
			alert('Debe ingresar un paciente v&aacute;lido para la receta.'.unescapeHTML());
            return;
		}
		if($('paciente').value=="") {
			alert('Debe ingresar un paciente v&aacute;lido para la receta.'.unescapeHTML());
        ...
        if(directo)
        {
            ...
            */
            ...
            if($('centro_servicio').value=="-1" || $('centro_servicio').value=='')
            {
                alert('Debe seleccionar Centro de Costo/Servicio.');
                return;
            }
        ...
        for(i=0;i<medicamentos.length;i++)
        {
            ...
            if(receta_control==0)
            {
                if($('cronica').checked && medicamentos[i][3]>30) 
                {
                    ...
                    //if(medicamentos[i][3]>28)
                    cantidad_art=Math.floor(cantidad_art/(medicamentos[i][3]/30));
                }
                else
                {
                    cantidad_art = Math.ceil(1*((medicamentos[i][3]*24))/(medicamentos[i][2])*(medicamentos[i][1])/(medicamentos[i][8]*1));
            ...
            cantidad_desp=$('totalcant_'+i).value;
            if((medicamentos[i][6]*1)<(cantidad_desp*1) && $('despachar').checked)
            {
                ...
                msg+='\n\n &iquest;Desea ingresar la Receta de todas maneras?';
                if(!confirm(msg))  
                    return;
            }
            if((medicamentos[i][20]*1)<=0 && $('despachar').checked && $('despa_'+i+'').checked) {
                var msg='No hay un LOTES VIGENTES DISPONIBLES no se realizar&aacute; el Despacho para el siguiente medicamento:\n\n';
                msg+='['+medicamentos[i][4]+'] '+medicamentos[i][5];
                msg+='\n\n &iquest;Desea ingresar la Receta de todas maneras?';
                
                if(!confirm(msg.unescapeHTML()))  
                    return;
            }
            
            
            cadena+=medicamentos[i][0]+'/'+medicamentos[i][1]+'/'+medicamentos[i][2]+'/'+medicamentos[i][3]+'/'+medicamentos[i][8]+'/'+medicamentos[i][12]+'/'+medicamentos[i][13]+'/'+medicamentos[i][14]+'/'+medicamentos[i][15]+'/'+medicamentos[i][19]+'/'+cantidad_desp+'!';
            observacion+=medicamentos[i][16]+'!';
        ...
        $('medica').value=cadena;
        $('observacion_m').value=observacion;
        if (cadena=='')
        {
        ...
        var myAjax = new Ajax.Request('ficha_clinica/sql_receta.php', 
        {
            method: 'get', 
            ...
            onComplete: function(pedido_datos)
            {
                ...
                if(datos[0]==true)
                {
                    ...
                    alert('Receta ingresada exitosamente. Nro ['+datos[2]+']');
                    //} 
                    if(es_cronica)
                    {
...
<form name='receta' id='receta'>
    <input type='hidden' id='receta_id' name='receta_id' value='' />
    <?php
    if(!$cheque)
    {
    ?>      
        ...
        <div class='sub-content'>
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>
                        <input type='text' id='nro_receta' name='nro_receta' style='text-align:right;' onKeyUp='if(event.which==13) $("nombre_medico").focus();' value=''>
                    </td>
                </tr>
            </table>
        </div>
        ...
        <table>
            <tr>
                ...
                <td> 
                    ...
                    <img src='iconos/date_magnify.png' id='fecha_retro_btn'>
                </td>
            </tr>
            <tr>
                <td style='text-align: right;'>M&eacute;dico:</td>
                <td> 
                    <input type='text' id='rut_medico' name='rut_medico' size=10 style='text-align: center;' disabled>
            ...
            <tr>
                ...
                <td>
                    <input type='text' id='centro_costo' name='centro_costo' size=35 style='text-align: center;'>
                    <input type='hidden' id='centro_servicio' name='centro_servicio' value="-1">
                </td>
            </tr>
            <tr>
                <td style='text-align:right;'>
                    <input type='checkbox' id='prov_alta' name='prov_alta' />
                </td>
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    ...
                    <input type='hidden' id='adq_rut_correcto' name='adq_rut_correcto' value=0>
                </td>
            </tr>
            <tr>
    ...
    ?>
    <div class='sub-content'>
        <input type='hidden' id='paciente' name='paciente' value='<?php echo $paciente?>'>
        <input type='hidden' id='medica' name='medica' value=''>
        <input type='hidden' id='observacion_m' name='observacion_m' value=''>
        ...
        <input type='hidden' id='articulo_lotes' name='articulo_lotes' value=''>
        ...
        <table style='width:100%;'>
            <tr>
                <td style='text-align: right;width:20%;'>Cod. Medicamento:</td>
                <td colspan=2>
            ...
            <tr>
                ...
                <td>
                    <div id='cant_normal'>
                        <input type='text' id='cant' name='cant' size=3  style='text-align: right;' onKeyUp='calcular_cant();if(event.which==13) $("horas").focus();'>
                            <select id='fraccion' name='fraccion' onChange='calcular_cant();'>
                            ...
                            cada 
                            <input type='text' id='horas' name='horas' size=3  style='text-align: right;' onKeyUp='calcular_cant();if(event.which==13) $("dias").focus();'>
                            <select id='rangodosis2' onChange='calcular_cant();'>
                    ...
                    <div id='cant_insulina' style='display:none;'>
                        <table>
                            <tr>
                                <td>
                                    <input type='text' id='cant1' name='cant1' size=3  style='text-align: right;' onKeyUp='calcular_cant();'>
                                </td>
                                <td>Ma&ntilde;ana</td>
                            </tr>
                            <tr>
                                <td>
                                    <input type='text' id='cant2' name='cant2' size=3  style='text-align: right;' onKeyUp='calcular_cant();'>
                                </td>
                                <td>Tarde</td>
                            </tr>
                            <tr>
                                <td>
                                    <input type='text' id='cant3' name='cant3' size=3  style='text-align: right;' onKeyUp='calcular_cant();'>
                                </td>
                                <td>Noche</td>
                            </tr>
                        </table>
                ...
                <td>
                    por
                    <!--<input type='text' id='dias' name='dias' size=3  style='text-align: right;' onKeyUp='calcular_cant();if(event.which==13) agregar_art();'>-->
                    <input type='text' id='dias' name='dias' size=3  style='text-align: right;' onKeyUp='calcular_cant();if(event.which==13) $("txt_observacion").focus();'>
            ...
            <tr>
                ...
                <td id='detalle_prod' colspan=2></td>
            </tr>
            <tr>
                <td style='text-align: right;'>Total:</td>
                <td style='font-weight: bold;' colspan=2>
                    <span id='total'>0</span>
                    <span id='unidad_ua'>U.A.</span>
                </td>
            </tr>
            <tr>
		<td style='text-align: right;'>Indicaciones:</td>
		<td colspan=4>
                    <input type='text' name='txt_observacion' id='txt_observacion' size='40' onKeyUp='if(event.which==13) agregar_art();'>
		</td>
      </tr>  
            <tr>
                <td style='display:none;text-align:center;font-size:14px;' id='art_despacho' colspan=3></td>
            </tr>
            <tr>
                <td colspan=3>
                    <div class='sub-content2' id='lista_medicamentos' name='lista_medicamentos' style='height: 120px; min-height: 120px; overflow: auto;'>
                    </div>
                </td>
            </tr>
            <tr>
                <td colspan=3>
                    <table>
                            ...
                            <td colspan='3'>
                                <center>
                                    <span id='despaprimera' style='display:none;'>
                                        <input type='checkbox' id='despachar' name='despachar' CHECKED onChange='check_articulo();'>
                        ...
                        <tr>
                            ...
                            <td colspan=3>
                                <input type='text' id='nomd_diag_cod' name='nomd_diag_cod' value='<?php echo $ndet['nomd_diag_cod']; ?>' DISABLED size=5 style='font-weight:bold;text-align:center;' />
                                <input type='text' id='nomd_diagnostico' value='<?php echo $ndet['nomd_diag']; ?>' name='nomd_diagnostico' size=30 onDblClick='$("nomd_diag_cod").value=""; $("nomd_diagnostico").value="";' onKeyUp='if(event.which==13) $("num_serie").focus();'/>
                            </td>
                        </tr>
                        <tr>
                            <td style='text-align: right;'>Nros de Serie:</td>
                            <td>
                        ...
                        <tr>
                            ...
                            <td colspan=2>
                                <input type='text' id='observaciones' name='observaciones' size=40>
                            </td>
                        </tr>
                    </table>
                </td>
                        ...
                        <div class='boton'>
                            <table>
                                <tr>
                                    ...
                                    <td>
                                        <a href='#' onClick='limpiar_art();'>Limpiar Selecci&oacute;n...</a>
                                    </td>
                                </tr>
                            </table>
                        </div>
...
<script> 
   ...
   <?php
    ...
    {
    ?>
        cargar_ultima_receta(<?php echo $_GET['receta_id']; ?>);
    <?php
    }
    ?>

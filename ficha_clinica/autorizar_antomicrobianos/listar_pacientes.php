<?php 
	...
	if($autf_id==102 OR $autf_id==98) exit('DATOS PROTEGIDOS.');
	
	$lista=cargar_registros_obj("
		SELECT *,
		(CASE WHEN (autfp_vigente AND autfp_fecha_inicio<=CURRENT_DATE AND autfp_fecha_final>CURRENT_DATE) THEN true ELSE false END) AS vigente
	...
	for($i=0;$i<sizeof($lista);$i++) {
		...
		if($aut['autf_validar']=='t') {
			if($lista[$i]['doc_nombres']!='')
				print("<td style='color:green;text-align:center;'>".$lista[$i]['doc_nombres']."</td>");
			else
				print("<td style='color:red;text-align:center;'><i>(Pendiente...)</i></td>");
		...
		print("
			...
			<td><center><img src='iconos/delete.png' onClick='eliminar_paciente(".$lista[$i]['pac_id'].");' /></center></td>
			</tr>
		");

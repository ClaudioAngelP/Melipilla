<?php
    ...
    $sexohtml = desplegar_opciones("sexo", "sex_id, sex_desc",'','true','ORDER BY sex_id'); 
    $estcivhtml = desplegar_opciones("estado_civil", "estciv_id, estciv_nombre",'0','true', 'ORDER BY estciv_id');
    $nacionhtml = desplegar_opciones("nacionalidad", "nacion_id, nacion_nombre",'0','true', 'ORDER BY nacion_id');
    $previsionhtml = desplegar_opciones("prevision", "prev_id, prev_desc",'0','true','ORDER BY prev_id'); 
    $sangrehtml = desplegar_opciones("grupo_sanguineo", "sang_id, sang_desc",'0','true','ORDER BY sang_id'); 
    $grupohtml = desplegar_opciones("grupos_etnicos", "getn_id, getn_desc",'0','true','ORDER BY getn_id'); 
    $institucionhtml = desplegar_opciones("institucion_solicita", "instsol_id, instsol_desc",'','true','ORDER BY instsol_desc');
    $comunahtml = desplegar_opciones("comunas", "ciud_id, ciud_desc",'','true','ORDER BY ciud_desc');
    $bodegashtml = desplegar_opciones("bodega", "bod_id, bod_glosa",'1','bod_id IN ('._cav(10),')','ORDER BY bod_glosa'); 
?>
<script>
    var win='';
    //--------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    editar_receta = function(receta_id,tipo_talonario,receta_bod_id)
    {
        var params='';
	$('bodega_id').value=(receta_bod_id*1);
	if((tipo_talonario*1)==0){
		params='';

	} else {
		params='&cheque';
	}
        var win = new Window("win_receta", {className: "alphacube", top:30, left:0, 
        width: 850, height: 650, 
        title: '<img src="iconos/pencil.png"> Editar Receta',
        minWidth: 500, minHeight: 500,
        maximizable: false, minimizable: false,
        wiredDrag: true });
        ...
        {
            method: 'get', 
            parameters: 'directo&paciente='+($('paciente_id').value*1)+'&receta_id='+receta_id+params,
            evalScripts: true
	});
        $('win_receta').win_obj = win;
        win.setDestroyOnClose();
	...
	if(isDate($('paciente_fecha').value))
        {
            $('mostrar_edad').innerHTML='<i><b>('+calc_edad($('paciente_fecha').value)+')</b></i>';
    ...
    buscar_paciente = function()
    {
        ...
        var myAjax = new Ajax.Request('registro.php', 
        {
            ...
            onComplete: function (pedido_datos)
            {
                ...
                {
                    if(pedido_datos.responseText=='error|1')
                    {
                        alert("RUT DEL PACIENTE NO ESTA REGISTRADO EN FONASA");
                        ingresar_paciente();
                    }
                    else
                    {
                        if(pedido_datos.responseText=='error|2')
                        {
                            alert("RUT DEL PACIENTE PRESENTA PROBLEMAS AL RESCATAR DE FONASA");
                            ingresar_paciente();
                        }
                        else
                        {
                            if(pedido_datos.responseText=='error|3')
                            {
                                alert("RUT DEL PACIENTE PRESENTA BLOQUEO S/COTIZ. AL DIA");
                                ingresar_paciente();
                            }
                            else
                            {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    revisar_altas=function()
    {
        var myAjax=new Ajax.Request('ficha_clinica/altas_recientes.php',
        {
            method:'post',
            parameters:$('paciente_id').serialize(),
    ...
    ver_altas=function()
    {
        altas = window.open('ficha_clinica/ver_altas.php?pac_id='+($('paciente_id').value*1),
	'altas_receta', 'left='+((screen.width/2)-325)+',top='+((screen.height/2)-200)+',width=650,height=400,status=0,scrollbars=1');
	altas.focus();
    }
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    ...
    ver_stock=function()
    {
        sigges = window.open('ficha_clinica/ficha_basica/form_consulta_stock.php',
        'sigges', 'left='+((screen.width/2)-325)+',top='+((screen.height/2)-200)+',width=750,height=400,status=0,scrollbars=1');
        sigges.focus();
    }
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    ver_receta=function()
    {
        sigges = window.open('ficha_clinica/ficha_basica/form_consulta_receta.php','sigges', 'left='+((screen.width/2)-325)+',top='+((screen.height/2)-200)+',width=750,height=400,status=0,scrollbars=1');
        sigges.focus();
    }
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    ...
    actualizar_prevision = function()
    {
        ...
        var myAjax3 = new Ajax.Request('fonasa_conector.php', 
        {
            method: 'get', 
	...
	param+='&farma='+encodeURIComponent($('bodega_id').value)
    ...
    {
        var pat=trim($('pacpat_valid').value)
	if(pat=='') return;
    ...
    listar_parientes = function(id_paciente, modo)
    {
        if(modo==0)
        {
            var myAjax = new Ajax.Updater('parentezco_div',
    ...
    ver_pariente = function()
    {
        ...
        var myAjax = new Ajax.Request('ficha_clinica/parientes.php', 
        {
            method: 'get', 
	...
	$('paciente_tramo').disabled=false;
        $('paciente_editar').style.display='none';
	var myAjax = new Ajax.Request('ficha_clinica/ficha_basica/sql.php', 
        {
            method: 'get', 
    ...
    {
        $('tab_recetas_content').innerHTML='<br><br><img src="imagenes/ajax-loader2.gif"><br>Cargando Recetas...';
        var myAjax0 = new Ajax.Updater('tab_recetas_content', 
    ...
    {
        $('tab_historial_content').innerHTML='<br><br><img src="imagenes/ajax-loader2.gif"><br>Cargando Historial de Recetas...';
        var myAjax0 = new Ajax.Updater('tab_historial_content', 
    ...
    cerrar_receta = function(receta_id)
    {
        ...
        <table style="width: 750px;">
            <tr>
                <td>
                        ...
                        <table style="width:100%;">
                            <tr>
                                ...
                                <td style="text-align:right;width:400px;">
                                    Acciones:
                                    <input type='button' value='Consulta Saldos' onClick='ver_stock();' />
                                    <input type='button' value='Consulta Recetas' onClick='ver_receta();' />
                                    <input type="button" value="Ingresar Paciente..." onClick="ingresar_paciente_nuevo();">
                                </td>
                    ...
                    <div class='sub-content'>
                        <div class='sub-content'>
                            <img src='iconos/user_red.png'>
                            <b><span id='titulo_form'>B&uacute;squeda/Ingreso de Pacientes</span></b>
                            <input type='button' id='sigges' name='sigges' value='-- SIGGES --' style='display:none;' onClick='ver_sigges();' />
                            <input type='button' id='altas' name='altas' value='-- Paciente con Altas Recientes... --' style='display:none;' onClick='ver_altas();' /> 
                        </div>
                        <div class='sub-content'>
                            <table>
                                <tr style='height:22px; font-weight: bold;'>
                                    <td style='text-align:left;' colspan=3>
                                        <table border=0 cellpadding=0 cellspacing=0>
                                            <tr>
                                                <td width=100>
                                                    <input type='text' id='paciente_rut' name='paciente_rut' size=11 style='text-align: center;font-size:13px;' onKeyUp='if(event.which==13) { this.value=this.value.toUpperCase();verificar_rut(); }' maxlength=11>
                                                </td>
                                                ...
                                                <td>
                                                    <img src='iconos/zoom_in.png' id='buscar_paciente' onClick='busqueda_pacientes("paciente_rut", function() { verificar_rut(); });' onKeyUp="fix_bar(this);" alt='Buscar Paciente...' title='Buscar Paciente...'>
                                ...
                                <tr>
                                    <td><input type='text' id='paciente_nombre' name='paciente_nombre' size='22' onKeyUp='if(event.which==8 && this.value.length==0) $("paciente_rut").focus();' maxlength=100></td>
                                    <td><input type='text' id='paciente_paterno' name='paciente_paterno' size='22' onKeyUp='if(event.which==8 && this.value.length==0) $("paciente_nombre").focus();' maxlength=50></td>
                                    <td><input type='text' id='paciente_materno' name='paciente_materno' size='22' onKeyUp='if(event.which==8 && this.value.length==0) $("paciente_paterno").focus();' maxlength=50></td>
                                </tr>
                            </table>
                        </div>
                        <div class='sub-content' id='ajax_fonasa' style='display: none;'>
                            <center>
                                <table>
                                    <tr>
                                        <td><img src='imagenes/ajax-loader3.gif'></td>
                                        <td><b>Conectando con Fonasa...</b><br></td>
                                    </tr>
                                </table>
                            </center>
                        </div>
                        <table cellpadding=0 cellspacing=0>
                            <tr>
                                <td>
                                    <div class='tabs' id='tab_ficha' style='cursor: default;' onClick='ver_ficha();'>
                                        <img src='iconos/report_user.png'>Ficha B&aacute;sica
                                    </div>
                                </td>
                                <td>
                                    <div class='tabs_fade' id='tab_refs' style='cursor: pointer;' onClick='ver_refs();'>
                                        <img src='iconos/group.png'>Referencias
                                    </div>
                                </td>
                                <td>
                                    <div class='tabs_fade' id='tab_recetas' style='cursor: pointer;' onClick='ver_recetas();'>
                                        <img src='iconos/pill_go.png'>Recetas Vigentes
                                    </div>
                                </td>
                                <td>
                                    <div class='tabs_fade' id='tab_historial' style='cursor: pointer;' onClick='ver_historial();'>
                                        <img src='iconos/pill.png'>Historial Recetas
                                    </div>
                                </td>
                            </tr>
                        </table>
                            ...
                            <center>
                                ...
                                <table width=100%>
                                    ...
                                    <tr style='text-align: center;'>
                                        ...
                                        <td>
                                            <input type='text' name='paciente_sector' id='paciente_sector' size=24 maxlength=80>
                                        </td>
                                        <td>
                                            <input type='text' name='paciente_fono' id='paciente_fono' size=15 maxlength=60>
                                        </td>
                                    </tr>
                                    <tr style='text-align: center;'>
                                        <td>Comuna:</td>
                                    ...
                                    <tr style='text-align: center;'>
                                        ...
                                        <td style='text-align: center;'>
                                            <input type='text' id='paciente_fecha' name='paciente_fecha' style='text-align: center;' onBlur='calcular_edad();'>
                                            <br>
                                            <span name='mostrar_edad' id='mostrar_edad'></span>
                                        </td>
                                        <td width=200 style='text-align: center;'>
                                            <input type='text' size='2' id='paciente_cod_prev' name='paciente_cod_prev' style='text-align: center;display:none;' value=0>
                                ...
                                <!--<input type='text' size='2' id='paciente_cod_prev' name='paciente_cod_prev' style='text-align: center;display:none;' value=0>
                                <select id='paciente_prevision' name='paciente_prevision' onClick='actualizar_cod_prev();' >
                                    <?php //echo $previsionhtml; ?>
                                </select>-->
                                <input type='text' size='1' maxlength=1 id='paciente_tramo' name='paciente_tramo' style='text-align: center;display:none;'>
                        ...
                        <div class='tabbed_content' id='tab_refs_content' style='display: none;'>
                            <table>
                                <tr>
                                    ...
                                    <td><input type='text' size=70 maxlength=200 id='paciente_padre' name='paciente_padre'></td>
                                </tr>
                                <tr>
                                    <td style='text-align: right;'>Nombre de la Madre:</td>
                                    <td>
                                        <input type='text' size=70 maxlength=200 id='paciente_madre' name='paciente_madre'>
                                    </td>
                                </tr>
                            </table>
                            <div class='sub-content'>
                                ...
                                <div class='sub-content2' id='parentezco_div' name='parentezco_div' style='min-height: 100px; height: 100px; overflow: auto;'>
                                </div>
                            </div>
                        </div>
                        <div class='tabbed_content' id='tab_recetas_content' style='height:250px; overflow:auto; display: none;'>
                            (No ha seleccionado un paciente...)
                        </div>
                        <div class='tabbed_content' id='tab_historial_content' style='height:250px; overflow:auto; display: none;'>
                            (No ha seleccionado un paciente...)
                        </div>
                                            ...
                                            <div class='boton'>
                                                <table>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href='#' onClick='despachar_receta($("paciente_id").value, false);'>Desp. Receta...</a>
                                                        </td>
                                                    </tr>
                                                </table>
                                            </div>
                                            ...
                                            <div class='boton'>
                                                <table>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href='#' onClick='despachar_receta($("paciente_id").value, true);'>Desp. Receta Controlados...</a>
                                                        </td>
                                                    </tr>
                                                </table>
                                            </div>

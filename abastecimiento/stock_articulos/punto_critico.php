  ...
  ');
    ...
    if( $i==0 OR $comp) {
      ...
      ($i%2==0) ? 	$clase='tabla_fila' : $clase='tabla_fila2';
			...
			if($art[7]>0) {
			   $cantidad = (($art[7]-$art[6])-$art[9])-$art[14]+$art[15];
			} else {
    ...
    //    for($j=0;$j<=count($pedidos);$j++)
     //   {
            //$resultado=pg_query("Select date_trunc('days', current_date -(Select pedido_fecha from pedido where pedido_nro='$pedidos[$j]'))");
     ...
     //      $resultado=pg_query("Select pedido_fecha from pedido where pedido_nro='$pedidos[$j]'");
      ...
      //      $dias=abs($dias);
       ...
       //     if($dias>=15)
       //     {
      //          $ultpedido='<span style="font-size:10px;cursor:pointer;color:red;"
      //          onClick="abrir_pedido('.$pedidos[$j].');">
      //          ['.$pedidos[$j].']</span>';
      //      }
      //      else
       //     {
       //         $ultpedido='<span style="font-size:10px;cursor:pointer;color:blue;"
        //        onClick="abrir_pedido('.$pedidos[$j].');">
       //         ['.$pedidos[$j].']</span>';
       //     }
     ...
     // } //else {
      ...
      if(trim($art[10])!='{}')
      {
        ...
        $pedidos=pg_array_parse($art[10]);
		  ...
		  //print("<b>$pedidos: ".sizeof($pedidos)." (".$pedidos[0].")</b>");
			
		  if($pedidos[0]!='')
        for($j=0;$j<sizeof($pedidos);$j++)
        {
        	
        		$pedido_nro=$pedidos[$j];
        		
            $resultado=pg_query("SELECT pedido_fecha FROM pedido WHERE pedido_nro=$pedido_nro");
            if(pg_num_rows($resultado)!=0)
            {
                $var = pg_fetch_row($resultado);
                $fecha=$var[0];

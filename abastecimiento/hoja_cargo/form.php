...
<script>
	...
	if($('art_kit').value=='1')
        {
            var myAjax=new Ajax.Request('abastecimiento/hoja_cargo/cargar_kit.php',
            {
                method:'post',
		...
		onComplete:function(r)
                {
                    try 
                    {
                        ...
                        for(var d=0;d<det.length;d++)
                        {
                            ...
                            {
                                if(articulos[i].id==det[d].id)
                                {
                                    articulos[i].cantidad+=det[d].cantidad*1;
                                    fnd=true;
                                    break;
                                }
                            }
                        ...
                        $('art_id').value='';
                        $('art_cant').value='';
                        $('art_stock').innerHTML='0.-';
                        $('art_kit').value='';
                        $('art_codigo').value='';
                        $('art_glosa').value='';
                        $('art_nombre').innerHTML='(Seleccione Articulos...)';
                        $('codigo').value='';
                        $('codigo').select();
                        $('codigo').focus();
                    }
        ...
        num=-1;
        //Si el articulo ya fué ingresado se agrega.
        for(var i=0;i<articulos.length;i++)
        {
            var fnd=false;
            if(articulos[i].id==$('art_id').value)
            {
                articulos[i].cantidad+=$('cantidad').value*1;
                $('art_id').value='';
                $('art_cant').value='';
                $('art_stock').innerHTML='0.-';
                $('art_kit').value='';
    ...
    //*******************************************************************
    pistolear_articulo = function ()
    {
        var myAjax=new Ajax.Request('abastecimiento/hoja_cargo/cargar_codigo.php',
        {
            method:'post',
            parameters:$('barras').serialize()+'&'+$('bodega_id').serialize(),
            onComplete:function(r)
            {
                ...
                for(var n=0;n<datos.length;n++)
                {
                    ...
                    for(var i=0;i<articulos.length;i++)
                    {
                        if(articulos[i].id==datos[n].id)
                        {
                            articulos[i].cantidad+=datos[n].cantidad*1;
                            fnd=true;
                            break;
                        }
                    }
                    ...
                    num = articulos.length;
                    articulos[num] = new Object();
                    articulos[num].id=datos[n].id;
                    articulos[num].codigo=datos[n].codigo;
                    articulos[num].glosa=datos[n].glosa;
                    articulos[num].clasificacion=datos[n].clasificacion;
                    articulos[num].cantidad=datos[n].cantidad*1;
                    articulos[num].stock=datos[n].stock*1;
                }
                redibujar();
                if(limpiar)
                    $('barras').value='';
    ...
    redibujar = function()
    {
        ...
        for(var i=0;i<articulos.length;i++)
        {
            ...
            if(articulos[i].stock<articulos[i].cantidad)
            {
                color='red'; color2='white';
            }
            else
            {
                color='skyblue'; color2='black';
            }
            html+='<td style="text-align:center;"><input type="text" style="width:50px;text-align:right;font-size:14px;font-weight:bold;background-color:'+color+';color:'+color2+'" id="cant_'+i+'" name="cant_'+i+'" value="'+number_format(articulos[i].cantidad,0)+'" onKeyUp="guarda_vals('+i+');" /></td>';
            html+='<td><center>';
	...
	$('cant_'+i).style.background=color;
	$('cant_'+i).style.color=color2;
	//}
    }
    //*******************************************************************
...
//*******************************************************************
    ...
    verifica_tabla = function()
    {
        ...
        for(i=0;i<articulos.length;i++)
        {
            if(articulos[i].stock<articulos[i].cantidad)
            {
                alert('El articulo no disponde de saldo suficiente. Por favor, baje la cantidad o retirelo del listado.');
		$('cant_'+i).select();
		$('cant_'+i).focus();
		return;
            }
        ...
        var myAjax=new Ajax.Request('abastecimiento/hoja_cargo/sql.php',
        {
            method: 'post',
            parameters: params,
    ...
    {
        var myAjax=new Ajax.Updater('listado_movs','abastecimiento/hoja_cargo/listar_pacientes.php',
        {
          method:'post',
...
//*******************************************************************
    ...
    {
        tmp_pac_id=num;
        var myAjax=new Ajax.Updater('listado_movs','abastecimiento/hoja_cargo/listar_salidas.php',
        {
            method:'post',
            evalScripts:true,
    ...
    mostrar_detalle2=function(num)
    {
        ...
        var myAjax=new Ajax.Updater('listado_movs','abastecimiento/hoja_cargo/listar_salidas2.php',
        {
            method:'post',
            evalScripts:true,
    ...
    mantener_kits=function()
    {
        ...
        new_win = window.open('abastecimiento/hoja_cargo/form_kits.php?'+params,
    ...
    carpeta_codigos=function()
    {
        params='bod_id='+($('bodega_id').value*1);
        top=Math.round(screen.height/2)-300;
        left=Math.round(screen.width/2)-400;
        new_win = window.open('abastecimiento/hoja_cargo/carpeta_codigos.php?'+params,
        'win_talonarios', 'toolbar=no, location=no, directories=no, status=no, '+
        'menubar=no, scrollbars=yes, resizable=yes, width=800, height=600, '+
        'top='+top+', left='+left);
        new_win.focus();
    ...
    eliminar_cargo=function(stock_id)
    {
        if(!confirm('&iquest;Est&aacute; seguro que desea eliminar el cargo paciente? - NO HAY OPCIONES DE DESHACER.'.unescapeHTML()))
            return;
        var myAjax=new Ajax.Request('abastecimiento/hoja_cargo/sql_eliminar_cargo.php',
        {
            method:'post',
            parameters:'stock_id='+stock_id,
			onComplete:function(r) {
				mostrar_detalle(tmp_pac_id);
			}
        });
    }

    imprimir_salidas=function()
    {
        /*
        if ($j('bt_save_obs').length){
            $('bt_save_obs').style.display='none';
        }
        */
        _nom_pac='Rut: '+$('paciente_rut2').value+' // Nombre: '+$('nom_pac2').innerHTML;
        _div_movimientos = $('listado_movs').innerHTML;
        ...
        imprimirHTML(_nom_pac+'<br><br>'+_separador2+_div_movimientos);
        /*
        if ($j('bt_save_obs').length)
        {
            $('bt_save_obs').style.display='block';
        }
        */
    }
</script>
                                ...
                                <table style='width:100%;'>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <input type='button' id='kits' name='kits' value='-[[ Mantenedor de Kits ]]-' onClick='mantener_kits();' />  
                                            <input type='button' id='ccods' name='ccods' value='-[[ Carpeta C&oacute;digos ]]-' onClick='carpeta_codigos();' />  
                                        </td>
                                    </tr>
                                    <tr>
                                            ...
                                            <table>
                                                <tr>
                                                    <td>
                                                        ...
                                                        <input type='text' id='paciente_rut' name='paciente_rut' style="font-size:10px;" value='' size=15 onKeyPress='if(event.which==13) buscar_paciente();'>
                                                        <input type='hidden' id='pac_id' name='pac_id' value=''>
                                                        <img src='iconos/zoom_in.png' style='cursor:pointer;' onClick='busqueda_pacientes("paciente_rut",function() { buscar_paciente(); });'>
                                                        <img src='iconos/cross.png' style='cursor:pointer;' onClick='$("pac_id").value="";$("paciente_rut").value="";$("nom_pac").innerHTML="";'>
                                                    </td>
                                                    <td style='width:250px;' id='nom_pac'></td>
                                                </tr>
                                            </table>
                                        </td>
                                    </tr>
                        ...
                        <div class="sub-content">
                            <center>
                                <table cellpadding=0 cellspacing=1 style='margin:0px;padding:0px;'>
                                    <tr>
                                        <td><img src='abastecimiento/hoja_cargo/barras.png' /></td>
                                        <td colspan=5>                         
                                            <input type='text' id='barras' name='barras' size=15 style='font-size:16px;text-align:center;' onKeyUp='if(event.which==13) pistolear_articulo();' onFocus='this.style.border="3px dashed red";this.select();' onBlur='this.style.border="";this.style.background="";' />
                                        </td>
                                    </tr>
                                    <tr>
                                        <td><img src='iconos/magnifier.png' style='width:32px;height:32px;' /></td>
                                        <td>
                                            ...
                                            <input type='text' id='codigo' name='codigo' size=15 style='font-size:16px;' /><br />
                                        </td>
                                        ...
                                        <td>
                                            <input type='text' id='cantidad' name='cantidad' style='text-align:right;' onKeyUp='if(event.which==13) agregar_articulo();' value='0' size=5>
                                        </td>
                                        <td>
                                            <input type='button' id='agregar_art' name='agregar_art' onClick='agregar_articulo();' value='Agregar Art&iacute;culo...'>
                                        </td>
                                    </tr>
                                </table>
                            </center>
                                            ...
                                            <table>
                                                <tr>
                                                    <td>
                                                        ...
                                                        <input type='text' id='paciente_rut2' name='paciente_rut2' style="font-size:10px;" value='' size=15 onKeyPress="if(event.which==13)mostrar_detalle2($('paciente_rut2').value);">
                                                        <input type='hidden' id='pac_id2' name='pac_id2' value=''>
                                                    </td>
                                                    <td>
                                                        <img src='iconos/zoom_in.png' style='cursor:pointer;' onClick='busqueda_pacientes("paciente_rut2", function() { buscar_paciente2(); });'>
                                                        <img src='iconos/cross.png' style='cursor:pointer;' onClick='$("pac_id2").value="";$("paciente_rut2").value="";$("nom_pac2").innerHTML="";'>
                                                    </td>
                                        ...
                                        <div class='boton'>
                                            <table>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href='#' onClick='imprimir_salidas();'> Imprimir Listado...</a>
                                                    </td>
                                                </tr>
                                            </table>
                                        </div>

<?php
  ...
  if($_GET['tipo']=='buscar_meds' or $_GET['tipo']=='buscar_meds_stock') {
      $condicion="AND (art_item='2204004001' OR art_item='2204005001' OR art_item='2204004003' OR art_id=6543) AND art_control=0";
    } else if ($_GET['tipo']=='buscar_meds_controlados' or $_GET['tipo']=='buscar_meds_controlados_stock') {
      $condicion="AND (art_item='2204004001' OR art_item='2204005001' OR art_item='2204004003' OR art_id=10619) AND (NOT art_control=0)";
    } else {
      $condicion='';
    }
...

    {
      ...
      if(isset($_GET['bodega_id'])) {
		...
		(
			...
			WHERE (kit_codigo ILIKE '%$cadena%' OR  kit_nombre ILIKE '%$cadena%') AND bod_id=$bod
		...
		(SELECT
			...
			$query.=", art_control, art_id, art_item, item_glosa, art_vence
  ...
  for($i=0; $i<pg_num_rows($listado); $i++) {
	...
	if($array[$i][3]=='KIT' AND $array[$i][7]=='KIT' AND $array[$i][8]=='KIT') {
		$array[$i][2]='<b>'.$array[$i][2].'</b>';
	}
 ...
 //*******************************************************************************
  ...
  if($_GET['tipo']=='carpeta') {
    ...
    cpat_id,
    cpat_nombre
    FROM control_patologias
    WHERE 
    cpat_nombre ILIKE '%$cadena%'
    AND cpat_tipo='$tipo';
    ");

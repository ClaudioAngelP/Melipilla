    ...
    <tr class='tabla_header'><td><b>C&oacute;digo Int.</b></td><td width=35%%><b>Glosa</b></td>
		...
		for($i=0;$i<pg_num_rows($listado);$i++) {
			...
			if(/*($fila[4]*1)==0 AND */($fila['critico_pedido']*1==0 AND $fila['critico_critico']*1==0)) {
				...
				if(($fila['critico_pedido']*1)==0) { $color1="yellow"; $icono='<img id="error_'.$fila['real_art_id'].'" src="iconos/exclamation.png">'; }
				else 				{ $color1='inherit'; $icono='<img id="error_'.$fila['real_art_id'].'" src="iconos/tick.png">'; }
				if(($fila['critico_critico']*1)==0) { $color2="yellow";	$icono='<img id="error_'.$fila['real_art_id'].'" src="iconos/exclamation.png">'; }
				else				{ $color2='inherit'; $icono='<img id="error_'.$fila['real_art_id'].'" src="iconos/tick.png">'; }

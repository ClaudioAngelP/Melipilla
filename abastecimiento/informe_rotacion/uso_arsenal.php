<?php 
    ...
    AND art_arsenal=true AND log_tipo IN (9,15,18) 
    AND stock_cant<0 
    --and stock_bod_id in ()
    ...
    $detalle= cargar_registros_obj("SELECT DISTINCT art_codigo,art_glosa,forma_nombre FROM articulo
    JOIN autorizacion_farmacos_detalle ON autf_id=1 AND autorizacion_farmacos_detalle.art_id=articulo.art_id
    LEFT JOIN stock ON stock_art_id=articulo.art_id
    LEFT JOIN logs ON log_id=stock_log_id
    LEFT JOIN bodega_forma ON forma_id=art_forma
    WHERE log_fecha BETWEEN '".$fecha1." 00:00:00' AND '".$fecha2." 23:59:59'
    AND log_tipo IN (9,15,18) 
    AND stock_cant<0 
    AND stock_bod_id in (35,36,24)
    AND art_activado
    ORDER BY art_glosa,forma_nombre
    ");
    ...
    $arsenal=pg_query("SELECT * FROM autorizacion_farmacos_detalle JOIN articulo USING (art_id)  WHERE autf_id=1 and art_activado");
	//print(sizeof($arsenal));
...
<table style='width:100%;'>
    <tr class='tabla_header'>
        <td colspan=4><center><b>Porcentaje de Uso del Arsenal Farmacol&oacute;gico</b></center></td>
    </tr>
    <tr>
...
<?php
    ...
    AND log_tipo IN (9,15,18)
    AND stock_cant<0) AND art_arsenal=true
    AND art_activado
    ORDER BY art_glosa,forma_nombre
    ");
     * 
    ...
    $nousados=cargar_registros_obj("
    SELECT art_codigo,art_glosa,forma_nombre, art_id FROM articulo
    LEFT JOIN bodega_forma ON forma_id=art_forma
    WHERE art_id NOT IN(
        SELECT DISTINCT art_id FROM articulo
        LEFT JOIN stock ON stock_art_id=art_id
        LEFT JOIN logs ON log_id=stock_log_id
        LEFT JOIN bodega_forma ON forma_id=art_forma
        WHERE log_fecha BETWEEN '".$fecha1." 00:00:00' AND '".$fecha2." 23:59:59'
        AND art_id IN (select art_id from autorizacion_farmacos_detalle WHERE autf_id=1) 
        AND log_tipo IN (9,15,18)  
        AND stock_bod_id in (35,36,24)
        AND stock_cant<0
    ) AND  art_id IN (select art_id from autorizacion_farmacos_detalle WHERE autf_id=1) 
    ...
    print("<tr class='tabla_header'>
        <td colspan=4>Detalle Art&iacute;culos NO Utilizados</td>
    </tr>
    <tr class='tabla_header'>
        <td>N &ordm;</td>
	<td>C&oacute;digo</td>

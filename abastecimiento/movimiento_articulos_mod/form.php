<?php
  require_once("../../conectar_db.php");
	$bodegashtml = desplegar_opciones("bodega", "bod_id, bod_glosa",'1','bod_id IN ('._cav(3),')',
	'ORDER BY bod_glosa'); 

  $servs="'".str_replace(',','\',\'',_cav2(3))."'";
        
	$servicioshtml = desplegar_opciones_sql("SELECT centro_ruta, centro_nombre FROM centro_costo WHERE (length(regexp_replace(centro_ruta, '[^.]', '', 'g'))=3 OR length(regexp_replace(centro_ruta, '[^.]', '', 'g'))=4) AND
        centro_medica AND centro_ruta IN (".$servs.") ORDER BY centro_nombre", NULL, '', "font-style:italic;color:#555555;"); 


	
	$bodegas2html = desplegar_opciones("bodega", "bod_id, bod_glosa",'1','1=1',
	'ORDER BY bod_glosa'); 
	...
	"centro_ruta, centro_nombre",'',"(length(regexp_replace(centro_ruta, '[^.]', '', 'g'))=2 or length(regexp_replace(centro_ruta, '[^.]', '', 'g'))=3)",'ORDER BY centro_ruta');
	
	$lista_servicios=cargar_registros_obj("
        SELECT centro_ruta, centro_nombre FROM centro_costo
        WHERE (length(regexp_replace(centro_ruta, '[^.]', '', 'g'))=3 or length(regexp_replace(centro_ruta, '[^.]', '', 'g'))=4)
        ",true);
...
?>
    ...
    descargar_pedido = function() {
      ...
      if(!bloquear_descarga) {
        ...
        'abastecimiento/movimiento_articulos/bajar_pedidos.php',
        {
          method: 'get',
	...
	var myAjax = new Ajax.Request(
			'abastecimiento/movimiento_articulos/sql.php', 
			{
				method: 'post', 
    ...
    win.show();
      ...
      if($('bodega_origen').value!=1){
		$('boton_agregar').style.display='';
	}else{$('boton_agregar').style.display='none';}
      $('boton_agregar_venc').style.display='none';
      $('boton_agregar_sacar').style.display='none';
      $('boton_vencidos').style.display='none';
      $('boton_balance').style.display='none';

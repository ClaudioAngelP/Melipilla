<?php 
	...
	if($tipo==1)
		$filtro='(docp_tipo_pago IS NULL OR docp_tipo_pago=0)';
	else if($tipo==2)
		$filtro='NOT (docp_tipo_pago IS NULL OR docp_tipo_pago=0)';
	else
		$filtro='true';
	...
	$d=cargar_registros_obj("SELECT documento.doc_id AS real_doc_id, *, 
	...
	WHERE docp_autorizado=1 AND $filtro AND $prov_w
...
<?php 
	...
	$frmpago=array('(Pendiente...)','Cheque','Transferencia','Pago Prov.');
	...
	for($i=0;$i<sizeof($d);$i++) {
		...
		if($d[$i]['doc_tipo']=='1' OR $d[$i]['docp_nro_factura']*1>0) {
			...
			$ing_factura=" <img src='iconos/magnifier.png' onClick='abrir_recepcion(".$d[$i]['real_doc_id'].");' style='cursor:pointer;width:12px;height:12px;' />";
			$color2='blue';
			...
			$ing_factura="<img src='iconos/magnifier.png' onClick='abrir_recep(".$d[$i]['real_doc_id'].");' style='cursor:pointer;width:12px;height:12px;' /> <img src='iconos/pencil.png' style='cursor:pointer;width:12px;height:12px;' onClick='asociar_factura(".$d[$i]['real_doc_id'].");' />";
			$color2='red';
		...
		for($j=0;$j<sizeof($frmpago);$j++) {
			...
			if($d[$i]['docp_tipo_pago']*1==$j) $sel='SELECTED'; else $sel='';
		...
		<td style='text-align:right;font-weight:bold;'>$ ".number_format($d[$i]['subtotal'],0,',','.').".-</td>
		<td style='text-align:center;font-weight:bold;'><center>
		<input type='text' size=8 id='fecpago_".$d[$i]['real_doc_id']."' name='fecpago_".$d[$i]['real_doc_id']."' value='".substr($d[$i]['fecha_pago'],0,10)."' style='text-align:center;' onChange='ver_guardar(".$d[$i]['real_doc_id'].");' />
		</center></td>
		<td style='text-align:center;font-weight:bold;'><center>
		...
		<td style='text-align:center;font-weight:bold;'><center>
		<input type='text' size=7 id='numpago_".$d[$i]['real_doc_id']."' name='numpago_".$d[$i]['real_doc_id']."' value='".$d[$i]['docp_numero']."' style='text-align:right;'  onChange='ver_guardar(".$d[$i]['real_doc_id'].");' />
		</center></td>
		<td style='text-align:center;font-weight:bold;'>".$d[$i]['dias']."</td>
		<td><center><img src='iconos/disk.png' id='guardar_".$d[$i]['real_doc_id']."' style='cursor:pointer;display:none;' onClick='guardar_pago(".$d[$i]['real_doc_id'].");' /> <img src='iconos/$icono' id='icono_".$d[$i]['real_doc_id']."' /> </center></td>
		</tr>
		");

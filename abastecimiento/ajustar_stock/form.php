<?php
    ...
    $bodegashtml = desplegar_opciones("bodega", "bod_id, bod_glosa",'1','bod_id IN ('._cav(24),')', 'ORDER BY bod_glosa'); 
?>
<script>
    ...
    {
        var myAjax = new Ajax.Request('abastecimiento/ajustar_stock/cargar_lotes.php',
        {
            method: 'get',
            parameters: 'art_id='+articulos[num].id+'&'+$('bodega_id').serialize(),
    ...
    redibujar = function()
    {
        ...
        html+='<td>Lote</td><td>Cant. Actual</td><td colspan=2>Cant. Real</td>';
        html+='<td colspan=2>Acciones</td></tr>';
        //for(var i=0;i<articulos.length;i++) {
        for(var i=articulos.length-1;i>=0;i--)
        {
            //ultimo en agregar es primero de la lista
            temp_html='';
            temp_html2='';
    ...
    ingresar_ajuste = function () {
      ...
      if($('carga_masiva').value=='-- Generar Ajuste Manual... --') {
		  
		  window.open('', 'ajuste', 'width=600, height=450, left=100, top=100, scrollbars=yes');
		  
		  $('datos_ajuste').target='ajuste';
		  $('datos_ajuste').submit();
      ...
      function() {
        ...
        return {
          ...
          modo_ajuste=function() {
		  
		  if($('carga_masiva').value=='-- Generar Ajuste Masivo... --') {
		  
		  $('ajuste_manual').hide();
		  $('ajuste_masivo').show();
		  
		  $('carga_masiva').value='-- Generar Ajuste Manual... --';
		  ...
		  $('carga_masiva').value='-- Generar Ajuste Masivo... --';
...
<form id='datos_ajuste' name='datos_ajuste' 
enctype="multipart/form-data" method='post' action='abastecimiento/ajustar_stock/form_carga_masiva.php'
...
<input type='button' id='carga_masiva' name='carga_masiva' value='-- Generar Ajuste Masivo... --' onClick='modo_ajuste();' />
...
<tr id='ajuste_masivo' style='display:none;'>
<td style='text-align:right;'>Planilla Ajustes:</td>
<td>
<input type='file' id='archivo_carga' name='archivo_carga' />
</td>
</tr>
<tr>

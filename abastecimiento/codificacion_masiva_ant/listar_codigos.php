<?php 
    ...
    if(isset($_POST['txt_filtro']))
    {
        if($_POST['txt_filtro']!="")
        {
            $strin_filtro="and orserv_glosa ILIKE '%".$_POST['txt_filtro']."%'";
        }
    }
    if($filtro==1)
    {
...
<table style="width:100%">
    <tr class='tabla_header'>
        <td>#</td>
        <td>Descripci&oacute;n Art&iacute;culo</td>
    ...
    if($d)
        for($i=0;$i<sizeof($d);$i++)
        {
            ...
            print("<tr class='$clase' onMouseOver='this.className=\"mouse_over\";' onMouseOut='this.className=\"$clase\";'>");
                ...
                print("<td class='ajustar'>".$glosa."</td>");
                print("<td style='text-align:right;'>".$d[$i]['cuenta']."</td>");
                print("<td style='text-align:center;'>");
                    print("<input type='hidden' id='art_id_$i' name='art_id_$i' value='".$d[$i]['art_id']."' />");
                    print("<input type='text' id='codigo_art_$i' name='codigo_art_$i' size=15 style='font-size:10px;' value='".$d[$i]['art_codigo']."' />");
                print("</td>");
                print("<td style='text-align:justify;color:green;' id='glosa_$i'>".$d[$i]['art_glosa']."</td>");
                print("<td><center><input type='button' style='display:none;' id='guardar_$i' value='[MOD]' onClick='$accion' /></center></td>");
            print("<tr>");

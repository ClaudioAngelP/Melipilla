<?php
	...
	if(isset($_GET['pds_propios']))
        ...
        if($art_id!=0)
        {
            $join_pedido_detalle="left join pedido_detalle on pedido.pedido_id=pedido_detalle.pedido_id";
            $where_art=" where art_id=".$art_id." ";
        }
        else
        {
            $join_pedido_detalle="";
    ...
    $consulta="SELECT distinct * FROM(
		SELECT DISTINCT pedido.pedido_id, pedido_nro, date_trunc('second', pedido_fecha) AS pedido_fecha, 
    ...
    $pedidos=cargar_registros_obj($consulta,true);
    ...
    if($pedidos)
    {
       print('
        ...
        for($i=0;$i<count($pedidos);$i++)
        {
			...
			case 4: $pedidos[$i]['pedido_estado']='No Procesado'; break;
                    }
                }
                else
                {

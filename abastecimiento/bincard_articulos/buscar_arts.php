<?php
  ...
  if(isset($_GET['codigo'])) {
    ...
    $buscar = $_GET['control']*1;
  
    $arts = pg_query("
    SELECT art_id, art_codigo, art_glosa, forma_nombre 
    FROM articulo LEFT JOIN bodega_forma ON forma_id=art_forma
    WHERE art_control=".$buscar." AND art_activado ORDER BY art_codigo
    ");
  ...
  for($i=0;$i<$numarts;$i++) {
    ...
    print("
    <tr class='$clase' id='art_fila_".$_arts[0]."'
    ...
    <tr class='$clase'  id='art_fila_".$_arts[0]."___2'
    onClick='seleccionar_art(".$_arts[0].", \"".$_arts[1]."\");' 
    style='cursor:pointer;'>

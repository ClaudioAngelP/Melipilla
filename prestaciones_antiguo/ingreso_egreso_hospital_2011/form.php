<?php 
	...
	$r=cargar_registro("SELECT COALESCE(MAX(hosp_folio),0) AS n FROM hospitalizacion where  EXTRACT(YEAR FROM hosp_fecha_egr) ='2011' AND hosp_folio>=1;");
...
<script>
	...
	cargar_hosp=function() {
		...
		var myAjax=new Ajax.Request(
			'prestaciones/ingreso_egreso_hospital_2011/info_hospitalizacion.php',
			{
				method:'post',
				parameters:$('nro_folio').serialize()+'&'+$('anio').serialize(),
				onComplete:function(r) {
					try {
						...
						if(reg) {
							...
							//$('paciente_tipo_id').value=2; Editado por Ricardo Corominas
							if(reg.pac_rut=='')
							{
    ...
    ver_camas = function() {
	 ...
	 guardar_hosp = function() {
		...
		if($('disponible').value=='0') {
			alert( 'Debe seleccionar un n&uacute;mero de cama v&aacute;lido.'.unescapeHTML() );
			return;
		}	 
		...
		*/ 
		...
		var myAjax=new Ajax.Request(
			'prestaciones/ingreso_egreso_hospital_2011/sql_hosp.php',
			{
				method:'post',
				parameters: $('nro_folio').serialize()+'&'+$('paciente').serialize()+
								'&'+$('hosp').serialize()+'&'+serializar_arr('diagnosticos,diagnosticos_egreso,prestaciones,traslados_int,listado_partos'),
				onComplete: function(r) {
...
<option value='2'>No tiene (Cancela)</option>
<option value='3'>Cajas de Prevision FFAA</option>
<option value='4'>CAPREDENA</option>
<option value='5'>DIPRECA</option>
...
<option value='4'>Ley 19.650/99 de Urgencia</option>
<option value='5'>PRAIS</option>
<option value='6'>Chile Solidario</option>
<option value='7'>Chile Crece Contigo</option>
<option value='8'>Otro Programa Social</option>
</select>
</td></tr>
...
<option value='1' selected="selected">Vivo</option>
...
<div class='sub-content'>
	<table style='width:100%;'>
		<tr>
			...
			<td style='width:35%;'>
				<b>Datos del Reci&eacute;n Nacido</b>
			</td>
			<td style='text-align:left;'>
				Agregar R.N.:
			</td>
			<td style='text-align:right;'>
				Condici&oacute;n al Nacer:
			</td><td>
				<select id='parto_cond' name='parto_cond'>
                <option value='-1) No aplica...' selected="selected">(No aplica...)</option>
				<option value='0) ABORTO'>Aborto</option>
                <option value='1) VIVO'>Vivo</option>
				<option value='2) FALLECIDO'>Fallecido</option>
				</select>			
			</td>
			<td style='text-align:right;'>
				Sexo:
			</td><td>
				<select id='parto_sexo' name='parto_sexo'>
                <option value='0) No aplica...' selected="selected">(No aplica...)</option>
				<option value='1) MASC.'>Masculino</option>
				<option value='2) FEM.'>Femenino</option>
				<option value='3) IND.'>Indefinido</option>
				</select>			
			</td>
			...
			<td style='text-align:right;'>
				...
				<input type="text" id='parto_peso' name='parto_peso' size=5 style='text-align:center;'>
			</td>

			<td style='text-align:right;'>
				APGAR 5m:
			</td><td>
				<input type="text" id='parto_apgar' name='parto_apgar' size=5 style='text-align:center;'
				onKeyPress='if(event.which==13) seleccionar_parto(); '>
			</td>

		</tr>
	</table>
	<div class='sub-content2' id='partos'>
		(No hay registro de partos.)
	</div>
</div>
<div class='sub-content'>
	<table style='width:100%;'>
...
<script>
	...
	part[2]=['Orden','Condici&oacute;n','Sexo','Peso (gr)','APGAR 5 m','Bajar','Subir','Eliminar'];

	diag[3]=[-5,0,1,-1,-2,-3];
	diagf[3]=[-5,0,1,-1,-2,-3];
	pres[3]=[-5,0,1,2,-1,-2,-3];	
	tras[3]=[-5,1,2,-3];	
	part[3]=[-5,0,1,2,3,-1,-2,-3];	
	...
	part[4]=['text-align:right;width:20px;','text-align:center;','text-align:center;padding:2px;','text-align:center;','text-align:center;','',''];
    ...
    seleccionar_prestacion = function(d) {
		...
		$('nombre_medico').select();
		$('nombre_medico').focus();

    }

	seleccionar_parto=function() {
	
		insertar(listado_partos, 
			[	
				$('parto_cond').value,
				$('parto_sexo').value,
				$('parto_peso').value,

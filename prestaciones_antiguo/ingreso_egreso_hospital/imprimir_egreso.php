<?php
	...
	$h=cargar_registro("
		...
		date_part('month',age(now()::date, pac_fc_nac)) as edad_mes,
		date_part('days',age(now()::date, pac_fc_nac)) as edad_dias, 
	...
	$hosp_pac_ficha=espaciar(str_replace('PSI-','',$h['pac_ficha']));
	...
	if($hosp_leyprog==0) $hosp_leyprog='X';
    ...
    $hosp_pac_institucion=($h[0][68]);
    $hosp_pac_cod_institucion=($h[0][70]);
...
$pdf->Text(94, 33.5, 'INSTITUTO PSIQUIATRICO'); //Establecimiento
$pdf->Text(171, 33, '   0 0 0 0 0 '); //Codigo del establecimiento
...
$pdf->SetXY(110, 133.5); $pdf->Cell(14, 7, $servicio_ingreso,0,1,'L'); //año de admision
...
if($h['hosp_fecha_egr']!='') {
	...
	$pdf->SetXY(110, 157); $pdf->Cell(14, 7, $servicio_egreso,0,1,'L'); 
...
$pdf->SetXY(110, 116); $pdf->Cell(60, 5, $hosp_pac_institucion,0,1,'L'); // Establecimiento de procedencia
$pdf->SetXY(182, 112.5); $pdf->Cell(20, 5, $hosp_pac_cod_institucion,0,1,'L'); // Codigo Institucion

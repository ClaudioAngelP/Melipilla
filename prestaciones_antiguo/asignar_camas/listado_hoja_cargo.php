<?php 
	...
	$hc=cargar_registros_obj("
		...
		SELECT * FROM (
			SELECT 
			log_fecha, art_codigo, art_glosa, 
			SUM(-stock_cant) AS cantidad, forma_nombre,
			art_val_ult AS punit, 
			(SUM(-stock_cant)*art_val_ult) AS subtotal, 0 AS hosphc_id
			FROM cargo_hoja
			JOIN logs USING (log_id)
			JOIN stock ON stock_log_id=log_id
			JOIN articulo ON stock_art_id=art_id
			LEFT JOIN bodega_forma ON art_forma=forma_id
			WHERE pac_id=$pac_id AND stock_cant<0
			GROUP BY log_fecha, art_codigo, art_glosa, forma_nombre, art_val_ult
	...
	for($i=0;$i<sizeof($hc);$i++) {
		...
		<td style='text-align:right;'>".$hc[$i]['forma_nombre']."</td>

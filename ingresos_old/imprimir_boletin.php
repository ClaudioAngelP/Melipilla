<?php 
	...
	class PDF extends FPDF {
		function header() {
			...
			$this->Image('../imagenes/Logo_CRS.png',5,10,54,25);

			$this->Ln(20);
			$this->Cell(200,10,utf8_decode('Comprobante de Pago #'.number_format($bolnum,0,',','.')),0,0,'C');	
			$this->Ln();
		...
		function footer() {
			...
			$pdf->SetY(320);
			$pdf->SetFont('','B');	
			$pdf->Cell(100,6,$f['func_nombre'],0,0,'C');	
			$pdf->Cell(100,6,trim($c['pac_nombres'].' '.$c['pac_appat']),0,0,'C');
			$pdf->Ln();	
			$pdf->SetFont('','');	
			$pdf->Cell(100,6,'Funcionario Emisor',0,0,'C');	
			$pdf->Cell(100,6,'Paciente',0,0,'C');
			$pdf->Ln();
			$pdf->Ln();			
			$pdf->SetFontSize(10);
	...
	$pdf->Cell(50,10,utf8_decode('INSTITUTO PSIQUIÁTRICO'),0,0,'C');
	...
	$pdf->Cell(200,5,utf8_decode('Datos del Paciente'),1,0,'C',1);
	
	$pdf->Ln();

	$pdf->SetFillColor(200,200,200);
	
	$pdf->Cell(60,5,'R.U.T. Paciente',1,0,'C',1);	
	$pdf->Cell(140,5,'Nombre Paciente',1,0,'C',1);	
	$pdf->Ln();
	
	$pdf->Cell(60,5,$c['pac_rut'],1,0,'C');
	$pdf->Cell(140,5,($c['pac_nombres'].' '.$c['pac_appat'].' '.$c['pac_apmat']),1,0,'L');
	$pdf->Ln();
	...
	$pdf->Cell(200,5,utf8_decode('Detalle de Prestaciones'),1,1,'C',1);
	$pdf->SetFillColor(200,200,200);
	...
	if(!$paga_cuota) {
		...
		for($i=0;$i<sizeof($p);$i++) {
			...
			$pdf->SetFontSize(8);
			$pdf->Cell(120,5,trim($p[$i]['bdet_prod_nombre']),1,0,'L');
			$pdf->SetFontSize(10);
			$pdf->Cell(30,5,'$ '.number_format($p[$i]['bdet_valor2'],0,',','.').'.-',1,0,'R');
			$pdf->Cell(30,5,'$ '.number_format($p[$i]['bdet_valor'],0,',','.').'.-',1,0,'R');
			$pdf->SetFontSize(10);
			$pdf->Ln();
			$totalb+=round($p[$i]['bdet_valor']*1);
			$totalbb+=round($p[$i]['bdet_valor2']*1);
				...
				if($saldo>0) 
					$pdf->Cell(120,5,utf8_decode('Paga Cuota(s) Crédito #'.$cr['crecod']),1,0,'L');
				else
					$pdf->Cell(120,5,utf8_decode('Concluye Pago del Crédito #'.$cr['crecod']),1,0,'L');
				
				$pdf->SetFontSize(10);
				$pdf->Cell(30,5,'',1,0,'R');
				$pdf->Cell(30,5,'$ '.number_format($b['bolmon'],0,',','.').'.-',1,0,'R');
				...
				$pdf->Cell(20,5,'',1,0,'C');
				$pdf->SetFont('','I');				
				$pdf->Cell(120,5,utf8_decode('Saldo Pendiente $ '.number_format($saldo,0,',','.') ).'.-',1,0,'L');
				$pdf->SetFont('','');				
				$pdf->SetFontSize(12);
				$pdf->Cell(30,5,'',1,0,'R');
				$pdf->Cell(30,5,'',1,0,'R');
	...
	for($i=0;$i<sizeof($l);$i++) {
		...
		$pdf->Cell(20,5,'*',1,0,'C');
		$pdf->Cell(120,5,utf8_decode($texto),1,0,'L');
		$pdf->SetFontSize(12);
		$pdf->Cell(30,5,'',1,0,'R');
		$pdf->Cell(30,5,'- $ '.number_format($l[$i]['monto'],0,',','.').'.-',1,0,'R');
		$pdf->SetFontSize(10);
		$pdf->Ln();
	...
	if($totalbb>=0)
                $pdf->Cell(30,5,'$ '.number_format($totalbb,0,',','.').'.-',1,0,'R',1);
        else
                $pdf->Cell(30,5,'- $ '.number_format(-$totalbb,0,',','.').'.-',1,0,'R',1);

	if($totalb>=0)
		$pdf->Cell(30,5,'$ '.number_format($totalb,0,',','.').'.-',1,0,'R',1);
	else 
		$pdf->Cell(30,5,'- $ '.number_format(-$totalb,0,',','.').'.-',1,0,'R',1);
	$pdf->Ln();

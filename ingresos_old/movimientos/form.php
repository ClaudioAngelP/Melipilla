				...
				if(r) {
					...
					try {
						...
						if(r.clirut != null && r.clidv != null ) {
							...
							$('clirut2').value=r.clirut+'-'+r.clidv;
...
guardar_uso=function() {
	...
	for(var i=0;i<uso.length;i++) {
		...
		uso[i].us_bloqueo=$('r_'+i+'_us_bloqueo').value;
		uso[i].bolnum2=$('r_'+i+'_bolnum2').value;
...
redibujar=function() {
	...
	for(var i=0;i<uso.length;i++) {
		...
		html+='<input type="hidden" id="r_'+i+'_us_bloqueo" name="r_'+i+'_us_bloqueo" style="text-align:center;" value="'+u.us_bloqueo+'" />';
		html+='<input type="hidden" id="r_'+i+'_bolnum2" name="r_'+i+'_bolnum2" style="text-align:center;" value="'+u.bolnum2+'" />';
						
		if($('morosidad').value*1==0 && u.us_id==0) {
			...
			if($('morosidad').value*1==0) {
				...
				html+='<td><center><img src="iconos/cross.png" /></center></td>';	
			}
...
<input type='hidden' id='morosidad' name='morosidad' value='0'>
...
<div class='sub-content' style='height:320px;' id='selsep'>
<center>
<br /><br/>
<br /><br/>
...
style='height:320px;overflow:auto;display:none;'>
...
<script> 
	...
	<?php 
		if(isset($_GET['us_id'])) {
			...
			$u=cargar_registro("SELECT * FROM uso_sepultura WHERE us_id=".($_GET['us_id']*1));
	...
	?>
		...
		$('sel_clases').value='<?php echo $clase; ?>';
		
		select_codigos();
		
		$('sel_codigos').value='<?php echo $codigo; ?>';
		
		if($('sel_codigos').value!='<?php echo $codigo; ?>') {
			$('sel_codigos').value='-1';
			$('sel_codigon').value='<?php echo $codigo; ?>';
		}
				
		$('numero').value='<?php echo $numero; ?>';
		$('letra').value='<?php echo $letra; ?>';

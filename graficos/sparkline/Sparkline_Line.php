...
class Sparkline_Line extends Sparkline {
  ...
  function Render($x, $y) {
    ...
    while (list(, $v) = each($this->featurePoint)) {
      ...
      $this->DrawCircleFilled($this->dataSeriesConverted[1][$v['ptX']][0] + $this->graphAreaPx[0][0],     $this->dataSeriesConverted[1][$v['ptX']][1] + $this->graphAreaPx[0][1],     $v['diameter'], 
                              $v['color'], 
                              $this->imageHandle);
      $this->DrawTextRelative($v['text'],
                              $this->dataSeriesConverted[1][$v['ptX']][0] + $this->graphAreaPx[0][0],     $this->dataSeriesConverted[1][$v['ptX']][1] + $this->graphAreaPx[0][1],     $v['color'], 
                              $v['textpos'], 
                              round($v['diameter'] / 2),
                              $v['font'],
                              $this->imageHandle);

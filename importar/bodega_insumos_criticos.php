<?php
    ...
    $bod_id=25;
    $f=explode("\n", file_get_contents('stock_critico_abastecimiento.csv'));
    ...
    for($i=1;$i<sizeof($f);$i++)
    {
        ...
        if(!$chk)
            pg_query("INSERT INTO stock_critico VALUES ($art_id, $pedido, $critico, $bod_id, $gasto);");
        else
            pg_query("UPDATE stock_critico SET critico_pedido=$pedido, critico_critico=$critico, critico_gasto=$gasto WHERE critico_bod_id=$bod_id AND critico_art_id=$art_id");
    }
    pg_query("COMMIT;");
?>
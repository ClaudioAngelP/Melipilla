<?php 
    ...
    $fa=explode("\n",file_get_contents('CORRECION DE CLASIFICACION.csv'));
	...
	$nom=trim(strtoupper($r[1]));
        $clasificacion=trim(strtoupper($r[2]));
	...
	$f=cargar_registro("SELECT * FROM bodega_forma WHERE forma_nombre='$forma'");
         ...
         */
        //----------------------------------------------------------------------
        $c=cargar_registro("SELECT * FROM bodega_clasificacion WHERE upper(clasifica_nombre)=upper('$clasificacion')");
        if($c)
        {
        ...
        {
            print("<br>");
            print("INSERT INTO bodega_clasificacion VALUES (DEFAULT, '$clasificacion', '');");
            print("<br>");
	...
	$chk=cargar_registro("SELECT * FROM articulo WHERE art_codigo='$cod'");
        ...
        else
        {
            ...
            print("<br>");
            print("UPDATE articulo set art_clasifica_id=$clase_id where art_id=".$chk['art_id']."");
            print("<br>");
            pg_query("UPDATE articulo set art_clasifica_id=$clase_id where art_id=".$chk['art_id']."");

<?php 
	...
	$l=explode("\n",utf8_decode(file_get_contents("cupos_agenda_felix.csv")));
...
//		$frut=strtoupper(str_replace('.','',trim($r[8])));
		...
		if(trim($r[5])=='' OR trim($r[6])=='') continue;
		...
		$cupos=Array($r[8]*1,$r[10]*1,$r[12]*1,$r[14]*1,$r[16]*1,$r[17]*1);
  ...
  for(;$di<=$df;$di+=86400) {
    ...
    $ds=(date('w',$di)*1)-1;
    if($ds==-1) $ds=6;
    if(trim($dias[$ds])=='') continue;
    
    // Evita dias de ausencia médica global o propia del médico...
    
    $d=date('d/m/Y', $di);
    if($fechas_ausente AND array_search($d, $fechas_ausente)) continue;

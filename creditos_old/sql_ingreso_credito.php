<?php 
	...
	if($chk2) {
		pg_query('ROLLBACK;');
		die('COMPROBANTE DE PAGO # '.number_format($chk['bnum'],0,',','.').' YA EXISTE ( FECHA: '.$chk2['bolfec'].' ), EL ADMINISTRADOR DEBE CORREGIR LA NUMERACIÓN ACTUAL DE LOS BOLETINES.');
	} else {
		$bolnum = ($chk['bnum']*1);
	}	
	...
	for($i=0;$i<sizeof($presta);$i++) {
		...
		$presta_id=$presta[$i]->presta_id*1;

		pg_query("INSERT INTO boletin_detalle VALUES (
			DEFAULT, $currval, 
			".$presta[$i]->presta_id.", ".($valorp).",
			0, 
			'".pg_escape_string(html_entity_decode($presta[$i]->glosa))."',		
			'".pg_escape_string($presta[$i]->codigo)."', ".($presta[$i]->valor*1)."		
		);");
		
		// FLAG DE PAGO DE PRESTACION
		// NO APARECERÁ DENUEVO EN LISTADO...
		pg_query("UPDATE nomina_detalle SET nomd_pago=1 WHERE nomd_id=$presta_id;");

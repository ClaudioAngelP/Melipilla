<?php
  ...
  {
        GLOBAL  $tabla, $campos, $query, $nquery, $flds, $condiciones, $condiciones_eval, $vals;
        ...
        for($i=0;$i<count($campos);$i++)
            $query = str_replace(   '[%'.$campos[$i][0].']',$vals[$campos[$i][0]],$query);
        
        
        for($i=0;$i<count($campos);$i++)
        {
            if(strstr($campos[$i][2],'|'))
            {
                $array_campos=explode("|", $campos[$i][2]);
                $campo[$i][2]=($array_campos[0]*1);
                switch($campo[$i][2])
                {
                    ...
                    case 2:
                        if($vals[$campos[$i][0]]=="")
                        {
                            $query = str_replace('[%and recetad_art_id='.$vals[$campos[$i][0]].']','',$query);
                        }
                        else
                        {
                            $query = str_replace('select distinct on (receta_id)','select distinct',$query);
                            $query = str_replace('[%and recetad_art_id='.$vals[$campos[$i][0]].']','and recetad_art_id='.$vals[$campos[$i][0]].'',$query);
        ...
        $nquery = preg_replace($condiciones, $condiciones_eval, $query);
    ...
    function procesar_formulario($nombre, $callback='')
    {
        ...
        $visualizaciones= Array(Array(0, 'En Pantalla'),Array(1, 'En Impresora'), Array(2, 'Archivo XLS'));
        ...
        if(isset($_POST['form_submit']))
        {
            ...
            if($forma==0 OR $forma==1 OR $forma==2)
            {
                ...
                if($forma==0 OR $forma==1)
                    $html.='<head>'.cabecera_popup_head('../../..').'</head>
                    <body class="fuente_por_defecto '.$background.'" onLoad="'.$print_script.'">
                        <table style="width:100%;">
                            <tr class="tabla_header">';
                if($forma==2)
                    $html.='<body><table style="width:100%;" border=1><tr class="tabla_header">';
                ...
                <tr>
                    <td colspan='.$colnum.' style="text-align:center;font-weight:bold;">
                        <h2><u>'.$nombre.'</u></h2>
                    </td>
                </tr>
                <tr>
                    <td colspan='.$colnum.'>&nbsp;</td></tr>';
                for($i=0;$i<count($campos);$i++)
        ...
        for($i=0;$i<count($formato);$i++)
        {
            ...
            if(count($formato[$i])==5)
            {
                //$vals[$campos[$i][0]]
                if($vals[$campos[$formato[$i][4]][0]]!="")
                {
    ...
    function generar_campo($campo, &$html, &$script, &$val_script)
    {
        ...
        if(strstr($campo[2],'|'))
        {
            $array_campos=explode("|", $campo[2]);
            $campo[2]=($array_campos[0]*1);
            if(($array_campos[1]*1)==1)
            {
                $requerido=true;
            }
            else
        ...
        switch($campo[2])
        {
            ...
            case 1:     // Seleccionar Fechas
                if(!isset($campo[3]))
                    $campo[3]=date('d/m/Y');
                $html.='<input type="text" '.$tag_ids.' size=15 style="text-align:center;" value="'.$campo[3].'">';
                $html.='<img src="iconos/date_magnify.png" id="img_'.$campo[0].'">';
            ...
            case 2:     // Seleccionar Artículos
                ...
                $html.='<input type="text" id="art_codigo_'.$campo[0].'" name="art_codigo_'.$campo[0].'" size=10 style="text-align:left; font-size:11px;" onDblClick="liberar_articulo();" autocomplete="off">';
                $html.='<input type="text" id="art_glosa_'.$campo[0].'" name="art_glosa_'.$campo[0].'" size=35 DISABLED style="text-align:left; font-size:11px;">';
                ...
                {
                    $('form_".$campo[0]."').value='';
                    $('art_codigo_".$campo[0]."').value='';
                    $('art_glosa_".$campo[0]."').value='';
                }
                ";
                
                if($requerido)
                {
                    $val_script.="if($('form_".$campo[0]."').value*1==0)
                    {

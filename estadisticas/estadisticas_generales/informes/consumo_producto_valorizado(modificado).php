    ...
    "
     SELECT
       ...
       ABS(SUM(stock_cant)) as cant_despachada,
        [if %tipo==-1]
                ((sum(ABS(stock_cant))* art_val_min)) as valor [/if]
            [if %tipo==2]
                ((sum(ABS(stock_cant))* art_val_med)) as valor [/if]
            [if %tipo==3]
                ((sum(ABS(stock_cant))* art_val_max)) as valor [/if]
            [if %tipo==4]
                ((sum(ABS(stock_cant))* art_val_ult)) as valor [/if]
       FROM stock
